...
TL;DR: 研究了一种在光阱中使用 Dipolar 分子模拟量子统计体性质的新方法，通过使用圆偏振微波驱动分子的旋转能级来产生长程相互作用。发现当 $N=0,J=1/2,F=1$ 态与 $N=1,J=3/2,F=2$ 能级相互作用时，可以得到一种特殊的自旋 1 量子磁性哈密顿量。一维情况下，通过张量网络方法计算出的相图中包含著名的 Haldane 相，即使考虑到 SU(3) 校正项，Haldane 相仍然存在。讨论了实验可能性，提出了未来使用激光冷却分子 MgF 进行实验证实方案。


<details>
  <summary>Details</summary>
Motivation: 研究量子统计体和强长程相互作用下的量子相变，为量子物质的模拟提供新的工具。

Method: 使用圆偏振微波驱动 $^2Σ$ 分子的旋转能级，结合磁场和分子光栅，进行了理论分析和数值模拟。

Result: 在量子磁性哈密顿量下，一维情况下发现 Haldane 相，并且此相在 SU(3) 校正下仍然稳定存在。

Conclusion: 光阱中的 Dipolar 分子提供了一种新的研究量子统计体和强相互作用相变的平台。
...
TL;DR: This paper discusses advanced inertial sensing technology, highlighting the ability to improve sensor accuracy and dynamic range using atom interferometry.


<details>
  <summary>Details</summary>
Motivation: Inertial navigation systems with high accuracy and wide dynamic range are currently a critical challenge in navigation technology.

Method: The authors developed a dual-channel closed-loop operation system for atomic beam interferometers using Raman frequency modulation.

Result: They successfully achieved precision measurements of $m{m{m{m{m{1^{m{a}}/s}}}}}$ in rotation and $m{0.17m{g}}$ in acceleration.

Conclusion: This breakthrough enhances the practicality of atom interferometry for real-world inertial navigation applications.
...
TL;DR: 文章提出了一种基于非连续极端紫外谐波的自参考方法，用于验证渐近近似在分离两光子延迟方面的有效性。实验结果表明，渐近近似预测的相位与实际测量的相位存在几毫弧度的差异（相当于几阿托斯瀑布），并一致了全维度仿真。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在探索光电子释放过程的精细时间分辨率，为理解光致逃逸机制和开发新奇的活性分析技术提供理论支持。

Method: 研究采用自参考方法，基于非连续极端紫外谐波，并结合全维度仿真进行理论分析。

Result: 实验结果显示出渐近近似在分离两光子延迟方面存在局限性，预测与实验结果的差异达几毫弧度（相当于几阿托斯瀑布），表明渐近近似在高精度时分辨率达到极限。

Conclusion: 该研究揭示了渐近近似在极短时间尺度下的适用范围，为未来研究指明了新方向。
...
TL;DR: 该论文提出了一种集成方法来生成短 lived同位素分子及其光谱，展示了在射频 Matt 装置中形成分子离子束，并使用激光光谱技术进行高分辨率测量。


<details>
  <summary>Details</summary>
Motivation: 探测短寿命分子用于研究超越标准模型的新物理现象，尽管制造和光谱测量技术上具有挑战性。

Method: 采用了分子离子束形成结合线性激光光谱技术的方法，在射频 matt 装置中产生分子离子，并进行了高分辨率激光光谱分析。

Result: 利用该工艺成功生产如 BaF 和 YbF 的分子离子，并对 138BaF 进行了高分辨率光谱分析，得到了其振动和旋转光谱结构。

Conclusion: 该方法为未来在射频离子设施中形成和光谱研究短寿命放射性分子提供了实用的途径。
...
### [8] [Adaptive tensor train metadynamics for high-dimensional free energy exploration](https://arxiv.org/abs/2603.13549)
*Nils E. Strand,Siyao Yang,Yuehaw Khoo,Aaron R. Dinner*

Main category: physics.chem-ph

TL;DR: 本文提出了一种名为TT-Metadynamics的方法，用于提高多元 Collective Variables (CV)下的分子动力学模拟的采样效率。通过将传统的Metadynamics中的高维偏势函数压缩为张量训练( Tucker Tensor Train, TT)表示，TT-Metadynamics能够以线性规模增长的计算成本进行模拟，从而在大量CV的情况下显著提高了效率。


<details>
  <summary>Details</summary>
Motivation: 随着分子动力学模拟在研究复杂系统中的重要性增加，特别是在涉及多个CV的情况下，如何高效地探索自由能景观成为了关键挑战。传统的Metadynamics方法在高维情况下的计算成本急剧增加，这限制了其应用范围。

Method: 本文通过将Metadynamics偏势函数压缩为TT表示，结合线性缩放的构建算法，提出了TT-Metadynamics。TT表示不仅节省了内存，还使得计算成本与模拟时间无关。

Result: 实验结果表明，TT-Metadynamics在处理最多14个CV的基准系统时，其精度与传统Metadynamics相当或更高，尤其是在系统具有高能垒的情况下表现尤为突出。

Conclusion: TT-Metadynamics提供了一种在多个CV情况下高效且精确的自由能计算方法，为研究复杂分子系统奠定了基础。
...
### [9] [A Primary Unified Geometric Framework of Molecular Reaction Dynamics Based on the Variational Principle](https://arxiv.org/abs/2603.13923)
...
### [10] [Systematically Improvable Numerical Atomic Orbital Basis Using Contracted Truncated Spherical Waves](https://arxiv.org/abs/2603.13995)
...
### [11] [Nonadiabatic rare events from transition-path sampling of MASH trajectories](https://arxiv.org/abs/2603.14102)
*Danial Ghamari,Jeremy O. Richardson*

Main category: physics.chem-ph

TL;DR: The paper presents a novel framework combining MASH and transition-path sampling to efficiently simulate rare nonadiabatic reactions, enabling detailed analysis of reaction mechanisms and statistical properties.


<details>
  <summary>Details</summary>
Motivation: Understanding and simulating rare nonadiabatic reactions is crucial for many molecular processes, but traditional methods are insufficient due to their computational demands. This paper introduces an innovative approach to address these challenges.

Method: The authors combine the Mapping Approach to Surface Hopping (MASH) with transition-path sampling. MASH is a recent development in molecular dynamics that allows for Markovian, time-reversible, and Liouville-satisfying simulations. Transition-path sampling is a technique used to study rare events by focusing on reactive trajectories. The integration of these two methods allows for the efficient simulation of nonadiabatic processes without biasing the underlying dynamics.

Result: The combined approach successfully demonstrates the ability to generate nonadiabatic reactive pathways and analyze reaction mechanisms. The method is applied to a spin-boson model under thermal equilibrium across various diabatic coupling strengths, showing its effectiveness in studying rare processes that are beyond the scope of brute-force simulations.

Conclusion: This paper provides a practical and systematic tool for investigating rare nonadiabatic processes, significantly advancing the field of molecular dynamics and reaction mechanism analysis.
...
### [12] [The Python Simulations of Chemistry Framework: 10 years of an open-source quantum chemistry project](https://arxiv.org/abs/2603.14155)
...
### [13] [Auto-WHATMD : Automated Wasserstein-based High-dimensional feature extraction Analysis of Trajectories from Molecular Dynamics](https://arxiv.org/abs/2603.14414)
*Sosuke Asano,Ikki Yasuda,Katsuhiro Endo,Yoshinori Hirano,Kenji Yasuoka*

Main category: physics.chem-ph

TL;DR: The paper introduces auto-WHATMD, a method using optimal transport and simulated annealing to identify key residues in protein-ligand systems that correlate with ligand binding affinities.


<details>
  <summary>Details</summary>
Motivation: The need for efficient residue prioritization in protein-ligand systems for comparative analysis and feature design.

Method: Combines optimal transport distance for trajectory comparison with simulated annealing to identify distinguishing residues.

Result: auto-WHATMD successfully identifies key residues in bromodomain 4 variants, even with limited data, aiding ligand-binding affinity prediction.

Conclusion: This method provides an automated approach for residue selection, enhancing analysis of analogous protein systems.
...
### [14] [Carbon black and hydrogen production from methane pyrolysis: measured and modeled insights from integrated gas and particle diagnostics in shock tubes](https://arxiv.org/abs/2603.14314)
*Gibson Clark,Mohammad Adib,Chengze Li,Taylor M. Rault,Jesse W. Streicher,Enoch Dames,M. Reza Kholghy,Ronald K. Hanson*

Main category: physics.chem-ph

TL;DR: 该研究通过实验和模型相结合的方法研究了甲烷在反射激波波后pyrolysis的作用，为CH4转化为氢气和碳黑的生产提供了重要的基准。该研究关注了甲烷转化为乙烷和乙炔的过程，验证了不同条件下甲烷分解产物的形成，并通过激光吸收诊断、多波长消 [[[ extinction]]]和透射电镜等手段，详细分析了颗粒的形成和形态变化。研究发现，尽管小分子的形成在模拟中表现得相对一致，但对于大型分子如多环芳香烃（PAHs）的预测存在较大差异。同时，结合气体和颗粒的模拟能够较好地捕捉体积分数的变化，但由于计算资源的限制，在高温条件下预测颗粒的形成和动态过程仍存在限制。研究还发现，随着温度的升高，颗粒的大小逐渐减小，而碳纳米结构逐渐增大。这些结果为开发更精确的模型提供了重要的参考。


<details>
  <summary>Details</summary>
Motivation: 随着全球对清洁能源的需求增加， methane (CH4) 的绿色转化方法受到广泛关注。在这类方法中， pyrolysis 具有独特的优势，因为它既能生产氢气和焦炭，又能避免传统蒸汽甲烷重整和焦化过程中的污染。因此，开发一种高效可靠的方法来模拟和优化 methane 的 pyrolysis 过程具有重要的理论意义和实际应用价值。

Method: 该研究采用实验和数值模拟相结合的方法。实验部分通过反射激波装置对 5% 的 methane/氩混合气体在不同温度和压力条件下进行 pyrolysis，并利用激光吸收诊断技术、多波长消 extinction 技术和透射电镜进行测量。数值模拟部分则使用了结合气体和颗粒动力学的模型，对不同的物理和化学过程进行了模拟。

Result: 研究发现，实验数据在小分子的形成和分布上与数值模拟结果基本一致，但对大型分子如 PAHs 的预测存在较大差异，这表明模型在捕捉颗粒物多环芳香烃的形成过程中的局限性。此外，数值模拟能够较好地捕捉到颗粒的体积分数的变化，但对高温条件下的颗粒形成和动态过程预测不够准确。此外，实验结果还表明，随着温度的升高，颗粒的大小逐渐减小，而碳纳米结构的密度逐渐增加。这些结果为开发更精确的模型提供了参考。

Conclusion: 本研究通过实验和数值模拟相结合的方法，系统研究了 methane 在反射激波波后的 pyrolysis 过程，详细分析了分子分解产物、颗粒物形成和形态的变化。尽管实验数据与模型在小分子成分的预测上基本一致，但对于大型分子如 PAHs 的预测仍存在较大差异，表明需要进一步改进模型。同时，研究也揭示了温度和压力对颗粒物性质变化的影响规律，为未来研究提供了重要参考。该研究为开发更精确的 methane pyrolysis 模型奠定了基础。
...
### [15] [Explicit, Machine-Learned Two-Body Potentials for Molecular Simulations](https://arxiv.org/abs/2603.14466)
...
### [16] [Stochastic Collision Theory of Magnetism in Radical Fluids](https://arxiv.org/abs/2603.14677)
*Yoshiaki Uchida,Ryohei Kishi*

Main category: physics.chem-ph

TL;DR: 该研究通过量子主方程模型解释了微观随机事件如何导致宏观确定性特性，在受限径向聚合物溶液中，随机的分子碰撞主导系统的磁性特性。理论发现，一级交換贡献在碰撞中平均为零，而二级贡献成为有效共铁磁耦合，从而加强了磁性。该模型解释了实验中观察到与传统理论不符的现象，并可能适用于更广泛的软物质现象，如液晶体。


<details>
  <summary>Details</summary>
Motivation: 通过量子主程模型理解微观随机事件如何影响宏观磁性特性，可能是研究物理基础的重要进展。

Method: 研究使用了量子主程模型，并结合分子碰撞机制来解释磁性特性。

Result: 理论得出二级交換贡献成为有效共铁磁耦合，并解释了实验中观察到的现象与传统理论不符的特性。

Conclusion: 微观随机事件通过统计平均导致宏观磁性特性的共铁磁耦合，这可能适用于更广泛的软物质现象。
...
### [17] [Design Space of Self--Consistent Electrostatic Machine Learning Interatomic Potentials](https://arxiv.org/abs/2603.14700)
*William J. Baldwin,Ilyes Batatia,Martin Vondrák,Johannes T. Margraf,Gábor Csányi*

Main category: physics.chem-ph

TL;DR: 机器学习势能函数（MLIPs）在原子间模拟中得到广泛应用。尽管传统模型基于短程原子能量贡献，且局部性假设在许多现代基础模型中仍然存在。尽管这种方法使建模在许多情况下既高效又准确，但对于涉及长程电静力、电荷转移或诱导极化等系统的场景，存在局限性。本文提出了一种框架，通过将现有模型视为密度泛函理论（DFT）的粗略近似来处理MLIP中的电静力问题。这种方法明确了近似假说，明确了学习量的物理含义，并揭示了几种先前提出模型之间的联系和等价性。基于此框架，识别了构建自洽电静力MLIP的关键设计选择。在MACE架构和共享电荷密度表示的实现中，实现了几种不同模型的关键点，实现了在不同方法间进行的有控比较。在此基础上，我们在金属-水界面和硅酸盐中的有缺陷处评估了这些模型。结果揭示了现有方法的局限性，并证明了更表达式的自洽模型是解决失败的必要选择。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习势能函数在原子间模拟中的广泛应用，探索MLIP在处理长程电静力方面的潜力变得尤为重要。本文通过将MLIP视为DFT的粗略近似，为MLIP中的电静力建模提供了一种新框架，有助于更深入理解现有模型的假设和局限性。

Method: 本文的方法包括：1）将现有MLIP模型视为密度泛函理论的粗略近似；2）从DFT出发，明确模型的近似假说；3）通过MACE架构和共享电荷密度表示实现关键点；4）基于有控比较评估不同模型的性能。

Result: 通过评价金属-水界面和硅酸盐中的有缺陷处，本文揭示了现有MLIP方法的局限性，并表明更Expressive的自洽MLIP模型以解决现有方法的不足。

Conclusion: 本文提出了一种新框架，通过将MLIP视为DFT的粗略近似来处理电静力问题，并展示了该框架在评估现有模型和开发新模型中的有效性。未来研究应进一步探讨自洽MLIP模型的设计和实现。
...
### [18] [Reproducible Orchestration of Best Practices for Reaction Path Optimization with the Nudged Elastic Band](https://arxiv.org/abs/2603.14737)
*Rohit Goswami*

Main category: physics.chem-ph

TL;DR: The paper introduces an automated workflow using Snakemake and PET-MAD for NEB calculations, improving reproducibility and accuracy by automating the NEB method steps.


<details>
  <summary>Details</summary>
Motivation: The need for reproducible and error-free computational chemistry workflows motivates the development of this automated NEB method implementation.

Method: The method involves using Snakemake to create a dependency graph for NEB calculations, integrating machine learning potentials with eOn software. It automates endpoint minimization, alignment, and path generation.

Result: The workflow successfully automates NEB calculations for HCN isomerization, recovering the expected energy profile and product without manual intervention.

Conclusion: The work provides a robust, automated NEB implementation that enhances computational reliability in chemical kinetics studies.
...
### [19] [Hierarchy of extreme-event predictability in turbulence revealed by machine learning](https://arxiv.org/abs/2603.13789)
...
### [20] [Tipping resonance in a chaotically forced ice age model](https://arxiv.org/abs/2603.15043)
...
### [21] [Storage and selection of multiple chaotic attractors in minimal reservoir computers](https://arxiv.org/abs/2603.15155)
*Francesco Martinuzzi,Holger Kantz*

Main category: nlin.CD

TL;DR: Minimal reservoirs can store multiple chaotic systems but struggle with cue-based switching.


<details>
  <summary>Details</summary>
Motivation: Studying multi-attractor learning in reservoir computing to determine if they can handle task switching.

Method: Tested 28 system pairs using eight chaotic systems with ten reservoir topologies.

Result: Minimal reservoirs show average 45% success in flexibility, but no single topology outperformed others in storage or switching.

Conclusion: Minimal substrates can model multiple attractors but need better memory for cue-dependent changes.
...
### [22] [Pair beams unlock beyond-terawatt attosecond free-electron laser pulses](https://arxiv.org/abs/2603.15407)
...
### [23] [A Block Belief-Propagation Algorithm for the Contraction of Tensor-Networks](https://arxiv.org/abs/2603.13304)
*Nir Gutman*
...
### [24] [Modeling Energy Relaxation via Quantum Thermalization: A Superconducting Qubit Coupled to a Many-Body TLS System](https://arxiv.org/abs/2603.13322)
*Xue-Yi Guo*

Main category: quant-ph

TL;DR: 研究了超导量子比特与多体通量系统之间因热化驱动的非均衡衰减。通过数值模拟发现，热化抑制了量子比特与多体通量之间的相干能量交换，导致指数能量衰减。失真时间与耦合强度的平方成反比，且受多体通量系统的内部耦合强度、通量频率波动速率及热激发通量粒子数的影响。为理解量子比特能量衰减和相干性消散提供新的视角，可能对其他开放量子体系的消散场景有指导意义。


<details>
  <summary>Details</summary>
Motivation: 量子比特的能量衰减和相干性消散是量子计算中的重要问题。通过研究量子比特与多体通量系统之间的能量交换机制，可以更好地理解量子系统的稳定性并为其设计提供理论支持。

Method: 通过构建包含量子比特与多体通量系统的量子动力学模型，并运用数值模拟手段研究了不同参数下系统的动力学行为。

Result: 1. 热化驱动的非均衡过程显著减缓了量子比特与多体通量之间的能量交换。2. 失真时间与量子比特与多体通量之间的耦合强度呈反平方关系。3. 多体通量系统的内部分耦合率、通量频率波动速率及热激发通量粒子数均对失真时间有显著影响。

Conclusion: 该研究为理解量子比特在开放量子系统中的能量衰减机制提供了新的理论视角，且可能为量子比特保护和纠错编码策略提供指导。
...
### [25] [A stabilizer $\mathrm{AME}(4,6)$ state does not exist](https://arxiv.org/abs/2603.13442)
*Hyunho Cha*

Main category: quant-ph

TL;DR: 论文证明了在四个六维量子系统中，不存在稳定器完全最大纠缠态。


<details>
  <summary>Details</summary>
Motivation: 纠缠态在量子信息处理中具有重要性，特别是在量子通信和量子计算中。

Method: 论文通过分析量子系统中态的性质和相关定理来得出结论。

Result: 不存在满足条件的态。

Conclusion: 研究结果展示了纠缠态的限制性，这对量子系统的设计和运算具有理论意义。
...
### [26] [Distance learning from projective measurements as an information-geometric probe of many-body physics](https://arxiv.org/abs/2603.13485)
*Oleksii Malyshev,Simon M. Linsel,Fabian Grusdt,Annabelle Bohrdt,Eugene Demler,Ivan Morera*

Main category: quant-ph

TL;DR: We use neural distance learning to identify quantum phases and their transitions by clustering snapshots of quantum states and analyzing their statistical divergences.


<details>
  <summary>Details</summary>
Motivation: Understanding quantum phases and phase transitions is fundamental to quantum many-body physics. Traditional methods often struggle with high-dimensional data, so we seek an alternative approach that leverages machine learning to analyze quantum state distances more effectively.

Method: We employ neural distance learning, using a single neural discriminator to estimate Csiszar f-divergences between quantum state distributions. This unsupervised method allows us to cluster snapshots and infer phases and phase transitions without explicit dimensionality reduction.

Result: Our method successfully identifies quantum phases and their transitions. For models with conventional order parameters, we accurately recover phase boundaries and critical exponents. In the case of topological order, we classify distinct topological phases and pinpoint phase transitions. For higher-order correlated systems, we detect non-trivial correlation regimes.

Conclusion: Our framework provides a versatile, unsupervised method for studying quantum phases through distance learning, outperforming traditional representation-based approaches and enabling the exploration of higher-order correlations.
...
### [27] [Algebraic Structure of Quantum Controlled States and Operators](https://arxiv.org/abs/2603.13454)
...
### [28] [Quantum Correlations and Entanglement in Generalized Dicke-Ising Models](https://arxiv.org/abs/2603.13693)
*Santiago F. Caballero-Benitez*

Main category: quant-ph

TL;DR: 研究高Q腔中的量子系统如何通过光控制激发对称性及其与量子物质的相互作用。


<details>
  <summary>Details</summary>
Motivation: 探索光和量子系统之间的相互作用，利用高Q腔作为理想测试平台。

Method: 结合光-物质强相互作用和DMRG算法，模拟和分析量子体系。

Result: 发现了超射滤光态、量子色数字和光诱导多模式结构等新现象，提供了研究量子纠缠态和强量子相关性的新方法。

Conclusion: 利用高Q腔和光诱导的多模式结构，成功模拟和操控量子系统，具备广泛的应用前景。
...
### [29] [Characterization of Radiation-Induced Errors in Superconducting Qubits Protected with Various Gap-Engineering Strategies](https://arxiv.org/abs/2603.13460)
...
### [30] [Hadamard regularization of open quantum systems coupled to unstructured environments in the Schwinger-Keldysh formalism](https://arxiv.org/abs/2603.13462)
*Jakob Dolgner*

Main category: quant-ph

TL;DR: 研究了开放量子系统中快慢系统之间的分离标度效应，提出了一个基于Hadamard正则化的时序算法来处理快慢系统的效果，关键点是考虑了慢系统时间尺度上的Kadanoff-Baym方程的数值求解。


<details>
  <summary>Details</summary>
Motivation: 当前的开放量子系统理论在凝聚态物理和量子信息中非常重要，而快慢系统分离标度的问题也是很多实际问题的核心难点。

Method: 提出了一个基于Hadamard正则化的时序算法来处理快慢系统的效果，具体包括快环境对于慢系统的影响、低温度下的非马尔科夫效应、以及重正化效应的计算。

Result: 该方法成功开发出一种可用于计算快慢系统相互作用的高效数值方法，在涉及快慢分离标度的量子系统中，显著提高了计算效率。

Conclusion: 该研究为处理快慢量子系统提供了 vouchers的 numerical framework，有效解决了在实际计算中计算资源有限的问题。
...
### [31] [Agnostic Dynamical Decoupling for Single-Qubit Gates](https://arxiv.org/abs/2603.13477)
*Gumaro Rendon*

Main category: quant-ph

TL;DR: The paper presents a method for designing smooth single-qubit control pulses that implement a desired gate while suppressing unknown static error sources to first order. The approach is agnostic to the detailed form of the target-bath interaction and can also suppress mediated couplings between qubits to first order.


<details>
  <summary>Details</summary>
Motivation: The motivation for this work is the need for robust and reliable single-qubit gate operations in the presence of static noise and mediated qubit-qubit interactions, which are common sources of errors in quantum computing.

Method: The method involves parametrizing the control propagator through an auxiliary matrix expansion over orthogonal basis functions. Decoupling is achieved through algebraic orthogonality and equal-norm constraints on the expansion coefficients. The approach also utilizes a discrete cosine transform parametrization to cast the pulse-synthesis problem into a numerically stable constrained optimization with minimal free parameters.

Result: Numerical examples demonstrate that the proposed method achieves smooth control fields that realize the target gates while remaining robust against arbitrary static single-qubit noise and mediated multi-qubit couplings.

Conclusion: This work provides a hardware-friendly route toward implementing noise-agnostic, dynamically corrected single-qubit gates with reliable performance despite the presence of static and dynamic noise sources.
...
### [32] [Electrometry of extremely-low frequencies from kHz to sub-Hz with a Rydberg-atom sensor](https://arxiv.org/abs/2603.13827)
...
### [33] [A Chip-Scale Transmitter Module for Real-Time Continuous-Variable QKD](https://arxiv.org/abs/2603.13483)
...
TL;DR: 该论文提出了一种基于标准光纤网的连续变量量子密钥分发（CV-QKD）系统，通过集成芯片级混合 transmitter，实现了在102公里光纤中的秘密密钥生成，并显著降低了光学组件的尺寸。系统实现了实时操作，突破了传统实验设置的离线后处理瓶颈，为可扩展的、低成本的量子网络技术提供了可部署的基础。


<details>
  <summary>Details</summary>
Motivation: 随着量子通信技术的发展，如何在标准光纤上实现高效的、可扩展的量子密钥分发系统变得尤为重要，尤其是在成本和实用性的考虑下。

Method: 在本论文中，作者使用了微光外置腔激光器与monicophotonic集成的相位调制器的混成 transmitter，整合了标准的 telecom 光件，以实现芯片规模的缩减和高效率的实时操作。

Result: 文中实现了跨越102公里光纤中的秘密密钥生成，光学组件尺寸缩减95%，并且实现了实时操作，显著减少了传统离线处理所需的时间与资源，这证明了实验室实现向实际部署的过渡。

Conclusion: 这项工作在实验室与实际部署之间架起了桥梁，通过可扩展架构为量子网络提供了低成本的可扩展技术基础。
...
### [34] [Phononic Bragg Reflectors for Thermal Insulation of Scalable Cryogenic Control Electronics from Qubits](https://arxiv.org/abs/2603.13726)
*Isabelle V. Sprave,Denny Dütz,Sebastian Kock,René Otten,Tobias Hangleiter,Felix Mende,Marcus Wislicenus,Hendrik Bluhm*

Main category: quant-ph

TL;DR: The paper presents a cryopackaging solution using Ta/SiO2 DBRs to isolate qubits from high thermal loads, achieving efficient cooling even with limited refrigeration.


<details>
  <summary>Details</summary>
Motivation: Quantum computing requires extremely low temperatures for qubits, but control electronics need higher temperatures for cooling, creating a thermal mismatch. This paper addresses this issue by proposing a cryopackaging solution.

Method: Experiments involve fabricating and characterizing Ta/SiO2 DBR structures to act as a thermal barrier, demonstrating effective insulation.

Result: The DBRs achieve sub-1 mW/cm² thermal conduction from 1.5 K to 100 mK, enabling Watt-level cooling while maintaining qubit temperatures around 100 mK.

Conclusion: This solution effectively bridges the thermal gap, potentially enabling large-scale quantum computing.
...
### [35] [Velocity-Enabled Quantum Computing with Neutral Atoms](https://arxiv.org/abs/2603.15561)
...
### [36] [Qubit syndrome measurements with a high fidelity Rb-Cs Rydberg gate](https://arxiv.org/abs/2603.13492)
...
TL;DR: 他们实现了Rubidium和Cesium两种原子之间的离子级联量子门，信噪比达到0.975±0.002。这种多原子阵列在原位实现了量子错误纠正的关键功能。


<details>
  <summary>Details</summary>
Motivation: 量子位错误修复是量子计算中的关键问题，他们在此方面的突破可能有助于增强量子计算的可靠性。

Method: 他们利用离子级联量子位实现原子间的量子门，构建了多原子阵列，成功实现了QND测量。

Result: 构建的多原子阵列可以进行在原位的量子非破坏性测量，展示了二维和三维量子错误纠正的能力，相关度分别为0.933(12)和0.865(17)。

Conclusion: 他们成功实现了多原子量子位在原位的量子非破坏性测量，对量子位检测的增进可能促进量子计算错误纠正的发展。
...
### [37] [Resource-Optimal Importance Sampling for Randomized Quantum Algorithms](https://arxiv.org/abs/2603.13495)
*Davide Cugini,Touheed Anwar Atif,Yigit Subasi*

Main category: quant-ph

TL;DR: 该研究提出了一个基于经典重要性采样的通用框架，用于优化随机量子协议的计算资源需求，展示通过优化采样策略，减少所需资源的可行性和有效性。


<details>
  <summary>Details</summary>
Motivation: 量子计算中的随机化协议在模拟量子系统、抗噪声修复和测量任务中占据重要地位，而现有方法在实施中往往面临电路执行和测量等多方面的高成本。为此，该研究提出了一种基于经典重要性采样的通用框架，旨在通过优化采样策略来降低计算资源需求。

Method: 研究方法包括：1. 引入经典重要性采样的通用框架；2. 设计最小化成本函数的优化目标，确保计算开销和估计方差的平衡；3. 描述在量子计算中出现算法近似误差或物理噪声的情况下，该框架的扩展方法；4. 在存在误差检测方案时，分析采样策略的变化及其对资源需求的影响。

Result: 研究结果表明，该框架能够有效减少随机量子协议所需的计算资源，同时保持估计的无偏性，并在不同量子计算场景中展现出广泛的应用潜力。

Conclusion: 该研究为提高随机量子协议的效率提供了系统性的解决方案，并展示了其在量子计算实际应用中的价值。
...
### [38] [A Deep-Learning-Boosted Framework for Quantum Sensing with Nitrogen-Vacancy Centers in Diamond](https://arxiv.org/abs/2603.14728)
...
TL;DR: 提出了一种基于1D卷积神经网络的快速实时ODMR数据分析框架，显著提高了低信噪比下的分析能力。


<details>
  <summary>Details</summary>
Motivation: 量子传感器和成像在量子计算和材料科学中的重要性。

Method: 使用了一种基于1D卷积神经网络的机器学习方法进行快速实时分析。

Result: 相比传统的非线性拟合方法，在低信噪比下有显著的性能提升，适用于量子感知和成像。

Conclusion: 这种方法为量子传感和成像提供了强大的工具，提升了分析效率和可靠性。
...
### [39] [Perspective: Interactions and Nonlinearity in Non-Hermitian Physics](https://arxiv.org/abs/2603.13526)
*Federico Roccati,Federico Balducci*

Main category: quant-ph

TL;DR: 非厄米性物理在过去几十年从被否定到成为量子物理的重要研究领域，展示了其在开放量子系统、奇怪的能点、边沿效应等方面的独特现象，未来有望探索更高的复杂系统。


<details>
  <summary>Details</summary>
Motivation: 非厄米性在过去缺乏重视，但随着对开放量子系统和奇怪现象的深入研究，非厄米性展示了广阔的潜力，激发了该领域的迅速发展。

Method: 这篇综述首先阐述了非厄米性的起源，包括平均场近似、条件‘无点击’演化和精确的动力学描述。其次，探讨了非厄米性与相互作用的协同作用，发现了多体空间中的拓扑相变、边沿怀疑奇点等现象，并在耗散性量子混沌等领域发现了新的现象，最后总结了非厄米性在非线性系统中的集体行为及测量引起的量子相变。

Result: 文章展示了非厄米性物理的多样性和前沿性，特别是在开放量子系统、多体相互作用、量子混沌等领域的研究成果。

Conclusion: 非厄米性物理正成为量子物理的重要研究方向，未来研究应进一步探索更高复杂度的量子系统及其在材料科学、量子信息等领域的应用。
...
### [40] [Error semitransparent universal control of a bosonic logical qubit](https://arxiv.org/abs/2603.15356)
*Saswata Roy,Owen C. Wetherbee,Valla Fatemi*

Main category: quant-ph

TL;DR: 研究提出了一种基于动态编码子空间的框架，实现了对振荡器光损失的半透明错误门，显著减少了寄生误差，并展示了其在量子计算中的潜在应用。


<details>
  <summary>Details</summary>
Motivation: 在量子计算中，构建高效、可靠的量子逻辑位是关键挑战，振荡器光损失是影响量子记忆寿命的主要障碍，因此开发半透明错误门对提升系统性能至关重要。

Method: 通过动态编码子空间，研究设计了线性驱动策略，实现了{X, H, T}等方面的半透明错误门，并结合可检测辅助量子错误修正技术，实现了量子计算中所需的一系列操作。

Result: 实现的半透明错误门降低了寄生误差five-fold，延长了量子操作时间，成功构建了复合非Clifford操作序列，证明了该框架的有效性和可行性。

Conclusion: 该研究为量子位上更高效的错误弥补方法提供了新途径，有助于提升量子计算系统所需的冗余度，推动实际量子计算机的开发。
...
### [41] [Active Sampling Sample-based Quantum Diagonalization from Finite-Shot Measurements](https://arxiv.org/abs/2603.13536)
*Rinka Miura*

Main category: quant-ph

TL;DR: AS-SQD is a quantum algorithm that optimizes the selection of basis states for efficient ground-state energy estimation despite the limitations of near-term quantum devices.


<details>
  <summary>Details</summary>
Motivation: Improving the accuracy of ground-state energy estimation in noisy intermediate-term quantum (NITQ) devices is motivated by the need to leverage quantum advantages without extensive resources.

Method: AS-SQD employs active learning to iteratively select additional basis states, guided by a scoring function based on second-order perturbation theory, to minimize energy estimation errors.

Result: AS-SQD consistently achieves lower absolute energy errors than traditional SQD and random expansion methods, even in the presence of state preparation and measurement errors.

Conclusion: AS-SQD represents a significant advancement in resource-efficient quantum algorithms for energy estimation.
...
### [42] [Quantum Process Realization of LDPC Code Dualities and Product Constructions](https://arxiv.org/abs/2603.13538)
*Shuhan Zhang,Deepak Aryal,Yi-Zhuang You*

Main category: quant-ph

TL;DR: A general framework for quantum coding transformations is presented, incorporating Kramers-Wannier duality, tensor products, and check products as gauge transformations and coupled-layer constructions in the ZX-calculus diagrammatic language.


<details>
  <summary>Details</summary>
Motivation: The paper introduces a unified framework for understanding quantum coding transformations and their applications in quantum phase classifications.

Method: The authors use ZX-calculus to represent quantum processes diagrammatically and develop a systematic algorithm for extracting quantum circuits from code constructions.

Result: A formal correspondence is established between classical LDPC codes and operator algebras, enabling a systematic study of transformations between distinct quantum phases of matter.

Conclusion: This framework provides a novel approach to designing quantum error-correcting codes and mappings between quantum phases, with implications for topological quantum computing.
...
### [43] [Bit-Vector Abstractions to Formally Verify Quantum Error Detection & Entanglement](https://arxiv.org/abs/2603.13554)
*Arun Govindankutty*
...
### [44] [The Quest for Quantum Advantage in Combinatorial Optimization: End-to-end Benchmarking of Quantum Solvers vs. Multi-core Classical Solvers](https://arxiv.org/abs/2603.13607)
*Pranav Chandarana,Alejandro Gomez Cadavid,Enrique Solano,Thorsten Koch,Stefan Woerner,Narendra N. Hegade*

Main category: quant-ph

TL;DR: HSQC表现出色，在单个量子处理器上实现了与强大经典解决方案竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 评估当前量子硬件在组合优化中的潜力，尤其是在亚秒级端到端运行时间方面。

Method: 比较了HSQC、CPU-based和量子加速器在HUBO问题中的表现，采用了严格的时间计量方法。

Result: HSQC在1秒内提供了高质量的解，并与多个经典方法相比。互补方法和加速器在相同时间内未达到HSQC的水平。

Conclusion: HSQC展示了量子计算与经典计算在性能上的竞争，为跟踪进展提供了基准。
...
### [45] [Quantum contextuality with mixed states of 1D symmetry-protected topological order](https://arxiv.org/abs/2603.13626)
*Leroy Fagan,Akimasa Miyake*

Main category: quant-ph

TL;DR: 研究发现，量子计算通过许多体纠缠态的非局域性和上下文敏感性在噪声存在时仍能表现出量子优势。


<details>
  <summary>Details</summary>
Motivation: 探索 noisy quantum 计算如何维持量子优势，为量子计算在近端设备中的应用提供理论依据。

Method: 使用对称保护的拓扑态，通过混合态分析，结合最小纠缠典型热态算法。

Result: 在一定温度范围内，量子优势仍然存在，且该优势在有限大小情况下表现出较好的可扩展性。

Conclusion: 噪声条件下，基于对称保护拓扑态的许多体纠缠态仍能有效展示量子优势，为量子计算的任务执行提供可靠基准。
...
### [46] [Residual quantum correlations and non-Markovian noise](https://arxiv.org/abs/2603.13648)
...
### [47] [Quantum dial](https://arxiv.org/abs/2603.13653)
*Aashish Sah,Suman Kundu,Priyank Singh,Eemeli Forsbom,Vasilii Vadimov,Mikko Möttönen*

Main category: quant-ph

TL;DR: 提出了一种量子控域装置，能实时调节量子比特与辅助系统之间的耦合强度，减少环境影响，提升量子计算机性能。


<details>
  <summary>Details</summary>
Motivation: 量子计算依赖于孤立和控制量子比特，但仍面临技术挑战，需高效解决。

Method: 使用带阻滤波器调节耦合强度，能在纳秒时间内显著改变量子比特特性。

Result: 在重置配置中，量子比特退相干时间显著降低；在控制配置中，获得极高的门态 fidelity。

Conclusion: 该装置为量子计算机中的隔离与耦合控制提供了新方法，有助于降低噪声和错误，为未来量子处理器性能提升奠定基础。
...
### [48] [Thermodynamic Limits of Quantum Search](https://arxiv.org/abs/2603.13654)
*Ralf Riedinger*

Main category: quant-ph

TL;DR: A keyed symmetric cipher with sufficiently long keys can withstand quantum attacks, but precise key length requirements remain to be established.
 motivation: To address quantum computing threats to cryptography, establish minimum key lengths resistant to quantum attacks.
 method: Explore quantum thermodynamic limits of search algorithms, derive work-runtime trade-offs, and evaluate protocol optimality.
 result: A 831-bit secret key cannot be reconstructed deterministically in an expanding universe dominated by dark energy.
 conclusion: This work suggests tight bounds for post-quantum cryptography and quantum key distribution protocols, implying that sufficiently long symmetric keys provide security against quantum attacks.
...
Result: A 831-bit secret key cannot be reconstructed deterministically in an expanding, dark-energy-dominated universe until star formation is expected to cease.
...
### [49] [Optimal Distillation of Non-Markovianity: Bounds, Multi-Copy Gain, and the Weak-to-Essential Transition](https://arxiv.org/abs/2603.13712)
*Gabriel M. Arantes,Barbara Amaral,Nadja K. Bernardes*

Main category: quant-ph

TL;DR: The paper discusses a protocol to increase distinguishability of quantum states after passing through a specific quantum channel; an algorithm is developed to determine maximal distinguishability gain, applicable to any quantum channel. It shows a weakly non-Markovian channel can be converted into an essentially non-Markovian one via a distillation-like process and identifies conditions for optimal enhancement.


<details>
  <summary>Details</summary>
Motivation: Understanding and improving the distinguishability of quantum states is fundamental in quantum information theory, with applications in quantum communication and computing. This paper aims to develop a systematic method to enhance this distinguishability, particularly in the context of quantum channels.

Method: The authors introduce an optimization protocol to maximize distinguishability gain for any quantum channel. An algorithm is developed and demonstrated to identify optimal channel transformation. The approach applies a distillation-like process to convert weakly non-Markovian into essentially non-Markovian channels.

Result: The paper successfully shows that weakly non-Markovian channels can be enhanced via their distillation process, providing a framework for distinguishability recovery in open quantum systems. It also outlines optimal conditions for this enhancement.

Conclusion: This work offers a robust framework for evaluating and optimizing distinguishability recovery in open quantum systems, with potential applications in enhancing quantum communication protocols and quantum computing architectures.
...
### [50] [Closed-time-path approach to the optomechanical back-reaction problem](https://arxiv.org/abs/2603.13932)
*Salvatore Butera*

Main category: quant-ph

TL;DR: 研究了一个量子场通过辐射压力作用于移动镜的光omechanical系统，推导了移动镜的效用作用，并验证了能量平衡。


<details>
  <summary>Details</summary>
Motivation: 研究光omechanical系统中量子场与移动镜之间的相互作用，探索其能量转换机制，具有重要的量子和经典交叉领域价值。

Method: 使用了闭合时间路径(in-in)形式化的量子场论方法，推导了移动镜的动力学方程，计算了辐射的压力和粒子的产生，保持精确地描述了系统的动态。

Result: 发现移动镜的动力学受到来自场的反作用力的显著影响，该反作用力与动态Casimir效应直接相关，能谱结构揭示了物理机制的直接联系。

Conclusion: 建立了一个完整描述移动镜和量子场之间相互作用的框架，能够预测和解释相关的物理现象。
...
TL;DR: 这篇论文提出了一种基于有限路径的分支流形的时空轨迹统计模型。通过将熵与作用量进行比例关系，推导出一种有限的威克旋转路径积分，能够同时描述微观量子的干涉现象和宏观经典的决定论。模型推测波函数的坍缩可以解读为熵最大化的结果，为量子与经典描述的统一提供了一种有限熵结构的框架。


<details>
  <summary>Details</summary>
Motivation: 这篇论文探讨了时空轨迹的统计模型及其与经典和量子描述的统一，提出了将熵与作用量进行比例关系的假设，试图解释波函数的坍缩原因，为物理学基础研究提供了新的思路。

Method: 该论文采用了统计力学和量子力学的理论框架，引入了分支流形和熵的概念，通过路径积分的方法推导出有限的威克旋转路径积分，最后提出了波函数坍缩的熵最大化解释。

Result: 论文的推导得出了一种新的路径积分形式，这种形式能够合理解释微观和宏观的物理现象，为量子与经典描述的统一提供了理论框架，并推测波函数坍缩与熵最大化有关。

Conclusion: 该研究探索了熵与作用量的比例关系，提出了有限熵结构下的路径积分模型，并据此解释了波函数坍缩现象，为物理学基础研究提供了新的理论框架。虽然目前仍属于理论理想化，但为未来的研究提供了方向。
...
TL;DR: The paper explores how measurement noise impacts quantum computation, reveals its limitations in predicting computation accuracy, and proposes a new method to directly assess this impact.


<details>
  <summary>Details</summary>
Motivation: Improving quantum computer performance and reliability, especially in handling noisy resources.

Method: Mathematical derivation of fidelity relation, experimental setup for correlation function measurements.

Result: State fidelity is a less accurate measure for MBQC, new method directly correlates with computation fidelity.

Conclusion: Better understanding of noise effects improves MBQC characterization and benchmarking.
...
TL;DR: 在本研究中，我们开发了一种基于Ramsey干涉仪的金刚石量子磁敏器，采用了短距离传感器样品之间的方法。传统自旋态检测传感器基于连续波光密检测磁性共振和Ramsay方法，通常依赖于瓦特级数的激光器以获得高灵敏度，但会导致热问题。通过采用光捕获金刚石波导技术在经过电子束辐照的高温高压金刚石样品中，我们获得了高达9.5%的光转换效率，在100-400赫兹频率范围内实现了高灵敏度2.93±7 pT/Hz^1/2，并且在低激光功率210毫瓦下仅增加了约13开尔文的温度。我们使用干燥的模拟-info枚头阵列测量了一个微弱磁场为77.7±2 pT，在2.5毫米传感器距离下的测量，避免了信号衰减，提供了较高的信噪比。这项研究对于基于Ramsey干涉仪的实用生物磁应用具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 随着生物磁监测技术的发展，如何提高传感器的灵敏度和减少温度升高是一个关键问题。本研究通过优化传感器设计和材料技术，提出了一种高效且稳定的金刚石磁敏器，为实际应用提供了可行的解决方案。

Method: 本研究采用了光捕获金刚石波导技术，结合了Ramsey干涉仪的原理，并在经过辐照处理的高压高温金刚石样品中实现了高光转换效率和良好的磁敏性能。

Result: 实验结果表明，该磁敏器在100-400赫兹频率范围内具有高灵敏度2.93±7 pT/Hz^1/2，并且在低激光功率下实现了仅有约13开尔文温升的性能。

Conclusion: 本研究成功开发了一种高性能的金刚石量子磁敏器，显著提高了灵敏度并减少了温升，为生物磁监测的实际应用奠定了基础。
...
### [54] [Mixed-State Entanglement in a Minimal Model of Quantum Chaos](https://arxiv.org/abs/2603.14292)
*Tanay Pathak*

Main category: quant-ph

TL;DR: 文章探索了量子纠缠在许多体系统中的动力学行为，通过结合复制品技巧和时空对偶性，在kickeded场伊辛模型中精确计算了部分转置简并密度矩阵的谱。结果显示，在早期时刻，纠缠负性、奇熵和Rényi互相对信息之间存在精确的关系。数值结果表明，对于相等的三部分分割，在早期时刻，所有纠缠度量都会趋于Haar随机近似值。与非平衡量子物理中中心的挑战问题相关。对于不均衡的三部分分割，Rényi互相对信息和负性在网络后期趋于零，意味着相应的简并密度矩阵可以分解。通过大量数值模拟，作者发现该关系对于初始状态也相当定量地成立，从而推测对于所有初态和所有时间该关系均成立。


<details>
  <summary>Details</summary>
Motivation: 非平衡量子物理中 Understanding many-body system的量子关联动力学是一个中心问题。这项研究旨在通过kicked filed Ising模型，使用复制品技巧和时空对偶性，精确计算部分转置简并密度矩阵的谱，从而揭示纠缠连带性在早期和网络后期的行为。

Method: 在kickeded场伊辛模型中，结合复制品技巧和时空对偶性，精确计算部分转置简并密度矩阵的谱。通过数值模拟验证了所有纠缠度量在相等三部分分割和网络后期的行为，进一步推测其对所有初态和所有时间均有效。

Result: 通过计算部分转置简并密度矩阵的谱，精确确定了在早期时刻，纠缠负性、奇熵和Rényi互相对信息之间存在精确的关系。数值模拟表明，对于相等的三部分分割，所有纠缠度量在后期趋于Haar随机近似值。对于不均衡的三部分分割，Rényi互相对信息和负性在后期趋于零。通过大量数值模拟，作者发现该关系对于初始状态也相当定量地成立，从而推测该关系对所有初态和所有时刻均成立。

Conclusion: 这项研究在kickeded场伊辛模型中首次通过精确计算部分转置简并密度矩阵的谱，深入探讨了量子关联在许多体系统中的动力学行为。结果表明，在早期时刻，纠缠度量之间存在明确的关系，而在后期时刻，这些度量趋于Haar随机分布或完全分解。这些发现为理解非平衡量子物理中许多体系统的量子关联动力学提供了新的见解，并为实验研究提供了理论支持。
...
TL;DR: The paper examines the Magnus expansion for a two-level system driven by a single axis, revealing that a third-order approximation closely matches exact results. It also shows that the second-order Magnus approximation in the adiabatic picture is highly accurate for the semiclassical Rabi model across parameter ranges.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to understand and simplify the Magnus expansion for two-level systems, particularly in the context of non-adiabatic transitions and Stokes phases.

Method: The authors employ Lie algebra (	extit{	extg}-algebra) techniques and Floquet theory to derive the Magnus expansion, focusing on commutator-free forms and their applications.

Result: The study demonstrates that the Magnus expansion yields highly accurate results when truncated at the third order for the Landau-Zener-Stückelberg-Majorana model and even better results at the second order for the semiclassical Rabi model.

Conclusion: This research provides valuable insights into using the Magnus expansion for diverse two-level systems, highlighting the effectiveness of different truncation orders based on the system under study.
...
### [56] [Quantum electrometry in a silicon carbide power device](https://arxiv.org/abs/2603.13836)
...
TL;DR: 该研究提出了一种基于硅碳onyte（Vsi）的新方法来实现高空间分辨率的高电场检测，这对SiC功率器件的可靠性至关重要。


<details>
  <summary>Details</summary>
Motivation: 随着硅碳onyte在高有 bias运算器件中的广泛应用，如电力转换和高频开关，对检测和管理这些器件中的高电场具有重要意义。高电场可能导致器件失效，因此需要一种高效、高分辨率的方法来监测和分析电场分布。

Method: 研究利用硅碳onyte中的Vsi位置敏感特征，能够感知平行和垂直于晶体轴的电场分量，这种方法能够检测高达2.3 MV/cm的电场，接近于4H-silicon carbide的-breakdown电场。此外，通过选择性刻蚀方法，实现了对电场分布的高分辨率成像。

Result: 研究结果表明，利用Vsi检测器可以准确检测高达2.3 MV/cm的电场，并且可以在实际应用中接近silicon carbide的-breakdown电场。此外，通过选择性刻蚀形成Vsi，还实现了对电场分布的高分辨率成像。这种方法不仅能够实时捕捉电场变化，还为硅碳onyte在高性能电力电子器件中的应用提供了理论支持和实验验证。

Conclusion: 该研究为硅碳onyte功率器件的可靠性提供了创新的解决方案。通过位置敏感的Vsi检测器及其高分辨率成像技术，可以有效监控高电场分布，为这些器件的优化设计和故障诊断提供了可行的方法。未来的研究可能进一步优化检测器的性能，并将其应用于更复杂的电力电子设备中。
...
### [57] [Millimeter Wave Readout of a Superconducting Qubit](https://arxiv.org/abs/2603.13837)
*Akash V. Dixit,Zachary L. Parrott,Dennis Chunikhin,Bradley Hauer,Trevyn F. Q. Larson,John D. Teufel*

Main category: quant-ph

TL;DR: 该论文通过在毫米波腔和超导体电容电感器之间引入大 detuning，成功实现了对超导体电容电感器的高保真度读出。


<details>
  <summary>Details</summary>
Motivation: 毫米波技术在量子平台之间的通信和增强具有重要应用价值，该论文通过引入大 detuning 实现了精确的 qubit 读出。

Method: 通过使用电路量子电动力学系统，研究了毫米波腔和超导体电容电感器之间的相互作用，选择了适当的参数以获得高保真度的 qubit 读出。

Result: 通过这种方法，能够在没有量子受限放大器的情况下，实现 qubit 读出的高保真度，并且在 1,000 激光光子后仍能保持良好的状态转移抑制能力。

Conclusion: 该研究展示了通过调整毫米波腔和超导体电容电感器的频率，可以有效改善 qubit 读出的性能，为量子计算和通信提供了新的技术路线。
...
### [58] [Evidence of Uncollapsed Quantum Amplitudes After Consecutive Measurements](https://arxiv.org/abs/2603.13974)
*Christoph Adami,Lambert Giner,Jeff S. Lundeen,Raphael A. Abrahao*

Main category: quant-ph

TL;DR: The paper discusses the disagreement between the standard collapse and unitary interpretations of quantum measurement, particularly when multiple consecutive measurements are performed. It demonstrates through experiments that these interpretations make different predictions and supports the unitary theory by analyzing the joint density matrix using single-photon measurements and decoherence analysis.


<details>
  <summary>Details</summary>
Motivation: Understanding the behavior of quantum measurements and their interpretations is fundamental to quantum mechanics. This paper aims to resolve discrepancies in the predictions of two interpretations by performing experimental tests with consecutive measurements.

Method: The research involved performing single-photon measurements on a tunable quantum state and analyzing the joint density matrix of three consecutive measurements. They compared the results under different decoherence conditions to test the predictions of the standard collapse and unitary theories.

Result: The study found that when three consecutive measurements are made on the same quantum system, the standard collapse theory and the unitary theory make different predictions. The experimental results aligned more closely with the unitary theory, which accounts for the persistence of unrealized amplitudes without a separate measuring apparatus memory. When decoherence was introduced, the results were consistent with the collapse theory predictions.

Conclusion: The paper contributes to the understanding of quantum measurement dynamics, particularly in scenarios involving multiple consecutive measurements. It provides empirical evidence supporting the unitary theory in specific contexts and highlights the challenges in distinguishing the two interpretations experimentally when multiple measurements are involved.
...
### [59] [Readout-induced degradation of transmon lifetimes: interplay of TLSs and qubit spectral reshaping](https://arxiv.org/abs/2603.13857)
...
TL;DR: Measurement backaction degrades dispersive readout of superconducting qubits even at modest drive strengths, often via the reduction of qubit lifetimes during readout.


<details>
  <summary>Details</summary>
Motivation: This work addresses the challenge of measurement backaction in superconducting qubits, which is crucial for optimizing readout protocols in quantum processors.

Method: The study combines theoretical analysis using time-dependent perturbation theory and master equation simulations with experimental measurements on a frequency-tunable transmon qubit.

Result: The researchers find that driving at the resonator frequency associated with the qubit ground state yields the narrowest qubit emission spectrum and the least degradation of qubit lifetimes for a fixed measurement rate.

Conclusion: This research provides a quantitative understanding of the interplay between qubit emission, readout drive, and measurement backaction, offering practical guidelines for optimizing readout protocols in future quantum processors.
...
### [60] [Engineering walk-off-induced orbital angular momentum spectrum in spontaneous parametric downconversion](https://arxiv.org/abs/2603.14742)
...
TL;DR: 研究展示了 ghost 哈密顿与Pais-Uhlenbeck模型的Bohmian动力学性质及其量子不确定性。


<details>
  <summary>Details</summary>
Motivation: 探索 ghost 哈密顿在Bohmian视角下的动力学行为和量子效应。

Method: 使用二维Bohmian理论，分析了高阶导数系统的波包演化和量子势。

Result: 发现在不同Hamiltonian下Bohmian轨迹和量子势不同，表明经典等效性不延伸到Bohmian动力学。

Conclusion: 高阶导数系统的Bohmian行为表现出量子不明确性，经典对量子动力学的影响存在限制。
...
### [62] [Benchmarking quantum simulation with neutron-scattering experiments](https://arxiv.org/abs/2603.15608)
...
TL;DR: 成功利用50量子比特超导处理器生成与实验数据精度匹配的定量对比，并为量子材料的计算机仿真奠定了框架。


<details>
  <summary>Details</summary>
Motivation: 探索在小规模量子处理器下进行可靠量子材料仿真的方法；对量子计算在材料科学中的潜在应用进行深入研究。

Method: 开发了量子经典结合 workflow来计算动力学结构因子(DSFs)，并使用多种指标比较了实验测值和仿真结果。

Result: 实现了对KCuF3Canonical Luttinger液体系及1D XXZ Heisenberg模型的量子仿真，展示了在小规模量子处理器下进行复杂量子材料研究的可行性。

Conclusion: 为小规模量子处理器在量子材料仿真中的应用提供了理论依据和实际案例，展示了量子计算在解决复杂量子体系问题中的潜力。
...
### [63] [Kirkwood-Dirac classical states based on discrete Fourier transform: Representation with directed graph](https://arxiv.org/abs/2603.13863)
*Lin-Yan Cai,Ying-Hui Yang,Zhu-Jun Zheng*

Main category: quant-ph

TL;DR: 这项研究探讨了在离散傅里叶变换矩阵转换下KD-经典态集合的结构特征。作者通过构造有向图，提供了KD-经典纯态的表征方法。同时，他们证明了KD-经典态集合在p^r维空间内可以表示为KD-经典纯态的凸包。该结论简洁明了，有效地解释了KD-经典态与量子优势的关系。


<details>
  <summary>Details</summary>
Motivation: 量子态的KD-经典性在量子 metrology、量子 chaos等前沿领域具有重要意义。深入研究KD-经典态的结构特征不仅有助于理解量子态的性质，也为开发新的量子技术提供了理论依据。

Method: 该论文采用了基于有向图的分析方法，系统地描述了KD-经典纯态的结构，并通过凸包理论明确KD-经典态集合的组成。

Result: 研究表明，离散傅里叶变换矩阵转换下的KD-经典态集合在p^r维空间内可以表示为KD-经典纯态的凸包。该结果不仅简化了KD-经典态的研究，还拓展了现有相关理论的适用范围。

Conclusion: 这项研究通过创新的方法，成功揭示了KD-经典态的结构特性，为量子态分类和量子技术发展提供了重要理论支持。
...
### [64] [Investigating Lipkin-Meshkov-Glick Model and Criticality-Enhanced Metrology in a Coherent Ising Machine](https://arxiv.org/abs/2603.13882)
*Shuang-Quan Ma,Jing-Yi-Ran Jin,Chen-Rui Fan,Chuan Wang,Qing Ai*

Main category: quant-ph

TL;DR: This paper presents a coherent Ising machine (CIM) using degenerate optical parametric oscillators (DOPO) to simulate the LMG model, demonstrating effective control of quantum phase transitions and critical phenomena for quantum sensing and metrology.


<details>
  <summary>Details</summary>
Motivation: Quantum criticality in many-body systems is crucial for quantum sensing applications, but practical implementation is challenging. This paper addresses these challenges by developing a CIM to simulate the LMG model.

Method: The authors map spin variables of the LMG model onto the phases of DOPO pulses and implement all-to-all spin-spin interactions through coupling among them.

Result: The CIM successfully captures the second-order QPT and reconstructs the complete phase diagram under ferromagnetic coupling. It also enables quantum-enhanced metrology with diverging measurement precision near the critical point.

Conclusion: This work demonstrates the potential of CIM as a flexible platform for studying QPT in quantum magnetic models, offering new insights into quantum simulation and critical phenomena.
...
### [65] [Thermodynamics of a biophotomimetic nonreciprocal quantum battery](https://arxiv.org/abs/2603.15268)
...
### [66] [Generalized Inverses of Quantum Channels: a categorical perspective](https://arxiv.org/abs/2603.13946)
...
### [67] [Simulating the Open System Dynamics of Multiple Exchange-Only Qubits using Subspace Monte Carlo](https://arxiv.org/abs/2603.15577)
*Tameem Albash,N. Tobias Jacobson*

Main category: quant-ph

TL;DR: 提出了一种基于蒙特卡罗方法模拟多个交换型量子比特（EO qubits）开放系统动力学的方法。在EO编码中，三个基团的总自旋投影量子数沿z轴保持不变，而与开放系统或 多重失准设定相反。在我们的方法中，在每次逻辑量子操作后测量每个EO qubit沿z轴的总自旋投影量子数，这种测量会将不同量子数状态下 的相干混合物 decohere。独立模拟会给出系统在相应子空间中的不同轨迹，因此我们将此方法称为子空间蒙特卡罗方法。每个EO qubit都有一个确定的自旋投影量子数，因此n个 qubit的密度矩阵可以用一个大小为3^{2n} 的向量表示，再加上一个大小为n的向量来标记每个 qubit 的量子数。我们表明，在模拟电路绕混噪声时，这种近似仍能忠实地反映真实动力学，无需任何额外的绕混处理即可介导相干错误为随机错误。我们使用这种模拟方法研究了带有重置如果泄漏机制的电路（例如使用6个EO qubits 的多轮Bell态稳定 化电路）的测量结果相关性，受CNOT实现方式的影响。


<details>
  <summary>Details</summary>
Motivation: 开发高效的方法模拟量子比特动力学对于量子计算的重要性和原创性来说是至关重要的。特别是在开放系统中，节点之间互相干扰严重，所以如何准确地模拟这些复杂系统的行为对研究者来说具有挑战性。

Method: 蒙特卡罗方法与子空间分解相结合。在每个时间步长后，独立地对每个EO qubit的沿z轴的总自旋投影量子数进行测量，将每个 Time Evolution 的状态限制在特定的子空间中，从而降低了整个系统的复杂度。特别是引入了辅助向量来标记每个 qubit 的量子数，从而最大限度地简化了计算和存储需求。

Result: 提出的方法有效缩小了模拟空间的维度，这在高阶量子系统中尤其重要。尤其是当量子系统规模扩大时，传统方法由于维度指数爆炸而导致无法进行有效计算。通过子空间蒙特卡罗方法，我们能够以线性增长的复杂度来处理这些复杂度高的问题。

Conclusion: 提供了一种新的策略，能够有效地模拟复杂的多量子比特开放系统，这对量子计算的理论研究和实际应用开发具有重要意义。
...
### [68] [Collective Nuclear Polaritons with Coherent and Tunable Excitation Dynamics](https://arxiv.org/abs/2603.13948)
...
### [69] [Folding-Free Zero-Noise Extrapolation by Layout-induced Noise Diversity](https://arxiv.org/abs/2603.13949)
*Debarthi Pal,Yogesh Simmhan*

Main category: quant-ph

TL;DR: FF-ZNE is a method for error mitigation in quantum computing that removes the need for selecting noise scaling factors and extrapolation models, making the process more user-friendly and effective for a broader range of circuits.


<details>
  <summary>Details</summary>
Motivation: Quantum processors operate in a noisy environment, and error mitigation techniques are essential to recover accurate results without full fault tolerance. ZNE is a widely used method, but it lacks rigorous error bounds and requires expert tuning, making it costly and impractical for many users. FF-ZNE addresses these issues by eliminating the need for noise scaling factor selection and provides a scalable solution for a wide range of quantum circuits.

Method: FF-ZNE achieves effective noise amplification without circuit folding by executing a fixed circuit across quantum processors with distinct native noise profiles. This allows for controllable variations in effective noise strength. Under a depolarizing noise model, the extrapolation process results in a fixed linear form, removing the need for extrapolator choice and simplifying the mitigation procedure. Two algorithms are proposed to identify sets of isomorphic hardware layouts that enable reliable zero noise extrapolation.

Result: Experiments on a 133-qubit IBM Quantum device show that FF-ZNE yields mitigated expectation values with average deviations of approximately 6% for 50-qubit EfficientSU2 (sparse) circuits and 4.5% for Hamiltonian simulation (dense) circuits. This demonstrates that FF-ZNE is scalable and effective across different circuit types.

Conclusion: By removing the need for expert tuning and providing a consistent and reliable method for noise extrapolation, FF-ZNE enhances the practicality and accessibility of error mitigation techniques for current quantum hardware.
...
### [70] [Revisited Quantification of the Resource Theory of Imaginarity](https://arxiv.org/abs/2603.13980)
*Yue Han,Naihong Hu*
...
### [71] [Dynamical Simulations of Schrödinger's Equation via Rank-Adaptive Tensor Decompositions](https://arxiv.org/abs/2603.13990)
*N. Anders Petersson,Chase Hodges-Heilmann,Stefanie Günther*

Main category: quant-ph

TL;DR: 这篇论文讨论如何使用张量分解方法来简化对大型量子计算设备运行的模拟问题。量子计算设备的量子状态随qubit数量增加呈指数增长，传统模拟方法变得不可行，因此作者提出利用张量分解来解决这一问题。他们特别关注了THP方法、TDVP、TDVP-2和BUG算法，并通过几种不同的哈密顿模型展示了这些方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的发展，对量子状态的模拟在理论上和实践中都非常重要。然而，随着qubit数量的增加，计算资源的需求也急剧增加，传统的模拟方法难以应对。因此，寻找高效的方法来简化量子状态表示尤为重要。

Method: 论文利用了张量分解方法，包括 Tucker 格式和 Tucker 格式的张量分解法，特别关注了时间依赖的 Schrödinger 方程。论文还分析了 TDVP、TDVP-2 和 BUG 时间积分算法的性能。通过这些方法，作者能够有效地跟踪和计算量子系统的动态行为和时间依赖性。

Result: 论文的结果表明，张量分解方法在计算量子系统时空依赖时表现出色。这样的方法可以有效地量化计算所需资源与所需精度的关系，特别是在处理由许多子系统组成的复杂量子系统时，资源消耗与系统规模呈线性增长，而与子系统数呈多项式增长。

Conclusion: 通过张量分解和时间积分算法，可以有效减少模拟大型量子计算设备所需计算资源，并随着所需的精度提高和子系统数量的增加，计算复杂度呈线性增长。这些方法适用于时间依赖的量子运动建模，为量子计算设备的研究提供了新的工具和途径。
...
### [72] [Quantum Dynamical Entropy and non-Markovianity: a collisional model perspective](https://arxiv.org/abs/2603.14015)
*Giovanni Nichele,Fabio Benatti*

Main category: quant-ph

TL;DR: 研究开放量子动力学中的非马尔科夫现象需要分析系统与环境的统计特性。通过多时间相关函数可以构建过程中的关联，用于计算ALF熵，反映系统动力学熵生产。在碰撞模型中可以通过环境参数显式计算ALF熵，与记忆效应的激活和超激活相关。


<details>
  <summary>Details</summary>
Motivation: 理解开放量子动力学中的非马尔科夫现象对量子信息科学有重要意义。

Method: 借助ALF熵和多时间相关函数，结合碰撞模型进行研究。

Result: ALF熵在碰撞模型中可显式计算，且与环境参数相关联，揭示记忆效应激活机制。

Conclusion: 这一研究为分析开放量子力学提供了新的工具和见解。
...
### [73] [Quantum Dynamical Entropy and Dissipative Information Flows](https://arxiv.org/abs/2603.14025)
*Giovanni Nichele,Fabio Benatti*

Main category: quant-ph

TL;DR: 文章提出了一种扩展的ALF熵，用于量化开放量子动力学中环境对量子系统的信息回流。


<details>
  <summary>Details</summary>
Motivation: 量化环境对量子系统的信息回流具有重要意义。

Method: 通过扩展ALF熵，提供了关于量子系统信息回流的度量方法。

Result: 推导得到了开放量子动力学中的环境相关性的明确表达式，特别是在单一量子比特与经典无序链碰撞耦合的情况下。

Conclusion: 该研究结果为理解开放量子动力学中的信息流动提供了新的视角，特别是在涉及环境相关性时。
...
### [74] [Block-QAOA-Aware Detection with Parameter Transfer for Large-Scale MIMO](https://arxiv.org/abs/2603.14042)
*Shuai Zeng*

Main category: quant-ph

TL;DR: This paper proposes a Block-QAOA-Aware MIMO Detector (BQA-MD) to address large-scale MIMO detection challenges.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of scaling up MIMO detection using quantum computing, particularly QAOA.

Method: The BQA-MD combines block-QAOA-aware preprocessing, a 5G NR Gray-HUBO interface, a dynamic regularized blockwise objective, and K-best candidate propagation with fixed-size block construction for parameter-transfer QAOA.

Result: The detector improves performance with regularization and achieves near-exhaustive performance, outperforming direct training QAOA in BER.

Conclusion: The BQA-MD effectively utilizes quantum resources with limited qubits and achieves superior MIMO detection performance compared to traditional QAOA approaches in the tested SNR regimes.
...
### [75] [Adaptive quantum metrology with large dynamic range using short one-axis twists](https://arxiv.org/abs/2603.14095)
*Tyler G. Thurtell,Akimasa Miyake*

Main category: quant-ph

TL;DR: 这篇论文提出了一种适用于大相位值的量子相位估计方法，实验上可行，利用一维旋转和一轴扭转操作，适用于玻色爱因斯坦凝聚体，展示了其在量子 Metrology中的应用。


<details>
  <summary>Details</summary>
Motivation: 量子 Metrology在 friesen atomic钟等设备中有重要应用，精确的相位估计是关键。

Method: 提出了一种基于交替应用一维旋转和一轴扭转操作的量子相位估计方法，实验上可实现。

Result: 实验结果展示了该方法在量子 Metrology中的高精度，尤其是在系统大小和非高斯参数下的表现。

Conclusion: 该方法提供了适用于大相位值的高精度量子相位估计方案，实验可行性高，适用于未来量子设备开发。
...
### [76] [Analysis of Hydrogen Contamination in Al/AlOx/Al Josephson Junctions](https://arxiv.org/abs/2603.14113)
*Yu Zhu,Aldilene Saraiva-Souza,Félix Beaudoin,Hong Guo*

Main category: quant-ph

TL;DR: 研究结果表明，氢杂质在超导量子比特中的铝氧化物层中表现出特定的结构特性，并通过分子动力学模拟和量子输运计算，成功量化了氢对约瑟芬结能的影响，得出一个准确的能值估计。


<details>
  <summary>Details</summary>
Motivation: 超导量子比特是量子信息处理的重要平台，而氢杂质作为电导率和能值变化的重要来源，其研究对于提高量子比特的可靠性和性能优化至关重要。

Method: 本研究通过分子动力学模拟和量子输运计算相结合的方法，首先使用CHGNet模拟铝表面在氧化剂环境下的氢 incorporation，获取了铝氧化物层的结构特性。接着，利用 atomistic quantum transport calculations 针对铝氧化物/铝结界体系构建模型，通过纳西夫-泛德公式计算了其能值特性。其中，GGA+U泛函用于调校能带隙和能对齐。

Result: 主要结果包括：1. 铝氧化物铝氧化物铝氧化物铝氧化物层的铝羟基和铝-铝羟基组成比例与氢含量呈现一致。 2. 氢原子在铝氧化物表面主要集中在表面区域，并以特定的结构形式存在。 3. 氢存在显著影响结能，能值偏移与p型掺杂一致，氢含量达到2.56%时，能值约为10.92 GHz。

Conclusion: 该研究表明，分子动力学和量子输运计算能够提供氢杂质在超导量子比特中的全面影响分析，对提升设备可靠性和性能优化具有重要意义。
...
### [77] [Practical Limits to Single-Mode Vacuum Squeezing in a SNAIL Parametric Amplifier](https://arxiv.org/abs/2603.14123)
*Theodore Shaw,Debsuvra Mukhopadhyay,Zhuoqun Hao,Josiah Cochran,Haley Cole,Archana Kamal,Shyam Shankar*

Main category: quant-ph

TL;DR: In practical single-mode vacuum squeezing using SPAs, loss dominates squeezing performance rather than Kerr nonlinearity.


<details>
  <summary>Details</summary>
Motivation: The study aimed to explore practical limitations of SPAs in generating squeezing, particularly focusing on the interplay between Kerr nonlinearity and loss.

Method: The research involved varying external flux and pump power to minimize Kerr-induced distortion and assess squeezing performance under different operating points.

Result: For fixed squeezing frequencies, squeezing performance was unexpectedly robust to varying Kerr nonlinearity, with internal losses (resonator and chain) primarily affecting squeezing.

Conclusion: SPAs' practical limitations in squeezing are more significantly influenced by loss than by Kerr effects, suggesting efforts to reduce loss should be prioritized for improved performance.
...
### [78] [NVRNet: Deep Learning Model for Fast Nitrogen Vacancy Characterization under Room Temperature](https://arxiv.org/abs/2603.14144)
...
### [79] [Frequency-resolved N-photon correlations in the ultra-strong coupling regime](https://arxiv.org/abs/2603.14201)
*Wen Huang,Qian Bin,Ying Wu,Xin-You Lü*

Main category: quant-ph

TL;DR: 研究了超强耦合腔量子电动力学系统中频率分resolved多光子相关性，并展示了对称性破坏如何增强相关光子对和三重光的生成。


<details>
  <summary>Details</summary>
Motivation: 通过研究超强耦合系统中光-物质的相互作用，可能揭示新的光子统计特性，为量子信息编码和高分辨率光谱学提供新工具。

Method: 使用了量子电动力学和频率选择性检测，结合数值模拟和实验观察，分析了系统的频率resolved多光子相关性。

Result: 发现了频率resolved多光子反聚集和聚集的特性，特别是在偶极反射相互作用下，光子间的关联系数显著增加，具有多频率相关光子对和三重光的 generatehung性。

Conclusion: 超强耦合系统中光子间的频率resolved多光子相关性表现出对称性破坏的特征，这为研究光-物质相互作用系统中的对称性提供了新的视角。
...
### [80] [Multi-qubit controlled gate with optimal T-count](https://arxiv.org/abs/2603.14202)
*Soichiro Yamazaki,Seiseki Akibue*

Main category: quant-ph

TL;DR: 该论文提出了一个高效的T门数量合成方法，适用于大多数多比特受控SU(2)操作。该方法改进了Gosset等人的先验工作，通过引入小误差允许的条件，降低了所需的T门数量到$3	ext{log}_2(1/	ext{ε}) + o(	ext{log}(1/	ext{ε}))$。此外，当禁止使用近似受控门时，该方法达到了理论下限。这一成果为量子算法中的受控操作合成提供了重要进展，尤其是在SU(4)操作的高效合成方面。


<details>
  <summary>Details</summary>
Motivation: 量子计算领域中，受控操作是许多量子算法的核心成分。如何在允许一定程度误差的情况下高效合成受控操作，是提高量子算法性能的重要问题。这篇论文为这一问题提供了新的解决方案，具有重要的理论和实践意义。

Method: 该论文采用了一些新的数学推导和组合技巧，特别是在概率门路技术和低深度门组合方面，提出了新的上界估计。具体方法包括：引入误差扩展概念，优化门路设计，分析T门的数量和效率。

Result: 论文的主要贡献包括：首次在允许小误差的情况下，给出了多比特受控SU(2)操作的高效T门合成方法。该方法不仅在复杂度上有所改进，而且在应用方面，能够用于一般的受控操作合成和高效SU(4)合成。此外，该结果还达到了下界，证明其在方法上的最优性。

Conclusion: 这篇论文在受控操作的T门数量合成方面取得了显著进展。通过引入新的方法和技术，该论文不仅扩展了我们对受控操作合成的理解，还为实际应用中的高效量子算法设计提供了理论支持。未来的工作中，可以进一步探索这些方法在更多量子算法中的应用，以及如何进一步优化T门的数量和性能。
...
### [81] [On aggregation-quantization permutability problem for discrete-time Markov chains](https://arxiv.org/abs/2603.14269)
*Adam Doliwa,Artur Siemaszko,Adam Zalewski*

Main category: quant-ph

TL;DR: 研究探讨了随机游走构成的离散时间量子游走与马尔可夫链分组过程，提出了将聚合技术应用于量子马尔可夫链的方法，并在等价条件下分析了这些操作的结果。特别地，针对具有均衡划分的图论随机游走，展示了条件满足的例子，并探讨了多个实例，如Platonic固体的经典与量子游走，以及N维超立方体与埃伦费斯特瓮模型的关系。此外，研究了自由群Cayley图上离散时间游走与Cantero-Moral-Velazquez方法的一致性。


<details>
  <summary>Details</summary>
Motivation: 量子游走作为一种 populate study的工具，在信息处理和量子计算领域具有广泛的应用。通过研究如何结合聚合过程和量子游走，可以更深入地理解量子系统的动力学行为，同时为开发高效量子算法提供理论基础。

Method: 使用Szegedy的量化方法与马尔科夫链的聚合过程相结合，研究了其在量子马尔科夫链中的应用。通过构造数学模型和实例分析，验证了条件在不同场景下的适用性。

Result: 在等价条件下，Szegedy量化与聚合操作的结果一致。特别地，在具有等价划分的图上随机游走的情况下，这种情况成立。同时，展示了多个实例，如Platonic固体和超立方体上的经典与量子游走，进一步验证了这一结论。此外，探讨了N维超立方体与埃伦费斯特瓮模型的关系，并比较了自由群Cayley图上游走与*Cantor uniformization*方法的一致性。

Conclusion: 提出了一种结合Szegedy量化和聚合过程的方法，用于分析离散时间的量子马尔可夫链。该方法在等价条件下有效地应用于各类案例，包括经典与量子游走，以及与经典随机游走模型的关系。研究结果为量子计算中的随机游走建模提供了新的视角和方法。
...
### [82] [Phase-preserving control of Floquet-engineered cavity quantum electrodynamics](https://arxiv.org/abs/2603.14279)
...
### [83] [Non-Resonant Boundary Time Crystals from Quantum Synchronization Breakdown](https://arxiv.org/abs/2603.14311)
...
### [84] [How to find expressible and trainable parameterized quantum circuits?](https://arxiv.org/abs/2603.14451)
*Peter Röseler,Dennis Willsch,Kristel Michielsen*

Main category: quant-ph

TL;DR: The article explores whether parameterized quantum circuits (PQCs) can be both trainable and expressive, providing insights and guarantees for designing such circuits using advanced statistical methods.


<details>
  <summary>Details</summary>
Motivation: Understanding the balance between trainability and expressivity in PQCs is crucial for advancing quantum machine learning and algorithms, particularly for practical applications on existing quantum hardware.

Method: The authors derive a finite-sample, dimension-independent concentration bound for the variance of PQCs to establish trainability guarantees and perform a systematic analysis across different ansätze, showing an anticorrelation between trainability and expressibility.

Result: The study reveals that improved effective dimension can be achieved using quantum neural network ansätze with fewer parameters, and for Variational Quantum Eigensolvers (VQE) on H₂, the approach achieves UCCSD-like accuracy with reduced circuit complexity.

Conclusion: The paper contributes to addressing a critical open question in quantum computing, offering new insights and methodologies for constructing hybrid quantum-classical architectures with practical implications for quantum algorithm development.
...
### [85] [Quantum-limited traveling-wave parametric amplifier based on DUV lithography-defined planar structures](https://arxiv.org/abs/2603.14455)
*Hao Li,Marco Scigliuzzo,Evgenii Guzovskii,Seog-Tae Han,Kyungho Han,Tobias J. Kippenberg*

Main category: quant-ph

TL;DR: 研究利用微极紫外光刻技术实现了高性能、高损失补偿的室温级表面波放大器，为大规模量子系统硬件提供了重要支撑。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的发展，大规模量子系统需要高性能的微波组件，而微波系统的开发需要材料、工艺和设计方法的支持。

Method: 采用微极紫外光刻技术在传统微波电路基础上开发新型表面波放大器，将 disappointment 表面波放大器和 Josephson 结结相结合，实现了高增益、广 bandwidth 和量子受限噪声性能。

Result: 制备的表面波放大器具有 3-11 GHz 的宽 bandwidth 和 -102 dBm 的 1 dB 压缩点，同时噪声接近量子限制，操作有效。

Conclusion: 该研究展示了微极紫外光刻技术在大规模量子系统硬件中的重要应用价值。
...
### [86] [The Twin-World road to reality in quantum mechanics](https://arxiv.org/abs/2603.14464)
*Daniel Braun*

Main category: quant-ph

TL;DR:  paper introduces a novel stochastic quantum mechanics based on twin worlds reproducing standard quantum mechanics effects. 


<details>
  <summary>Details</summary>
Motivation:  author presents a new theory extending grabit formalism to twin worlds to explain quantum mechanics in a stochastic framework. The aim is to provide a deeper understanding of quantum phenomena. 

Method:  author develops twin world framework with identical stochastic evolution laws, focusing on coincidence events in the intersection. The approach validates Schrödinger's equation and reproduces quantum effects such as tunneling. 

Result:  the stochastic approach successfully reproduces standard quantum mechanics, including Born's rule and Bell's inequality violations. The method handles arbitrary particles and interactions, and accurately models quantum effects. 

Conclusion:  author presents a coherent stochastic theory explaining quantum mechanics within a twin world framework, demonstrating its validity and applicability. 
...
### [87] [Quantum Enhanced Pauli Propagation](https://arxiv.org/abs/2603.14485)
...
### [88] [Parrondo-type enhancement of quantum-state transfer in spin chains](https://arxiv.org/abs/2603.14537)
...
### [89] [Reversible Lifetime Semantics for Quantum Programs](https://arxiv.org/abs/2603.14538)
*Simone Faro,Francesco Pio Marino,Gabriele Messina*

Main category: quant-ph

TL;DR: 该论文通过语义界限约束下量子程序的生存期，提出了下限和上限的语义验算机制，以确保临时量子信息在其不再具有意义时被安全地清理，从而降低了量子电路的门次数。


<details>
  <summary>Details</summary>
Motivation: 该论文旨在解决量子程序在运行时如何有效地管理和清理临时量子信息的问题。随着量子计算机的广泛应用，如何提高量子程序的效率和可扩展性是一个重要的研究方向。

Method: 该论文采用了形式化的方法，提出了有限层次语义_bound的模型和语义保留性机制，并通过结合生存期分析和验证，展示了如何确保临时量子信息在其意义失效时被安全地清理。

Result: 论文的结果表明，通过该方案，可以有效减少量子电路的门次数，提高程序的效率，同时确保一切都是安全的，避免程序运行时的逻辑错误。

Conclusion: 该研究为量子程序的语义设计和运行时优化提供了新的思路，为实现高效的量子程序设计和运行提供了理论基础。
...
### [90] [IQP Born Machines under Data-dependent and Agnostic Initialization Strategies](https://arxiv.org/abs/2603.14576)
*Sacha Lerch,Joseph Bowles,Ricard Puig,Erik Armengol,Zoë Holmes,Supanut Thanasilp*

Main category: quant-ph

TL;DR:  Circuit-based Born machines with IQP circuits show promise for quantum generative modeling, but their training landscapes lack understanding of initialization effects. This work reveals that certain initializations lead to problematic barren plateaus. Establishes variance bounds and proposes a data-dependent initialization that empirically performs well, suggesting better convergence in practice. Uses a general framework for analyzing similar quantum machine learning models. contributes to the understanding of warm-start effects in quantum learning algorithms.


<details>
  <summary>Details</summary>
Motivation: As quantum computing progresses, there is growing interest in quantum machine learning, particularly in developing models that can effectively utilize quantum resources for tasks like generative modeling. IQP-based circuits are promising candidates due to their structure and classical hardness assumptions.

Method: The study employs theoretical analysis, mathematical proofs, and numerical experiments. Proved that random full-angle-range initializations lead to barren plateaus using MMD loss landscapes. Derived lower bounds on loss variance for specific initializations. Introduced a data-dependent initialization based on target distribution. Conducted numerical training on 150-qubit circuits using genomic data to validate the approach.

Result: Key findings include the presence of barren plateaus with random full-angle initializations, meaning the optimization becomes highly challenging. Lower bounds on loss variance provided insights into the effectiveness of different initialization strategies. The data-dependent initialization showed potential to align better with the target distribution, leading to stable training. Practical implementation with 150 qubits on genomic data highlighted the viability of the data-dependent approach, suggesting it could be broadly applicable in quantum machine learning.

Conclusion: This work enhances our understanding of training IQP-based Born machines, particularly the role of initialization. The findings suggest that careful choice of initializations can mitigate issues like barren plateaus. The data-driven approach offers a promising direction for practical quantum machine learning applications, supported by numerical evidence from large-scale quantum circuits.
...
### [91] [Evaluating Calibration-Based Digital Twins for IBM Quantum Hardware Simulation](https://arxiv.org/abs/2603.14607)
...
### [92] [GPU-Accelerated Quantum Simulation of Stabilizer Circuits](https://arxiv.org/abs/2603.14641)
*Muhammad Osama,Dimitrios Thanos,Alfons Laarman*

Main category: quant-ph

TL;DR: 这项研究提出了新方法对GPU加速的量子门群进行高效模拟，重点是数据并行几何表型和对 NullPointerException的可扩展处理。


<details>
  <summary>Details</summary>
Motivation: 量子计算模拟领域的优化和发展。

Method: 通过重新设计瓶颈问题，如高斯消元和测量更新，将其转化为适合GPU的并行操作。

Result: QuaSARQ显著提高了运行速度，时间速度提升可达105倍，且能效比提高了超过80%。

Conclusion: 该研究为提升量子群体模拟的可扩展性提供了有效的解决方案，尤其是在需要大量测量和采样的工作负载方面。
...
### [93] [Exact characterizations for quantum conditional mutual information and some other entropies](https://arxiv.org/abs/2603.14650)
*Zhou Gang*

Main category: quant-ph

TL;DR: A precise mathematical characterization of mutual information, entropy, and related concepts is provided, offering sharp bounds and clear convex/concave properties.


<details>
  <summary>Details</summary>
Motivation: The paper advances the field of quantum theory by offering exact and optimal characterizations of mutual information and entropy, which are crucial for quantum error correction and channel reconstruction.

Method: The authors use a summation of explicitly constructed terms to define mutual information and entropy, ensuring positivity, convexity, and concavity.

Result: An exact characterization of mutual information is introduced, which is optimal regardless of the mutual information's magnitude, with rapid and absolute convergence.

Conclusion: This mathematical framework enhances our understanding of quantum information theory, offering tools for optimal channel reconstruction and error correction.
...
### [94] [Computing logical error thresholds with the Pauli Frame Sparse Representation](https://arxiv.org/abs/2603.14670)
...
### [95] [Adaptive Control of Stochastic Error Accumulation in Fault-Tolerant Quantum Computation](https://arxiv.org/abs/2603.14687)
*Tirtha Haque*
...
### [96] [Learning Quantum Operator Dynamics from Short-Time Data](https://arxiv.org/abs/2603.14699)
*Jinyang Li,Satoshi Iso,Shunji Matsuura,Lingxiao Wang,Xiaoyang Wang*

Main category: quant-ph

TL;DR: 提出了一种物理驱动的神经常微分方程（Neural ODE）框架，用于从短期测量重构量子多体系统中长期算子动力学，应用于transverse-field Ising模型，准确外推长期行为并分辨噪声短时间信号的激发谱。方法展示了从实际量子硬件提取动力学和谱信息的可扩展性和高效性。


<details>
  <summary>Details</summary>
Motivation: 随着量子设备的快速发展，如何有效利用有限的时间和资源进行量子计算和模拟是当前研究中的一个重要课题。本研究旨在通过物理驱动的方法，提出一种高效的数据驱动框架，解决量子多体系统长期动力学的重构问题。

Method: 提出了一种基于物理驱动的神经常微分方程（Neural ODE）框架，通过Pauli基展开和施加局部性和对称性约束，将算子演化降低到可处理的系数动态，从数据中学习这些系数的动态。

Result: 在transverse-field Ising模型中的应用结果表明，该方法能够准确外推长期行为，从噪声短时间信号中分辨出激发谱。

Conclusion: 本研究提出了一种物理驱动的Neural ODE框架，不仅在理论上具有可扩展性，而且在实践中实现了从实际量子硬件的数据中高效提取有用的动力学和谱信息。这一方法为量子系统的研究和量子计算的实际应用提供了新的工具和技术路径。
...
TL;DR: 在投影游戏中，每个玩家的回答之间可能存在不对称性，导致在量子和operator模型中无法直接使用同步游戏的结果。


<details>
  <summary>Details</summary>
Motivation: 投影游戏在量子信息和复杂性理论中具有重要意义，尤其是同步游戏的后设结果在这个领域中的应用。

Method: 研究者调整了同步游戏中Marrakchi和de la Salle的成果，分析了非同步投影游戏的策略结构，尤其是在量子和operator模型中。

Result: 研究表明，采用tracial策略使得投影游戏的成功概率达到1 - O( (Lε)^{1/4} )，并在约束系统游戏中进一步优化了其他参数，扩展了Paddock的结果。

Conclusion: 这项研究为投影游戏的量子约束和operator模型提供了新的成功概率界限，并扩展了现有的结论。
...
TL;DR: 这篇论文提出了一种基于量子-resistant区块链的可扩展架构，结合了双场量子密钥分发（TF-QKD）和测量设备独立性（MDI）协议，实现了从二次到线性复杂度的优化，有效解决了传统点对点链接的速率损失问题，为大规模、长距离的共识系统部署提供了理论支持和可行方案。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的发展，传统区块链系统的安全性面临挑战。量子密钥分发（QKD）技术被用于增强区块链的安全性，尤其是在保证通信通道安全性方面效果显著。然而，现有的QKD集成的量子网络存在可扩展性和基础设施复杂性等问题。因此，如何设计一种既能确保安全性又能高效扩展的区块链架构成为研究者关注的重点。

Method: 论文采用了双场量子密钥分发（TF-QKD）和测量设备独立的（MDI）协议，通过结合信息论安全性机制和分布式共识机制，优化了区块链的架构。利用这些技术手段，将基础设施复杂度从二次增加到线性规模，有效降低了通信距离限制。

Result: 论文成功地提出了一种可扩展、高安全性的量子-resistant区块链架构。通过TF-QKD和MDI协议结合，架构在理论上实现了从二次复杂度到线性复杂度的优化，并且能够在理论上解决传统点对点通信在速率损失方面的限制，为大规模和长距离的共识系统提供了可行的解决方案。

Conclusion: 这项研究的贡献在于提供了一种新型的、可扩展的、量子-resistant区块链架构，不仅解决了现有技术中的可扩展性问题，还为区块链技术在自然界中的广泛应用铺平了道路。
...
TL;DR: A protocol for cavity-free quantum networking using Rydberg atoms demonstrates significant advancements in entanglement generation and efficiency.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for scalable and efficient quantum communication systems motivates the development of cavity-free architectures.

Method: The approach utilizes Rydberg blockade for local control, Raman transitions for atom-photon conversion, and Hong-Ou-Mandel interference for remote entanglement creation. Quantum memories are employed to enable retry protocols.

Result: The protocol achieves high-fidelity entanglement generation (>97.5% Bell state fidelity), directional emission (35%), and single-node efficiency (19%). It also supports rates exceeding 600 Hz at 20 km separation.

Conclusion: This cavity-free architecture offers a practical and scalable solution for distributed quantum computing and secure communication, surpassing traditional cavity-based methods in terms of performance and scalability.
...
TL;DR: Photonic Quantum-Enhanced Knowledge Distillation (PQKD) introduces a novel framework combining photonic and classical components for efficient knowledge distillation in deep learning.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for efficient and scalable machine learning models has driven the development of methods like knowledge distillation, which aims to transfer knowledge from a high-capacity teacher model to a smaller, more efficient student model. However, traditional approaches often struggle with maintaining accuracy while reducing model complexity. The introduction of PQKD seeks to address this challenge by incorporating photonic quantum hardware, offering a unique source of structured randomness that enhances the training process.

Method: PQKD is a hybrid quantum photonic-classical framework where a programmable photonic circuit generates a compact conditioning signal. This signal constrains and guides a parameter-efficient student network during distillation. The system employs dictionary convolutions, where each layer learns only a small set of shared spatial basis filters. The photonic parameters are derived from shot-limited photonic features and are mapped through a fixed linear transform. Training alternates between standard gradient-based optimization of the student and sampling-robust, gradient-free updates of photonic parameters, avoiding differentiation through photonic hardware.

Result: Experiments on MNIST, Fashion-MNIST, and CIFAR-10 demonstrate that PQKD achieves a controllable compression-accuracy frontier. The framework remains close to teacher performance on simpler benchmarks under aggressive convolutional compression. Performance degradation correlates predictably with finite sampling, consistent with shot-noise scaling. The implementation also benefits from exponential moving-average feature smoothing, which suppresses high-frequency shot-noise fluctuations, extending the practical operating regime under moderate shot budgets.

Conclusion: PQKD represents a promising approach for enhancing knowledge distillation by integrating photonic quantum hardware. The framework's ability to maintain accuracy while significantly reducing model complexity could pave the way for more efficient and scalable machine learning solutions, complemented by techniques like feature smoothing to further improve performance under limited sampling conditions.
...
TL;DR: 摘要中提出了一种自测试方法，用于验证高维量子纠缠状态和通用校准测量仪器，为可伸缩的无设备量子信息研究提供了自测试方案。


<details>
  <summary>Details</summary>
Motivation: 当前量子信息理论需要一种无设备的方法来验证量子纠缠和测量仪器，特别是在可伸缩性和并行性方面。该方法解决了这一重要问题。

Method: 该研究通过构建一个Bell不等式，考虑了Alice和Bob双方的测量设置和空间分隔，使用Sum-of-Squares分解来确定量子上界，并通过反交换观察器来确保分析的最佳性。此外，研究确定了最小的量子维数以实现最大Bell违背，从而自测试这些维度的量子态。

Result: 该研究展示了如何在无设备环境中自测试高维量子纠缠和校准测量仪器，具有可扩展性，并提供了一个适用于后续研究的基础方法。

Conclusion: 这项工作为无设备环境中的高维量子纠缠和测量仪器验证提供了关键的方法，具有广泛的应用前景。
...
TL;DR: This paper demonstrates a quantum digital signature system achieving 504 km transmission distance and 21.1 Qbps signature rate using a large-system setup with one-time universal hashing.


<details>
  <summary>Details</summary>
Motivation: The paper aims to advance Quantum Digital Signatures (QDS) by overcoming performance limitations related to rate and distance, with applications in secure communication.

Method: Experiments utilized a twin-field QDS system with a GHz protocol, incorporating one-time universal hashing, employing advanced error correction, and maintaining high-efficiency transmission channels.

Result: Achieved record-breaking transmission distance of 504 km and effective signature rate of 21.1 Qbps, surpassing all previous QDS experiments at similar distances.

Conclusion: Significant leap in Quantum Digital Signature technology for secure and validated communication networks, paving the way for future advancements.
...
### [108] [Ab Initio Study of Erbium Point Defects in 4H-SiC for Quantum Devices](https://arxiv.org/abs/2603.15343)
*Michael Kuban*
...
### [109] [Autonomous quantum heat engine](https://arxiv.org/abs/2603.15355)
*Tuomas Uusnäkki,Miika Rasola,Vasilii Vadimov,Priyank Singh,Ahmad Darwish,Mikko Möttönen*

Main category: quant-ph

TL;DR: 我们实现了基于超导电路的自hatt量子热机，并通过热浴系统实现了有内驱动的微波功率生成。


<details>
  <summary>Details</summary>
Motivation: 量子热机在量子热力学中研究热和功的流动具有重要意义，之前的实验热机需要外部驱动，工作困难。自hatt热机只需热流即可运行，但尚未在任何系统中实证。本研究通过超导电路实现了自hatt热机。

Method: 实验方法包括构建热机电路，实现约厄 tire 循环，连接超导量子干涉装置，与光谱滤波的热和冷浴连通。

Result: 通过改变实验条件，观察到内部动态驱动的微波功率，验证了理论预测，为研究量子效应和利用热微波在超导电路中进行了电路量子电动力学研究。

Conclusion: 这项研究实证了自hatt量子热机的可能性，推动了量子热力学领域的进一步研究。
...
TL;DR: The paper proposes using the SU(3) Wigner function to represent a noisy spin ensemble, showing that physical noise limits the Wigner function to three real parameters, which can be interpreted as polar, azimuthal, and radial components. This leads to the concept of the solid spin Wigner function, visualized on a solid ball.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of the SU(2) Wigner function when dealing with noisy quantum states of spin ensembles. Since physical noise tends to take states outside of the initial SU(2) irreducible representation, the SU(2) Wigner function is no longer effective, and thus finding an alternative representation becomes necessary. The authors consider encoding the noisy state in an SU(3) irrep, leading to the exploration of the SU(3) Wigner function as a potential solution.

Method: The method involves considering a symmetric subspace of SU(2) spin-1/2 particles, where the state is represented as a real-valued function on the 2-sphere (the SU(2) Wigner function). When noise is introduced, the SU(2) irrep is no longer sufficient, so the authors explore expanding the state into an SU(3) irrep. By constructing and analyzing the SU(3) Wigner function for this irrep, they identify that the physical noise imposes constraints on the form of the Wigner function, reducing it to a function dependent on three real parameters. These parameters are interpreted as polar, azimuthal, and radial components, leading to the introduction of the solid spin Wigner function, which is represented as a distribution on a solid ball rather than a hollow sphere as in SU(2).

Result: The key result of this research is the demonstration that the SU(3) Wigner function, when limited by physical noise, reduces to a real-valued function of three real parameters: polar, azimuthal, and radial components. This simplification allows for an intuitive geometric interpretation of the noisy quantum state, visualizing it as a distribution on a solid ball in three-dimensional space. This interpretation provides a new way to represent and understand quantum states affected by noise, particularly in the context of spin ensembles.

Conclusion: The authors conclude that the SU(3) Wigner function offers a promising framework for representing noisy quantum states of spin ensembles. By leveraging the constraints imposed by physical noise, they derive a simplified representation (the solid spin Wigner function) that provides an intuitive geometric interpretation. This work opens new avenues for understanding and analyzing noisy quantum systems, potentially leading to improved methods for quantum error correction and noise mitigation in future studies.
...
TL;DR: A compact, low-cost ultra-high vacuum compatible cell made from glass-ceramic MACOR was developed, featuring CF flange connections for commercial vacuum tech and high numerical aperture viewports for precise optics. It achieved stable vacuum pressures below 1e-10 mbar for over a year.


<details>
  <summary>Details</summary>
Motivation: Quantum technology applications require precise and stable ultra-high vacuum systems. The creation of such a system that is both compact and low-cost, while maintaining high vacuum performance, addresses a critical need in quantum research.

Method: The cell was constructed using glass-ceramic MACOR, which is known for its durability and compatibility with ultra-high vacuum conditions. It includes CF flanges to integrate seamlessly with existing vacuum systems and high numerical aperture viewports for advanced optical measurements.

Result: The developed cell maintained stable vacuum pressures of less than 1e-10 mbar for an extended period, demonstrating its reliability and suitability for various quantum technology applications.

Conclusion: This technology represents a cost-effective and efficient solution for integrating high-precision vacuum measurements into quantum gas experiments, making it a valuable tool for advancing quantum technologies.
...
TL;DR: 论文提出了一种新的方法，称为交替Heisenberg，用于提升 Antarctic系统中 prepare 低能态的能量降低算法的表现，特别是在存在高度不一致性的情况下。该方法通过交替应用多个Hamiltonians来最大化能量降低的效率，能够在小系统规模下显著提高算法效果。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算和量子复杂性理论的快速发展，如何有效地 prepare 低能态成为了一个关键挑战。传统的低能态准备方法常常陷入次优状态，如高能本征态或低方差高能态。因此，开发一种能够有效跨越这一障碍的新方法非常必要。

Method: 论文提出了一种交替Heisenberg方法，通过交替应用多个Hamiltonians，在低能子空间上共享的情况下最大化能量降低效率。此外，还引入了能量不确定性原理，并将其应用于稀疏Hamiltonians，从而在更高能量级别上实现了更大的能量变化。

Result: 数值仿真表明，交替Heisenberg方法在1D AKLT模型和Heisenberg模型的实例上，显著提高了能量降低算法的性能，尤其是在低能态准备和能量降低方面。通过引入稀疏Hamiltonians的能量不确定性原理，能进一步提高算法的性能，尤其是在处理高能量障碍时。

Conclusion: 论文展示了交替Heisenberg方法是一种有效的新方法，用于提升量子计算中 prepare低能态的能量降低算法的效率。通过交替应用多个Hamiltonians，并引入稀疏Hamiltonians的能量不确定性原理，该方法能够在小系统规模下显著提高算法的性能，尤其是在挑战性问题中。未来研究可以进一步优化该方法并将其扩展到更大的系统规模。
...
TL;DR: 提出了一种基于高阶特罗次公式的方法，将第一阶脉冲序列转换为更高级的保持有限脉冲宽度效应鲁棒性的序列，实现了对控制周期时间的任意阶误差缩放。


<details>
  <summary>Details</summary>
Motivation: 为了提高量子模拟中脉冲序列的鲁棒性和精确性，特别是在有限脉冲宽度效应影响下，提出了一种新的方法。

Method: 将第一阶脉冲序列映射到第一阶特罗次公式，应用高阶特罗次公式构造，然后将结果演化为物理上可实现的有限宽度脉冲，通过动态修正门来实现。

Result: 得到的序列能够以控制周期时间的任意阶误差缩放，同时保持有限脉冲宽度效应的鲁棒性，并且支持使用多乘积公式以更高效地构造。

Conclusion: 该框架为量子模拟任务提供了一种高效且鲁棒的方法，实验验证了其预期的误差缩放效果。
...
TL;DR: 通过量子经典混合框架加速有限元分析， Physically reduce simulation time by up to 15% in specific cases and 7% on average.


<details>
  <summary>Details</summary>
Motivation: 有限元分析在工程中至关重要，优化解决方案有助于提高效率。

Method: 结合量子求解器和经典计算工具，利用现有量子和经典硬件进行测试。

Result: 量子框架在仿真中提升了效率，尤其是在振动分析和Transient模拟中。

Conclusion: 量子计算在Noisier中间阶段对大型FEA模拟有显著的加速效果。
...
### [116] [Quantum-Inspired Unitary Pooling for Multispectral Satellite Image Classification](https://arxiv.org/abs/2603.15522)
*Georgios Maragkopoulos,Aikaterini Mandilara,Ralntion Komini,Dimitris Syvridis*

Main category: quant-ph

TL;DR: 量子判敛对经典卷积神经网络中的池化操作具有帮助。


<details>
  <summary>Details</summary>
Motivation: 研究量子判敛方法对经典卷积神经网络的池化操作有何影响。

Method: 探讨了量子判敛和经典池化操作的结合，提出了一种基于复数投影空间的池化机制。

Result: 新的池化机制显著提高了优化稳定性，加速收敛，并降低了方差。

Conclusion: 量子判敛的几何结构特性可以通过经典的预判不变性池化机制来实现，验证了这些特性在完全经典模型中的效用。
...
### [117] [Analog-Digital Quantum Computing with Quantum Annealing Processors](https://arxiv.org/abs/2603.15534)
...
TL;DR: 研究者扩展了量子退火处理器的功能，使其能够执行更复杂的量子计算。通过模拟实验，他们展示了新的计算能力。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的快速发展，减小硬件限制和提高处理能力是关键。扩展量子退火处理器的性能符合这一趋势。

Method: 他们使用了模拟退火处理器，结合了模拟和数字计算，通过辅助量子比特实现了任意基测量。通过在固定多体哈密顿下进行弱耦合，他们利用近似XY模型进行操作。

Result: 实验成功实现了单比特和双比特相干振荡，多比特量子游走和无序链中的安德erson局域现象，验证了理论预测的可行性。

Conclusion: 这些研究扩展了量子退火处理器的应用范围，可能带来了新的计算可能性。
...
### [118] [QiboAgent: a practitioner's guideline to open source assistants for Quantum Computing code development](https://arxiv.org/abs/2603.15538)
...
### [119] [Product Weyl-Heisenberg covariant MUBs and Maximizers of Magick](https://arxiv.org/abs/2603.15550)
...
### [120] [Optimizing and Comparing Quantum Resources of Statistical Phase Estimation and Krylov Subspace Diagonalization](https://arxiv.org/abs/2603.15552)
*Oumarou Oumarou,Pauline J. Ollitrault,Stefano Polla,Christian Gogolin*

Main category: quant-ph

TL;DR: 该摘要介绍了两个早期的容错计算特征能量的方法，分别是QKS和Spe，进行了直接比较并提出了改进措施。


<details>
  <summary>Details</summary>
Motivation: 研究者希望比较不同方法的计算效率和准确性，尤其是在量子计算的误差容限内。

Method: 作者提出了一些方法，例如优化shots的分配以增加 shot 的效率，以及改进了 Spe 的误差约束，减少电路深度。

Result: 两个研究方法被比较，QKS和Spe在计算特征能量时表现出了各自的优势，尤其在优化后，Spe的电路深度减少了约2/3。

Conclusion: 该研究为计算特征能量的容错方法提供了优化的框架，特别对于高阶系统如54电子、36轨道的分子模拟具有重要应用价值。
...
### [121] [Interaction-Enabled Hartree Fixed Points in Fermionic Resetting Dynamics](https://arxiv.org/abs/2603.15554)
*Jishad Kumar,Achilleas Lazarides,Tapio Ala-Nissila*

Main category: quant-ph

TL;DR: 引入了相互作用的复位框架，扩展了复位方法以适用于弱相互作用的量子系统。


<details>
  <summary>Details</summary>
Motivation: 研究弱相互作用系统中复位方法的扩展，为更复杂量子系统提供了方法。

Method: 使用Hartree平均场处理密度-密度相互作用，保持SPDM的闭合性，构建完全积极的Gaussian Lindblad嵌入。

Result: 数值研究表明，相互作用允许构造新的稳定态，超出纯二次系统。

Conclusion: 该框架有潜力推广到更多复杂量子系统和操作场景。
...
### [122] [A direct controlled-phase gate between microwave photons](https://arxiv.org/abs/2603.15587)
*Adrian Copetudo,Amon M. Kasper,Tanjung Krisnanda,Gregoire Veyrac,Shushen Qin,Hui Khoon Ng,Yvonne Y. Gao*

Main category: quant-ph

TL;DR: We demonstrate a coherent, fault-tolerant method for directly entangling microwave photons in superconducting cavities using a Raman-assisted cross-Kerr interaction, which avoids exciting a nonlinear ancillary element and suppresses decoherence.


<details>
  <summary>Details</summary>
Motivation: Quantum computing relies on error-free information processing, which requires large-scale quantum systems implementable with coherent operations. bosonic codes provide a promising avenue by leveraging quantum states of light in highly excited states of harmonic oscillators. However, direct and reliable entanglement between these states remains challenging.

Method: We engineer a Raman-assisted cross-Kerr interaction between microwave photons hosted in two superconducting cavities. Unlike previous approaches, this method does not require exciting a nonlinear ancillary element. Instead, it utilizes the coherent dynamics of the system to generate interactions between photon number states in the two cavities. This avoids introducing additional dissipation channels and potential leakage from the encoded manifold.

Result: By implementing a controlled-phase gate in the two-photon subspace of the oscillators, we achieve direct entanglement between the photons with high fidelity. Furthermore, we harness this dynamics to map the photon-number parity of a storage cavity onto an auxiliary oscillator, enabling robust error detection while protecting the code space from measurement-induced decoherence. This approach realizes a reliable entangling gate within a bosonic code space, which is crucial for fault-tolerant quantum computing operations.

Conclusion: This work expands the capabilities of bosonic circuit quantum electrodynamics (cQED) by enabling direct, coherently mediated interactions between microwave photons in superconducting cavities. The proposed method avoids exciting a nonlinear ancillary element and suppresses decoherence, paving the way for scalable and fault-tolerant bosonic quantum computing architecture.
...
### [123] [Universal Weakly Fault-Tolerant Quantum Computation via Code Switching in the [[8,3,2]] Code](https://arxiv.org/abs/2603.15610)
*Shixin Wu,Dawei Zhong,Todd A. Brun,Daniel A. Lidar*

Main category: quant-ph

TL;DR: 该论文提出了一种通用的后选择型容错量子计算方案，通过代码切换绕过了东斯廷-金定理对单一量子编码的限制。


<details>
  <summary>Details</summary>
Motivation: 开发通用容错量子计算方案是当前量子计算领域的关键挑战。该论文通过代码切换方法，为量子计算提供了新的可能性。

Method: 论文采用两种[[8,3,2]]量子码作为基础，分别支持不同的基本量子门，构建了一个容错计算方案。方法涉及门的后选择和错误检测机制。

Result: 实验结果表明，该方案在二次错误检测下，逻辑错误率得到了平方上的抑制。具体应用了Grover搜索和三量子位问题的编码切换。

Conclusion: 通过后选择和错误检测机制实现的后选择容错量子计算方案，具有在具体算法中实现二次错误抑制的能力。这为解决东斯廷-金定理的限制提供了新方法。
...
### [124] [Exclusive Scattering Channels from Entanglement Structure in Real-Time Simulations](https://arxiv.org/abs/2603.15621)
*Nikita A. Zemlevskiy*

Main category: quant-ph

TL;DR: This paper introduces a method to isolate scattering channels in Matrix Product State simulations based on the entanglement structure of the late-time wavefunction.


<details>
  <summary>Details</summary>
Motivation: Real-time simulations in quantum field theory face challenges in resolving individual scattering channels, particularly due to the complexity of wavefunctions. This work proposes a novel approach inspired by experimental methods to address this limitation.

Method: The method involves performing Schmidt decompositions on spatial bipartitions of the post-scattering state. By analyzing these decompositions, the researchers can distinguish between elastic and inelastic scattering contributions, allowing them to detect outgoing particles of specific species.

Result: The method is successfully applied to detect heavy particles produced in a collision within the one-dimensional Ising field theory. This validates the approach and suggests its applicability to other quantum systems and processes.

Conclusion: This research advances the field of quantum field theory simulations by providing a practical tool for identifying scattering channels. The method's versatility indicates potential wide-ranging applications in future studies of quantum systems.
...
### [125] [A generalized K-space coherent averaging method for engineering lattices of spin-orbit beams](https://arxiv.org/abs/2603.13576)
*Pinki Chahal,Naume Shentevski,Priyanka Vadnere,David G. Cory,Owen Lailey,Dmitry A. Pushin,Dusan Sarenac*

Main category: physics.optics

TL;DR: 研究了基于轨道角动量自旋的光栅阵列的生成方法及其在光栅成像和量子信息中的应用。


<details>
  <summary>Details</summary>
Motivation: 光栅在材料表征、光学捕获、量子信息处理等多方面有重要应用，该研究旨在利用轨道-角动量相互作用的光子态构建复杂的光栅结构。

Method: 通过在k-空间中进行相干平均，通过调整输入光束的倾角、振幅和偏振，精确控制光栅的几何形状和周期性，以及光栅单元内的轨道和径向度数。

Result: 成功生成并表征了一个微米级的光正六边形光栅，每个单元单元内明确的轨道和径向数，且传输效率在96%以上。

Conclusion: 该方法为复杂光栅结构的生成和精细控制提供了新途径，展示了在光栅和量子信息处理等领域的广阔应用前景。
...
### [126] [AI assisted optimization of integrated waveguide polarizers containing 2D reduced graphene oxide](https://arxiv.org/abs/2603.13658)
*Rong Wang,Yijun Wang,Di Jin,Junkai Hu,Wenbo Liu,Yuning Zhang,Duan Huang,Jiayang Wu,Baohua Jia,David J. Moss*

Main category: physics.optics

TL;DR: 该摘要讨论了适量bgo在光集成芯片中的性能及其在偏振选择性光电器件中的应用前景，使用机器学习框架加快了模式模拟速度，减少了计算时间的大量需求。


<details>
  <summary>Details</summary>
Motivation: 研究者希望提高rgo集成型腔波导偏振选择器的性能，但传统方法计算耗时，因此希望寻找一种高效的方法。

Method: 使用了全连接神经网络（fcnn）构建机器学习框架，利用低分辨率模式模拟结果训练模型，从而快速准确地预测性能参数。

Result: 该方法显著减少了计算时间，平均预测偏差低于0.05，显示出高效性和准确性。

Conclusion: 该机器学习框架为rgo集成型腔波导偏振选择器的设计与优化提供了有效的工具。
...
### [127] [Probing strong coupling in core--shell nanoparticles with fast electron beams](https://arxiv.org/abs/2603.13797)
...
### [128] [Optical Resonances: From Eigenmodes to Scattering Features](https://arxiv.org/abs/2603.13845)
*Ilya Karavaev,Kirill Koshelev,Andrey Bogdanov*

Main category: physics.optics

TL;DR: Unidified framework for understanding the central role of electromagnetic resonances in nanophotonics.


<details>
  <summary>Details</summary>
Motivation: Explaining how to interpret resonant phenomena and design robust photonic systems.

Method: Proposing a unified framework that distinguishes between eigenmodes and scattering features.

Result: The framework helps clarify the relationship between eigenmodes, scattering channels, and interference effects.

Conclusion: This framework aids in interpreting resonant phenomena and designing photonic systems.
...
### [129] [Design of Angular-offset Interstitial-Tube- Assisted Hollow-Core Fibers with Ultrahigh Mode Purity and Ultralow Loss](https://arxiv.org/abs/2603.13851)
*Hao Chen,Jiayi Chen,Dawei Ge,Dong Wang,Shoufei Gao,Wei Ding,Dechao Zhang,Yingying Wang*

Main category: physics.optics

TL;DR: 防空心纤维需要更纯化的模式，这项研究通过调整间插管的角度来减少高阶模式的损耗，同时保持核心模式的损耗很低。


<details>
  <summary>Details</summary>
Motivation: 实际系统需要高纯度模式，这里提出了调整间插管角度的方法来实现，这在低损耗设计中非常有潜力。

Method: 通过改变间插管角度，研究者在一个双嵌套的反共振纤维中实现了对高阶模式的控制。他们通过数值模拟验证了这种方法的有效性。

Result: 通过这种方法，他们实现了每公里小于0.05分贝的高纯度核心模式损耗，同时在每公里1分贝的高阻失设计中更有效地去除高阶模式。

Conclusion: 这种方法为后续的实用低损耗光纤设计提供了一个新的途径，特别是在光纤通信中的应用前景广阔。
...
### [130] [Countershading coloration in blue shark skin emerges from hierarchically organized and spatially tuned photonic architectures inside skin denticles](https://arxiv.org/abs/2603.13937)
...
### [131] [Thermally accessible broadband soliton microcombs in silicon carbide enabled by dynamic polarization control](https://arxiv.org/abs/2603.14044)
*Haoyang Tan,Yi Zheng,Xiyuan Lu,Yang Liu,Andreas Jacobsen,Kresten Yvind,Kartik Srinivasan,Minhao Pu*

Main category: physics.optics

TL;DR: 论文提出一种基于动态极化控制的热补偿方案，用于解决微环扇区激光器中光孤子的稳定性问题。通过将泵浦光的一部分传递到正交极化模式，实现了自冷却和可靠光孤子接入，同时将冷却功率转移至微环扇区光孤子生成模式，提升了微环扇区光孤子的功率和带宽。实验结果表明，相比于静态自冷却配置，动态极化热补偿方案在450 nm范围内实现了108 GHz-FSR微环扇区光孤子，并且在20 dB信噪比下提升了约39%的带宽和60%的功率。


<details>
  <summary>Details</summary>
Motivation: 微环扇区激光器在高精密度应用中表现出极大的潜力，如通信、光谱学和 métrologie。然而，热光效应带来的不稳定问题仍是个主要障碍，如何可靠地实现光孤子接入成为亟待解决的问题。

Method: 论文提出了一种基于动态极化控制的热补偿方案。具体而言，该方案分为两个阶段：在光孤子形成初期，将泵浦光的一部分传递到正交极化模式，以实现自冷却和可靠接入；在光孤子形成后，通过极化旋转和泵浦调制将冷却功率转移至微环扇区光孤子生成模式，从而提升了微环扇区光孤子的功率和带宽。实验采用高保真度的微环扇区激光器，展示了Range约为450 nm，50 GHz-FSR频率分辨率的光孤子，同时对比了静态自冷却和动态自冷却配置下的性能提升。

Result: 实验结果表明，提出的方法显著提升了微环扇区光孤子的性能。具体来说，在450 nm范围内实现了108 GHz-FSR微环扇区光孤子，并且在20 dB信噪比下，相较于静态自冷却配置，当量带宽提升了约39%（达到265 GHz），关断功率增加了约60%（达到49.2 mW）。此外，动态极化热补偿方案实现了对光孤子产生效率和系统稳定性显著的改进。

Conclusion: 通过动态极化控制实现的热补偿方案，有效提升了微环扇区光孤子的性能，在保持高精密度的同时，使得微环扇区激光器在高密度通信和其他应用中展现出更大的潜力。这一创新性的方案为解决强热光效应带来的挑战提供了一种切实可行的方法。
...
### [132] [Electron-laser vacuum breakdown in head-on collision of relativistic electrons with intense laser pulse](https://arxiv.org/abs/2603.14063)
*Pavel Golovinski*
...
### [133] [Beyond optical chirality density: tensor-based description of electromagnetic chirality](https://arxiv.org/abs/2603.14071)
*Ilia Smagin,Sergey Dyakov*

Main category: physics.optics

TL;DR: Proposes tensor measures of electromagnetic chirality based on the Lipkin formalism to better capture chiral field-matter interactions in anisotropic media.


<details>
  <summary>Details</summary>
Motivation: Computers and technology heavily rely on understanding and manipulating electromagnetic fields, particularly their chiral properties in complex media.

Method: Extends the conventional optical chirality density to tensor measures by utilizing the Lipkin formalism, enabling a more comprehensive analysis of chiral fields in anisotropic environments.

Result: The new tensor measures provide a richer, more physically transparent description of electromagnetic chirality, aiding in characterizing interactions of structured fields with matter.

Conclusion: This work offers a significant advancement in quantifying electromagnetic chirality, with potential applications in optics, materials science, and nanophotonics.
...
### [134] [Minkowski-Space Modeling of Hyperbolic Lenses](https://arxiv.org/abs/2603.14166)
*Enrico Maria Renzi,Simon Yves,Sveinung Erland,Diana Strickland,Eitan Bachmat,Andrea Alù*

Main category: physics.optics

TL;DR: This paper introduces a Minkowski-space approach to describe hyperbolic wave propagation in anisotropic materials, highlighting its implications for device design despite the inherent phase-energy flow misalignment.


<details>
  <summary>Details</summary>
Motivation: The extreme anisotropy of hyperbolic materials presents challenges in device modeling, driving the need for a new geometric approach to better understand and utilize these materials.

Method: The authors employ a Minkowski-space formalism to embed anisotropy into an effective Lorentzian metric, establishing a framework for designing hyperbolic interfaces and lenses.

Result: The method enables the analytical derivation of transfer functions and resolution limits, leading to ultra-large numerical apertures and deep sub-diffraction focusing capabilities. Validation through a planar van der Waals polaritonic lens operating in the mid-infrared further supports these findings.

Conclusion: This approach paves the way for overcoming the geometric complexity in hyperbolic material design, opening new possibilities in optical device applications.
...
### [135] [Thermal Noise Reduction in Ternary Optical Coatings: From Ti::GeO$_2$-Based Ternary Systems to High Index Materials](https://arxiv.org/abs/2603.14322)
...
TL;DR: 研究提出了一种新的双层涂层设计，以减小光面反射效果和光程漂移，并在理论上预测ASD RF可达0.69。


<details>
  <summary>Details</summary>
Motivation: 增强引力波探测器的感受器性能，减少反射和光程漂移，提高信号与噪声比。

Method: 采用双层双涂层策略进行设计，利用参数化分析方法优化涂层性能，引入高折射率材料。

Result: 基于SiO₂、Ti::SiO₂、Ti::GeO₂的三元涂层，最大ASD RF达0.69，若材料吸收限制放宽，仍无法达到0.5目标。

Conclusion: 初步分析高折射率材料替代现有涂层材料具有潜力，未来的高损失材料可用于 next 世代 涂层设计。
...
### [136] [Robust and Active Visible-Light Integrated Photonics on Thin-Film Lithium Tantalate for Underwater Optical Wireless Communications](https://arxiv.org/abs/2603.14346)
*Changjian Guo,Xingjie Li,Xiaofeng Wu,Jiajie Deng,Wenchang Yang,Weilong Ma,Ziliang Ruan,Kaixuan Chen,Sailing He,Liu Liu*

Main category: physics.optics

TL;DR: 该研究开发了高性能的可见光光波导调制器，实现了高带宽和高稳定性的水下光通信系统。


<details>
  <summary>Details</summary>
Motivation: 随着可见光光子ics在 sensing、precision metrology 和 free-space data links 中的广泛应用，亟需解决高速和鲁棒的调制器问题。

Method: 基于摩擦层状 LiTaO3 波导材料，在 532 nm 波长下，制备了具有 dB/cm 级传播损耗和 ~50 GHz 平稳频率响应的调制器。实验结果表明，所用调制器在 5 dBm 的光功率下保持稳定，优于传统 LiNbO3 基础材料的同类产品。

Result: 在 3 米水下Distance 下实现了 112 Gb/s 的光通信，首次构建基于外置调制器的水下光通信系统，克服了传统激光直接调制系统的带宽-功率-啁啾限制。

Conclusion: 该研究为复杂、可靠和主动的可见光光子集成电路在水下光学应用中奠定了基础。
...
### [137] [4D reconstruction of alumina laser melt pools at 25 kHz via operando X-ray multi-projection imaging](https://arxiv.org/abs/2603.14391)
...
### [138] [Separating partially coherent light](https://arxiv.org/abs/2603.15517)
...
TL;DR: 该研究提出了一种自适应干涉仪架构，能够在硅光子电路中自动分离空间部分相干光的'相干模式'，并通过变分处理实现对共轭矩阵的特征值和特征向量的测量，从而分离部分相干态。该方法仅需线性随秩增加的物理元件，并通过串接优化提高了多重相干态的分离能力。


<details>
  <summary>Details</summary>
Motivation: 随着光通信和成像技术的快速发展，处理高维、部分相干光变得日益重要。本研究展示了通过自适应结构化干涉仪架构实现部分相干态的自动分离，为光通信和光子信息处理提供了新的工具。

Method: 研究采用了硅光子电路中的多层自配置干涉仪架构，并通过变分处理实现了对共轭矩阵的特征值和特征向量的测量。利用电子调制器实现了对其干涉相位调制器的频率调制优化。

Result: 研究结果表明，该方法能够有效地分离部分相干态，并通过实验与基于混合型的相干态测量方法进行了对比，显示了更高的性能和更小的分析误差。

Conclusion: 该研究为自适应相干态分析和控制提供了一种高效、可扩展的解决方案，为光学通信、成像和光子信息处理等领域带来了新的可能性。
...
### [139] [Incoherent Fourier transform spectroscopy with room-temperature coverage from NIR to THz](https://arxiv.org/abs/2603.14611)
*Jakub Mnich,Grzegorz Gomółka,Marco Schossig,Jarosław Sotor,Łukasz A. Sterczewski*

Main category: physics.optics

TL;DR: 该研究提出了一种新型的傅里叶红外光谱仪，实现了室温下从1到50微米（300到6太赫兹）的多波段光谱测量，具有较长的波长截止范围，采用单套光学系统，操作时间仅需几秒。


<details>
  <summary>Details</summary>
Motivation: 光谱学在材料、化学等领域具有广泛应用，但多波段光谱测量尤其在红外领域面临挑战。本研究提出了一种高效的解决方案，具有重要的应用价值。

Method: 该方法利用傅里叶红外光谱仪，结合钻石片分束器和无窗锂钛氧化物（LTO）探测器，实现了对综合不均匀光源的光谱探测。

Result: 实验表明，该光谱仪在室温下实现了从1到90微米（3.3太赫兹）的长波长覆盖，在短时间内完成了多种波段的光谱测量。

Conclusion: 该研究开发了一种高效实用的傅里叶红外光谱仪，为现代化学计量学、材料科学和医学领域提供了强有力的光谱工具。
...
### [140] [Nonlinear optical thermodynamics from a van der Waals-type equation of state](https://arxiv.org/abs/2603.14865)
*Meng Lian,Zhongfei Xiong,Yuntian Chen,Jing-Tao Lü*

Main category: physics.optics

TL;DR: 该论文提出了非线性光学热力学理论，通过均质场近似考虑了模之间相互作用导致的线性光谱重正化，并类比范德华理想气体理论，得出了非线性方程状态，用于预测光功率聚焦和光学乔 consultarheating在约瑟夫-汤姆逊展开中的冷却或加热，从而提供了光波非线性控制和传输的统一热力学视角。


<details>
  <summary>Details</summary>
Motivation: 研究非线性效应对光学热力学理论的影响，扩展现有理论的应用范围。

Method: 使用均场近似，考虑模之间相互作用，进行非线性方程推导。

Result: 预测了光功率聚焦、冷却和加热现象，并提出了统一的热力学视角。

Conclusion: 非线性光学热力学理论扩展了现有理论的应用范围，解释了光波在多模系统中的非线性行为。
...
### [141] [Non-equilibrium quantum plasmonics in nanoparticle-on-mirror nanocavities](https://arxiv.org/abs/2603.14905)
*Artur Avdizhiyan,Ilya Razdolski,Anton Yu. Bykov*

Main category: physics.optics

TL;DR: A new method for controlling quantum plasmonic properties using laser injection of hot electrons in plasmonic nano cavities shows promise for ultrafast optics in quantum nanophotonics and active plasmonics.


<details>
  <summary>Details</summary>
Motivation: Exploring ultrafast optical modulation of quantum-mechanical phenomena at the interface of plasmonic metals for applications in optics, quantum nanophotonics, and active plasmonics.

Method: Proposed experimental approach involves using nanoparticle-on-mirror plasmonic nanocavities and laser-induced ballistic hot electron injection to control plasmonic properties.

Result: The method overcomes limitations in observing laser-driven mesoscopic phenomena in the time domain and provides a pathway for future development in ultrafast electrodynamics in nanogaps and applications in photochemistry.

Conclusion: The work expands the realm of quantum nanophotonics into non-equilibrium electronic systems, facilitating the development of ultrafast methods in active plasmonics.
...
### [142] [Inverse Design Validated Optimization of Lead-Free Cs$_3$Cu$_2$Cl$_5$ Visible-Light Microring Resonators Using a Coupled DFT-FDTD Framework](https://arxiv.org/abs/2603.14931)
*Shoumik Debnath,Sudipta Saha*

Main category: physics.optics

TL;DR: The paper investigates the design and performance of add-drop microring resonators (MRRs) using the Cs3Cu2Cl5 material, a lead-free all-inorganic halide with desirable optical properties. The study employs numerical simulations to optimize the MRR's geometry, achieving a high loaded quality factor, strong coupling, and low extinction ratio. Sensitivity analysis shows that the coupling gap and bend radius are critical design parameters. Cross-platform validation using Tidy3D confirms the results, and inverse design improves performance slightly.


<details>
  <summary>Details</summary>
Motivation: Understanding and optimizing MRRs using Cs3Cu2Cl5 is crucial for next-generation photonic integrated circuits, particularly for visible-wavelength applications. This study provides a comprehensive analysis of the device's geometry and its impact on performance.

Method: The research utilizes numerical simulations with FDTD and Tidy3D, parameterizing the ring width, coupling gap, and bend radius. Density functional theory (DFT) is employed to calculate optical properties, which are then used as inputs in the simulations. Parametric sweeps are performed to evaluate the impact of different design parameters on the MRR's performance metrics.

Result: The study identifies the optimal geometry for the Cs3Cu2Cl5-based MRR, achieving a loaded quality factor of approximately 5386, a free spectral range of 11.3 nm, and a drop-port extinction ratio of 32.2 dB. The sensitivity analysis reveals that the coupling gap and bend radius significantly affect the device's performance. Cross-platform validation using Tidy3D corroborates the results, and inverse design of the coupling region yields minor performance improvements.

Conclusion: This work lays the foundation for the systematic design and optimization of Cs3Cu2Cl5-based microring resonators, which could enable high-performance photonic integrated circuits for various applications.
...
### [143] [Broadband temporal localization and delocalized temporal edge states in time photonic crystals](https://arxiv.org/abs/2603.15115)
*Junkai Jiang,Hao Hu,Yang Long,Liangliang Liu,Songyan Hou,Dongjue Liu,Zhuo Li*

Main category: physics.optics

TL;DR: 研究通过材料的双各向异性电磁响应引入了新的机制，探讨了时间晶体中的时空局域化和边沿态的相互作用。


<details>
  <summary>Details</summary>
Motivation: 时间光晶体在光子科学中具有广泛的应用，理解其非厄密性来源及其与时空动力学的相互作用具有重要意义。

Method: 通过扩展广义布里渊区框架到时间域，分析了电磁常数参数对时空局域化和边沿态的影响。

Result: 发现材料非厄密性增强了时间光晶体的时空局域化能力，且可调节参数控制能量集中在特定时间，还发现了二维时间光晶体中的广域时空间局域化和一维边沿态。

Conclusion: 该研究提供了新的机制来操控时间光晶体中的时空局域化和边沿态，为光子科学和 photonics 器件设计提供了理论基础。
...
### [144] [Velocity-tunable exciton-photon hybridization in cathodoluminescence](https://arxiv.org/abs/2603.15191)
*Sven Ebel,Martin Nørgaard,Christian Nicolaisen Hansen,N. Asger Mortensen,Sergii Morozov*

Main category: physics.optics

TL;DR: 研究通过调节电子能谱实现激发子-光子杂化的新途径。


<details>
  <summary>Details</summary>
Motivation: 探索激发子-光子杂化在缺乏物理结构修改条件下的动态调控方法。

Method: 研究了悬空亚微米薄片中的传输级射频干涉现象，模拟分析了不同厚度和电子能量下的效应。

Result: 通过调整电子能量可连续调控激发子-光子杂化，杂化程度与薄片厚度及电子速度有关。

Conclusion: 成功开发出一种无需结构修改的动态调节激发子-光子杂化方法，为研究介电效应下的激发子-光子相互作用提供新平台。
...
### [145] [Multi-Plane Spatially Resolved Phase Structuring Using Optical Communication Modes](https://arxiv.org/abs/2603.15222)
*Vinicius S. de Angelis,Maximilian Jeindl,Leonardo A. Ambrosio,David A. B. Miller,Federico Capasso,Ahmed H. Dorrah*

Main category: physics.optics

TL;DR: 该论文提出了一种确定性的三维光束整形框架，可用于多轴控制强度和相位，无需迭代优化或随机相位处理，降低了互平面交叉干扰，提高了控制精度。


<details>
  <summary>Details</summary>
Motivation: 光束整形在通信、医学成像等领域有广泛应用，现有技术如迭代优化和随机相位处理带来交叉干扰和 speckle 噬菌体 noise，限制了确定性相位控制。因此，开发无需迭代且能精确控制多轴相位的框架具有重要意义。

Method: 论文提出了一种基于奇异值分解的通信模式叠加方法。将目标场分解为源平面到多目标平面的通信模式的线性组合，这些模式彼此正交且能量高效，从而降低了互平面交叉干扰。

Result: 实验验证了该方法能用单个相位光栅准确重构多平面的强度和相位，包括任意结构相位奇点。

Conclusion: 该研究为多平面光束整形提供了一种切实可行的框架，尤其适用于需要深度相位结构和相干性的应用场景。
...
### [146] [Resolving Nonequilibrium Gas Kinetics in Supersonic Neutral Flows with Coherent Rayleigh Brillouin Scattering](https://arxiv.org/abs/2603.15272)
*Atulya Kumar,Gabriel Alfaro,Marios Kounalakis,Stefan Karatodorov,Alexandros Gerakis*

Main category: physics.optics

TL;DR: Single-shot coherent Rayleigh-Brillouin scattering (CRBS) is successfully used to study high-speed flows in nonequilibrium conditions within underexpanded jets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop and validate a technique for direct measurement of flow parameters in complex, unsteady supersonic environments, which is crucial for understanding turbulence and shock wave phenomena.

Method: The study employs single-shot CRBS using 200ns laser pulses to simultaneously probe velocity distributions and density across a jet's spatial locations, analyzing velocity gradients and flow structures.

Result: The technique shows good agreement with numerical simulations, demonstrating its effectiveness in capturing non-Maxwellian velocity distributions influenced by shocks, thus validating its application in unsteady supersonic flows.

Conclusion: Single-shot CRBS is established as a vital tool for direct, high-resolution measurements in intricate flow dynamics, significantly advancing the study of turbulence and supersonic environments.
...
### [147] [Phase retrieval technique from regions of unresolvable fringe density using Phase Shifting Interferometry and Liquid Crystal on Silicon (LCOS) Spatial Light Modulator (SLM)](https://arxiv.org/abs/2603.15318)
*Indrani Bhattacharya*

Main category: physics.optics

TL;DR: This paper describes a method to recover phase information from unresolvable fringe corners using wavefront compensation, building on previous research.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of obtaining phase information in scenarios where traditional fringe analysis fails, likely relevant to optical metrology and imaging.

Method: _wavefront compensation technique combined with corner analysis_, a method for phase recovery in unresolved fringe regions.

Result: The method successfully works for wavefront compensation.

Conclusion: Expands on prior work, [14], offering a new approach for phase recovery in unresolvable fringe density areas.
...
### [148] [High-performance Sources of Multidimensionally Engineered Quantum Light Based on Monolithic Microcavity-metalens Interfaces](https://arxiv.org/abs/2603.15391)
...
TL;DR: 该研究通过结合量子点和超薄金质片，开发出一种集成的高维量子光源，在单片半导体芯片上实现了对量子光的全方位调控，包括光辐射角散、发射方向、极化状态和角向动量，同时生成了高保真的极化-角向动量纠缠单光子和局域自旋拓扑光子，成功实现了量子光的稳定传播，尤其是在大气湍流中的表现优于传统结构量子光。


<details>
  <summary>Details</summary>
Motivation: 随着光量子技术的发展需求，如光量子计算、量子通信等，对量子光的可操控性提出更高要求，亟需一种能在单片半导体上集成多种调控方式的量子光源。

Method: 研究者在III-V化合物半导体芯片上设计并实现了量子点微球形源与超薄金质片的集成结构，通过腔量子电动力学效应实现量子光的高纯度和强亮度，同时通过多光子调控实现对光子多种属性的调节。

Result: 研究结果表明，该集成装置在单片内即可实现对量子光的全方位调控，成功生成高保真的极化-角向动量纠缠，局域自旋拓扑光子，并在大气湍流中实现了量子光的稳定传播。

Conclusion: 该研究为集成型量子光子学和超分辨光学提供了新方法，为光量子技术的实用化提供了理论和实验支撑。
...
### [149] [Noise and dynamics in acoustoelectric waveguides](https://arxiv.org/abs/2603.15482)
...
### [150] [A systematic design approach for one-dimensional and crossed photonic nanobeam cavities for quantum dot integration](https://arxiv.org/abs/2603.15498)
*Oscar Camacho Ibarra,Jan-Gabriel Hartel,Atzin David Ruiz Perez,Sonja Barkhofen,Klaus D. Jöns*

Main category: physics.optics

TL;DR: 设计了一维光子晶体纳米条纹腔，通过优化周期、空洞几何和长度，实现了光子被禁锢的同时减小了发射损失和 linewidth 的展宽。此外，还设计了交叉型同余和异相型纳米条纹腔，通过减少参数扫描，提升了量子光子集成应用的效率，且与单光子发射器兼容。


<details>
  <summary>Details</summary>
Motivation: 光子被被禁锢的纳米结构在量子光子ics中有广泛的应用，减少损失和提高效率是关键。

Method: 通过同时优化周期、空洞几何和长度，实现了光子被禁锢的同时减小了发射损失和 linewidth 的展宽。利用参数扫描和网络搜索方法计算.

Result: 优化后的结构成功地减小了参数扫描，提升了效率，且与单光子发射器兼容。

Conclusion: 该方法为光子ics中的优化设计提供了有效途径。
...
### [151] [Beyond Murray's Law: Non-Universal Branching Exponents from Vessel-Wall Metabolic Costs](https://arxiv.org/abs/2603.13687)
*Riccardo Marchesi*
...
### [152] [A Unified Variational Principle for Branching Transport Networks: Wave Impedance, Viscous Flow, and Tissue Metabolism](https://arxiv.org/abs/2603.14691)
*Riccardo Marchesi*

Main category: physics.bio-ph

TL;DR: A study reveals that the branching geometry in mammalian arterial trees exhibits a scaling exponent $α$ of approximately 2.70, which doesn't align with existing theoretical models but is explained by incorporating sub-linear vessel-wall scaling and optimizing metabolic costs.


<details>
  <summary>Details</summary>
Motivation: Understanding the principles behind the structure of biological transport networks is crucial for insights into evolution, design optimization, and cardiovascular health.

Method: The study mathematically models network optimization under varying metabolic costs using functional equations and game theory, incorporating vessel scaling effects.

Result: A unique metabolic-optimal network architecture is identified with scaling exponent $α^*=2.72$, matching observed data and showing insensitivity to metabolic changes.

Conclusion: This provides a foundational, data-driven understanding of arterial branching, suggesting evolutionary adaptation to metabolic demands.
...
### [153] [Digital unzipping of DNA through a solid-state nanopore: A theoretical study for base-by-base ratcheting](https://arxiv.org/abs/2603.14705)
*Tomoki Ohkubo*

Main category: physics.bio-ph

TL;DR: 这篇论文提出了一种无需蛋白质的ratchet机制，用于控制DNA在纳米孔中的移动，实现可重复的基础位移，为全固态纳米孔测序提供理论依据。


<details>
  <summary>Details</summary>
Motivation: 随着生物技术的发展，用于测序的微型设备需要更高程度的集成和简化，无需蛋白质的机制将有助于实现更小型化和更高效的测序仪器。

Method: 该论文通过理论建模和统计力学分析，进行了以下研究：1. 建立了一个基于双链DNA机械解离的ratchet模型；2. 引入电动力机制来保持DNA在解离期间的位置；3. 利用统计力学模型跟踪DNA位置的变化和方差。

Result: 本文的结果显示，通过亚微秒的 holding 机制切换，可以实现基础单位的单步移动，错误率小于5%。这意味着数字解离结合可逆 holding 机制可以产生确定的单基础运动，从而为全固态纳米孔测序提供可行的路径。

Conclusion: 该研究为非生物驱动的DNA传输机制在纳米孔中的应用提供了一个理论框架，推动物件化的全固态测序技术的发展。
...
### [154] [Mathematical and Computational Modeling of Amoeboid Cell Crawling](https://arxiv.org/abs/2603.15350)
...
### [155] [Oscillons from $Q$-balls in generalized models](https://arxiv.org/abs/2603.15505)
...
### [156] [M-theory on $S^1\vee S^1$ as Type 0A](https://arxiv.org/abs/2603.13468)
*Zihni Kaan Baykara,Emilian Dudas,Cumrun Vafa*

Main category: hep-th

TL;DR: We propose a new geometric dual for Type 0A string theory using M-theory on a non-standard 2D compact space called Sub-Planckian S¹∨S¹. Strong quantum effects cause fields to live on different resolutions of this space, linking tachyon condensation of 0A to a shrinking S¹ in IIA. We then connect this to dualities for 0B and propose a critical point for Type 0B, linking conformal fixed points for non-SUSY 4D gauge theories to 0B branes.


<details>
  <summary>Details</summary>
Motivation: Understanding dualities between string theories is a major goal for theoretical physics, particularly in finding AdS/CFT duals, new vacua, and testing for potential de Sitter vacua. This specific paper explores M-theory duals of Type II strings, employing concepts like tachyon condensation, which play a central role in string theory dynamics and their implications for gauge/gravity dualities.

Method: The paper introduces a Sub-Planckian 2D compact space and uses tachyon condensation and dualities to connect Type 0A and 0B string theories to IIA and 11D M-theory. It leverages mathematical techniques such as modular transformations in SL(2,Z), particularly the Γ0(2), to propose a critical point in the string potential, arguing for its existence using both M/F-theory dualities and subsequent analysis for gauge theories.

Result: The authors establish a potential relationship between dual string theories and different geometries, particularly linking tachyon condensation to decoupling limits. They describe an axio-dilaton F-theory description and propose a candidate for a de Sitter critical point in the String de Sitter Landscape conjecture. The connection is made to provide a strong coupling fixed point in non-SUSY 4D gauge theories via branes at the 0B horizon, aiding explorations of low-energy physics and AdS/CFT.

Conclusion: This paper makes a significant step towards unifying different string theories via novel compact spaces and connecting them through tachyon dynamics and M/F-theory duality. If their proposed critical point is unique, it would represent a strongly coupled dS vacuum, challenging certain conjectures in string cosmology.
...
### [157] [Analytic structure of holographic thermal correlators from Fourier series](https://arxiv.org/abs/2603.13469)
...
### [158] [Quantum obstructions for $N=1$ infinite distance limits -- Part II: Kähler obstructions](https://arxiv.org/abs/2603.13470)
*Lukas Kaufmann,Jeroen Monnee,Timo Weigand,Max Wiesner*

Main category: hep-th

TL;DR: 研究了四维Type IIB/F理论紧致化中N=1超对称情况下复结构模空间的量子修正，发现极限诱导其它领域很强的反作用，体现经典不可分的非N=1理论的量子行为。研究了F理论在Calabi-Yau四维流形上的Kähler模量子修正，从弦论世界面理论和BPS-instanton作用的角度分析，得出量子反应回到经典不可分，需要量子修正保序处理。


<details>
  <summary>Details</summary>
Motivation: 研究量子修正对超对称和string理论的影响，尤其是复结构模空间的量子行为和场空间不可分现象，有助于理解string紧致化和模型构建。

Method: 从世界面理论分析候选EFT弦在纯无限远复结构极限的量子反应回，然后验证了大量依赖复结构的BPS-instanton量子修正及其树级行为，探讨了钾模在量子不可控时需要的共缩放问题。

Result: 发现量子修正使纯大复结构极限失效，需要钾模的共缩放来保持微扰控制，从而修正经典有效作用。

Conclusion: 研究表明量子修正对复结构无限远区域的描述失败，需通过钾模的共缩放来维护微扰有效，影响string模型构建和模空间稳定化问题。
...
### [159] [State-dependent geometries from magic-enriched quantum codes](https://arxiv.org/abs/2603.13475)
*ChunJun Cao,Gong Cheng,Krishnanand Karthikeyan,Cathy Li,John Preskill*

Main category: hep-th

TL;DR: The paper discusses quantum error-correcting codes and their relation to emergent spacetime. Existing holographic models can only describe quantum fields on fixed backgrounds and fail to account for gravitational backreaction. The authors argue that approximate quantum error correction is necessary to incorporate backreaction. They introduce a Ryu-Takayanagi-like entropy decomposition for approximate codes, defining bulk matter entropy via optimal recovery and a proto-area entropy as the difference between boundary entropy and recoverable bulk entropy. For skewed codes, proto-area increases monotonically with bulk entropy, similar to quantum extremal surfaces. The origin of this behavior is linked to tripartite non-local magic in the Choi state of the encoding map, which vanishes in stabilizer codes but affects the leading matter-geometry coupling in approximate codes.


<details>
  <summary>Details</summary>
Motivation: This paper addresses a significant limitation in the application of quantum error-correcting codes to the study of spacetime and gravity: the inability of existing holographic models to describe gravitational backreaction. By introducing the concept of approximate quantum error correction, the authors open new possibilities for understanding how quantum information theory and general relativity can be unified.

Method: The authors build upon existing frameworks of quantum error correction and holography, introduce novel concepts like approximate erasure-correcting codes, and use mathematical tools such as entropy decomposition and analysis of the Choi state to derive their results. They consider specific classes of quantum codes and analyze their behavior under certain perturbations.

Result: The authors demonstrate that approximate quantum error correction can incorporate gravitational backreaction. They provide a decomposition of entropy that aligns with the behavior of quantum extremal surfaces. Their analysis reveals a link between the proto-area entropy and the structure of the encoding map's Choi state, particularly in relation to stabilizer codes and leading matter-geometry couplings.

Conclusion: This paper establishes a new framework for incorporating gravitational backreaction into holographic models using approximate quantum error correction. It provides a deeper understanding of the relationship between quantum information theory and spacetime emergence, suggesting that approximate codes are necessary for a complete description of spacetime dynamics.
...
### [160] [On the Gauge-Invariant Fermion](https://arxiv.org/abs/2603.13684)
*Kang-Sin Choi*

Main category: hep-th

TL;DR: The paper demonstrates that Dirac dressing of fermions is equivalent to a gauge parameter shift, with physical implications for mass renormalization and addressing issues like non-locality and path dependence.


<details>
  <summary>Details</summary>
Motivation: Understanding fermionic dressing and its relation to gauge theory is crucial in quantum field theory, particularly for phenomena involving mass and compositeness.

Method: The authors analyze the equivalence through mathematical transformations within the framework of quantum electrodynamics (QED), focusing on renormalization effects.

Result: The study reveals that physical mass is consistent across different gauges, resolving apparent obstructions such as non-locality and path dependence.

Conclusion: This equivalence offers a new perspective on fermion interactions, emphasizing the role of gauge invariance in maintaining physical consistency.
...
### [161] [A Quantum Weak Cosmic Censorship and Its Proof](https://arxiv.org/abs/2603.13957)
*Naman Kumar*

Main category: hep-th

TL;DR: The paper demonstrates that in quantum gravity theories, singularities must be hidden to maintain thermodynamic consistency, ensuring that no 'naked' singularities can form, thus protecting the principle of cosmic censorship.


<details>
  <summary>Details</summary>
Motivation: The study aims to explore the connection between quantum information, spacetime geometry, and cosmic censorship, seeking to establish a quantum mechanism that enforces censorship principles.

Method: The authors utilize quantum information theory and spacetime thermodynamics to analyze the entropy of singularities, comparing it to the generalized entropy bound.

Result: Singularities discovered in quantum gravity models violate the entropy bound and are thus ruled out; hence, singularities must be hidden to preserve thermodynamic consistency.

Conclusion: A Quantum Weak Cosmic Censorship principle is established, suggesting that singularities in nature are inherently censored, providing a robust explanation that aligns with both quantum and classical theories.
...
### [162] [Spectral Geometry and the One-Loop QED $β$-Function on $S^3 \times S^1$](https://arxiv.org/abs/2603.14081)
*Lyudmil Antonov*

Main category: hep-th

TL;DR: 该论文通过计算三维球面$S^3 	imes S^1$上带电荷的Dirac算子的热核数据，直接得出了QED$β$函数的系数为$β(e) = e^3/(12π^2)$。


<details>
  <summary>Details</summary>
Motivation: 研究者希望通过热核方法和ζ函数正规化方法，从紧致流形的谱数据中提取量子修正项，以验证谱RG流的普适性。

Method: 论文采用热核方法和ζ函数正规化，计算$S^3 	imes S^1$上带电荷的Dirac算子的凸展开式中的$a_4$系数，通过$F_{μν} F^{μν}$项的贡献确定了$β(e)$。

Result: 通过$a_4$系数中的$F_{μν} F^{μν}$项，精确得出了$β(e) = e^3/(12π^2)$，证明了谱RG流在无平坦空间传播子的情况下的有效性。

Conclusion: 该研究结果完全依赖于几何和谱数据，并独立于几何参数和规范选择，为谱动作原理在曲率背景下提供了非平凡的验证。
...
### [163] [Geometric Aspects of Covariant Phase Space Formalism: Solution Space Slicings and Surface Charge Integrability](https://arxiv.org/abs/2603.14439)
...
### [164] [Flux Quantization on M-Strings](https://arxiv.org/abs/2603.14440)
...
### [165] [Quantum potential with no perturbative series, and nonperturbative vacuum dominated by complex classical paths](https://arxiv.org/abs/2603.14467)
*Edward Shuryak*

Main category: hep-th

TL;DR: 该论文讨论了具有特殊耦合常数的势能，通过这种方法消除了摄动级数，成功用复解算出非微扰真空能量。


<details>
  <summary>Details</summary>
Motivation: 探索如何消除摄动级数，发现特殊耦合常数下的新方法。

Method: 使用复解和牛顿方程计算非微扰能量。

Result: 成功用复解模拟非微扰能量。

Conclusion: 提出了消除摄动级数的新方法，具有理论意义和潜在应用。
...
### [166] [A Holographic Model for Soft Photons and Gravitons in Four Dimensions](https://arxiv.org/abs/2603.14499)
...
### [167] [Quantum M2-branes and Holography](https://arxiv.org/abs/2603.14544)
*Jesse van Muiden*
...
### [168] [Residual group-like symmetries in selection rules without group actions](https://arxiv.org/abs/2603.14836)
*Jun Dong,Tatsuo Kobayashi,Shuhei Miyamoto,Ryusei Nishida,Hajime Otsuka*

Main category: hep-th

TL;DR: 该论文分析了群相乘类型的圈诱导对称性，探讨了如何通过'群化'方法保留精确的群对称性，进而影响非交换环路诱导相互作用的强度。


<details>
  <summary>Details</summary>
Motivation: 研究圈诱导对称性的产生及其在理论物理中的应用，为理解环路相互作用的对称性性质提供了新的视角。

Method: 利用组合数学和群论方法，研究有限群的共轭类在融合代数中的作用，特别是环路层面违反融合规则但保留群对称性的情况。

Result: 通过在共轭类上实施'群化'方法，发现保留的群对称性包括阿贝尔和非阿贝尔群，这些对称性能够精确控制环路相互作用的强度，从而使某些参数满足't Hooft意义下的自然性。

Conclusion: 研究结果表明，通过精确的对称性分析，可以更好地理解并解决非交换环路相互作用的对称性问题，为弦理论模型的发展提供了理论支持。
...
### [169] [Galois Covers of Calabi-Yau Quivers and BPS State Counting](https://arxiv.org/abs/2603.15334)
*Johannes Aspman,Cyril Closset,Elias Furrer,Jan Manschot*

Main category: hep-th

TL;DR: 论文的中心是BPS-quiver及其在Four-dimensional N=2超对称场论中的应用，特别是研究了BPS-quiver的价格覆盖及其在弦理论中的几何线索。涉及群作用、Kontsevich-Soibelman代数以及BPS不变量的显式覆盖公式。此工作提供了新的见解和工具来分析自然存在的广义场覆盖，验证了多个特殊情况，并提出了一般的猜想。


<details>
  <summary>Details</summary>
Motivation: 研究BPS-quiver及其在4d N=2场论中的应用具有重要意义，尤其是在弦理论和几何工程中的角色。通过群作用和覆盖理论可以更深入地理解这些结构的内在联系和物理意义。

Method: 该论文主要采用了几何工程方法和群作用理论，研究了BPS-quiver的甘斯覆盖以及这些覆盖在弦理论中的应用。利用Kontsevich-Soibelman代数的性质和BPS-invariants的覆盖公式，得出了关于物质不变量的显式表达式。

Result: 研究的结果表明，利用群作用和覆盖理论，可以将复杂的BPS-invariants分解和表达为覆盖quiver的更简单结构的组合。这为理解4d N=2场论中的BPS状态提供了新的方法和工具。此外，通过测试多个特殊情况，作者提出了一个一般的覆盖公式猜想，这为未来的研究提供了方向。

Conclusion: 论文的研究成功地将BPS-quiver的群作用覆盖与4d N=2场论中的BPS-invariants联系了起来。得到的覆盖公式为计算和理解这类场论的物理性质提供了种新的有效方法。未来的工作可以进一步探索这些应用以及在其他场论中的扩展。
...
### [170] [Classical Gravitational Scattering from the Ultraviolet and the Absence of Calabi-Yau Integrals in the Conservative Sector at $O(G^5)$](https://arxiv.org/abs/2603.15383)
*Zvi Bern,Avery Jackman,Gareth Mansfield,Michael S. Ruf*

Main category: hep-th

TL;DR: Calabi-Yau和完全椭圆积分在第五阶后并未影响保守观测值，尽管它们在中间步骤中出现。在偶数阶数中，保守贡献与动量传递的对数值成正比，这些对数源于红外奇异性。显示出，Classical极限中，引发这类对数的积分类在需要生成对数值的UV奇异性中不存在。这一视角也提供分析偶数阶数保守积分Classical极限的替代策略。


<details>
  <summary>Details</summary>
Motivation: 理论物理中的多圈积分在Post-Minkowskian展开中起着关键作用。理解这些积分在经典极限的行为对于验证理论预测至关重要。

Method: 深入分析多圈积分在Classical极限下的贡献，比较理论预测与计算结果，利用代数几何方法研究积分类的奇异性。

Result: 发现了Calabi-Yau和完全椭圆积分在Classical极限中不产生对数项，表明偶数阶保守贡献与动量传递的对数成正比。

Conclusion: 提供了一种新的分析策略，有助于更深入理解多圈积分在Classical极限的行为及其在Post-Minkowskian展开中的贡献。
...
### [171] [Holographic Krylov complexity in the Coulomb branch of ${\cal N}=4$ SYM](https://arxiv.org/abs/2603.15435)
*Dimitrios Zoakos*
...
### [172] [Baryons in $SO(N)$ vector models and their duals in higher spin theory](https://arxiv.org/abs/2603.15501)
*Bo Sundborg*
...
### [173] [Twisted Holographic Superfluids in External Magnetic Field](https://arxiv.org/abs/2603.15572)
...
### [174] [Bouncing geodesics, black hole singularities, and singularities of thermal correlators](https://arxiv.org/abs/2603.15598)
*Sašo Grozdanov,Samuel Valach,Mile Vrbica*

Main category: hep-th

TL;DR: 研究了 bounce测地线用于分析黑洞奇点的适用性，利用 Hadamard理论证明了当两点可被null测地线连接时，孤束和边界散射传播功能发生发散。分析了此陈述在测地线范围以外的有效性，讨论了 holographic renormalization对偶传播子结构的改变。给出了任意时空复杂测地线及其奇点相关联的通用表征。比较了位置和动量空间中相关函数的分析结构，并通过显式实例进行了讨论。最后验证了 bounce测地线方法在研究黑洞奇点应用中的有效性和具体限制。特别地，显示了一个自恰性线性轴ions黑洞，在这种情况下，其存在奇异点，尽管不存在 bounce测地线。


<details>
  <summary>Details</summary>
Motivation: 测地线历来是研究时空结构的重要工具，但当时空具有奇点或处于量子效应支配时，传统测地线理论可能出现失效。研究如何利用 bounce测地线及其相关传播函数来诊断和分类时空奇点，不仅有助于深入理解引力场的量子效应，同时也提供了一种在理论上和实验上检验奇点存在的有效途径。

Method: 研究采用的理论框架包括Riemann几何、波动方程传播理论以及AdS/CFT对偶。具体步骤主要包括绘制.component
1. 理解时空中的 bounce测地线性质及其对奇点的影响。

Result: 1. 证明了 bulk和边界传播者发散当且仅当两点可被null测地线连接。2. 分清了 bounce测地线方法在测地线范围内的有效性，即在算符任意维度情况下这一陈述依旧成立。3. 研究了 holographic renormalization对偶传播子结构的影响。4. 给出了任意时空中 bounce测地线及其相关奇点的一般描述。5. 比较了位置和动量空间中相关传播子的结构，并给出示例。6. 验证了 bounce测地线方法的有效性及其具体的限制条件。7. 展示存在即使没有 bounce测地线，时空仍可能具有奇点的情况。

Conclusion: 通过 bounce测地线及其相关传播函数分析时空奇点，展现了holographic框架在描述以及诊断时空奇点的潜在适用性。该研究不仅在量子场论和广义相对论的结合处丰富了理论工具，也为黑洞物理学中的奇点问题提供了新的思考角度。
...
### [175] [SAGE: Synthetic Aging for a Grid Environment](https://arxiv.org/abs/2603.13976)
*Paolo Esquivel,Mark A. Harris,Stephen J. Harris*

Main category: physics.app-ph

TL;DR: SAGE 是一个用于模拟电池群组长期icted aging的开源框架，通过物理建模和多学科数据集成进行验证。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源电池储能系统的广泛应用，their 长期 degradation 和 reliability 成为关键挑战。开发 accurate 和 efficient 的 degradation modeling 和预测算法至关重要。

Method: SAGE 是一个多学科集成框架，融合了随机环境因素、市场调度、电热行为、aging kinetics 和资产异质性。它使用物理模型验证一致性和建模。框架采用透明的、可外部参数化的架构。

Result: 模拟显示，温度环境的异质性、制造差异及生产环境等因素会导致电池组的 State-of-Health（SOH）轨迹在无强加统计失败假设下自然发散。框架为优化、SOH 估计和机器学习提供了基准平台，促进研究的可重复性。

Conclusion: SAGE 通过多维度建模为电池群组的长期 degradation 提供了可靠的工具，推动 grid-scale 能源存储系统的优化和研究。
...
### [176] [Nonabelian elastic metamaterials using holonomies acquired by crossing degeneracies](https://arxiv.org/abs/2603.13979)
*Mohit Kumar,Ralph M. Kaufmann,Fabio Semperlotti*

Main category: physics.app-ph

TL;DR: 通过将非阿贝尔特征融入弹性元材料，可以在许多实际应用中实现波控制，但现有实现受到限制。本研究提出了一种设计原则，用于构建连续的非阿贝尔弹性元材料波导，展示了如何通过几何控制参数变化来实现非阿贝尔和声学波动 Effects。


<details>
  <summary>Details</summary>
Motivation: 随着元材料在表面声学波装置、模式交叉器和在线计算等领域的广泛应用，探索非阿贝尔特性及其应用成为了当前研究的热点。然而，现有技术在实现连续介质方面受到限制，本研究旨在突破这一限制，通过自由设计非阿贝尔元材料波导，扩展其潜在应用领域。

Method: 研究通过理论分析和数值模拟相结合的方式，构建了一个由多个圆柱形波导组成的复合波导系统，通过空间变化的连接元素来实现参数的几何控制变化。通过设计合适的连接结构，使参数能够跨越关键退化点，从而诱导非阿贝尔和声学波动 Effects。

Result: 研究发现，通过交叉退化点的方法，可以实现非阿贝尔几何相位，这是与传统的阿贝尔和非阿贝尔相位理论截然不同的机制。这种新方法在广泛频率范围内有效，并且对连接结构的几何变化具有较强的鲁棒性。

Conclusion: 该研究为非阿贝尔物理在弹性元材料中的应用提供了一种切实可行的设计框架，扩展了元材料在波控制领域的潜力，为未来的实际应用奠定了基础。
...
### [177] [ANNA: a toolbox for Newtonian Noise Analysis](https://arxiv.org/abs/2603.15157)
...
### [178] [Effect of spin disorder on the specific loss power of a nanomagnet](https://arxiv.org/abs/2603.15376)
*A. Michels,H. Kachkachi*

Main category: physics.app-ph

TL;DR: 文章探讨了磁纳米结构中源于表面无序的弱磁不共线性，并通过有效的单磁单问题框架进行分析，结果表明表面引起的磁不一致会提升特定耗散功率，在缓慢退化解下。


<details>
  <summary>Details</summary>
Motivation: 研究表面无序对磁纳米结构特定耗散功率的影响，具有在材料科学和磁纳米技术中的应用价值。

Method: 利用有效单磁单问题框架，并结合朗格日近似，推导出特定耗散功率随影响比ζ变化的半定量表达式，研究了其在慢退化解时的行为。

Result: 表面磁不一致会增强特定耗散功率，在慢退化解下。其增强效应依赖于影响比ζ的符号，并受磁纳米结构形状（球形或立方体）的影响。

Conclusion: 本文通过有效的单磁单问题框架，分析了表面磁不一致对单磁纳米体特定耗散功率的影响，得出在慢退化解下，特定耗散功率得到增强。结论为理解磁纳米结构中的磁不一致效应提供了理论依据。
...
### [179] [Physics-Informed Deep Neural Network Design of Reactively Loaded Metasurfaces](https://arxiv.org/abs/2603.15430)
*Malik Almunif,John Le,Anthony Grbic*

Main category: physics.app-ph

TL;DR: 该研究提出了一种基于深度神经网络的元表面逆设计方法，通过结合物理建模的前向求解器，实现了根据预设的远场辐射特性设计元表面的开环负载分布。文章提到这种方法显著降低了训练成本，并通过数值模拟验证了其准确性。


<details>
  <summary>Details</summary>
Motivation: 元表面在天线、雷达等领域有广泛应用，如何实现精确设计使其具有特定的辐射特性是其重要研究方向。

Method: 该研究提出了一种结合深度神经网络和物理基准求解器的方法。DNN用于将目标远场模式映射到元表面单位元的开环负载分布上。通过前向求解器计算辐射模式，使用余弦相似性损失函数指导学习。

Result: 实验结果表明，该方法能够准确合成多种形状和方向的辐射模式，验证了其设计精度。

Conclusion: 该研究提供了一种高效的逆设计方法，适用于元表面的设计与优化。
...
### [180] [Ultrafast photo-thermoelectric currents in graphene junctions in the mid-infrared](https://arxiv.org/abs/2603.13457)
...
TL;DR: Graphene showed a broad ultrafast photocharacteristic even in mid-infrared, but the response slowed down with longer wavelengths.


<details>
  <summary>Details</summary>
Motivation: Understanding graphene's ultrafast photonic response at mid-infrared wavelengths has important implications for various applications, including optoelectronics and photonics.

Method: The researchers used ultrafast pump-probe photocurrent spectroscopy with dual split gates to examine carrier dynamics in graphene junctions under mid-infrared excitation.

Result: The study showed that graphene maintains its broad ultrafast photocharacteristic in the mid-infrared, though the response time increased with longer wavelengths. Electron-phonon interactions were found to be significant but not dominant at these wavelengths.

Conclusion: Graphene exhibits a robust, broad mid-infrared photoresonance, hinting at potential applications in ultrafast optoelectronic devices despite wavelength-dependent response characteristics.
...
### [181] [Magnetic-field-induced superconductivity in hexalayer rhombohedral graphene](https://arxiv.org/abs/2603.13498)
...
### [182] [Dissipative Nonlinear Phononics: Nonequilibrium Quasiperiodic Order in Light-Driven Spin-Phonon System](https://arxiv.org/abs/2603.13513)
*Brayan I. Eraso-Solarte,Yafei Ren*

Main category: cond-mat.mes-hall

TL;DR: The research investigates the role of dissipation in controlling nonequilibrium states within nonlinear phononics. By tuning spin relaxation time, the system transitions from a steady limit cycle to a temporally ordered state breaking discrete time-translation symmetry. Persistent spin and phonon angular momentum oscillations emerge at an emergent frequency, sustained by a dissipation-induced phase lag creating a feedback loop. These oscillations emerge from a dissipation-controlled transition described by a pseudo-potential in a Landau-type framework with U(1) phase symmetry.


<details>
  <summary>Details</summary>
Motivation: The paper's study is driven by the seeks to understand how dissipation can be manipulated to control and stabilize nonequilibrium states in spin-phonon coupled systems, particularly under circularly polarized light driving.

Method: The study employs theoretical analysis, including deriving equations of motion for the spin and phonon angular momentum, analyzing the system's response under varying spin relaxation times, and applying Landau-type theory to describe the resulting nonequilibrium states. Numerical simulations likely provided support for the theoretical findings.

Result: The key findings include the identification of a dissipation-induced phase lag that facilitates the emergence of a temporally ordered state characterized by persistent oscillations of both spin and phonon angular momentum. The oscillation frequency is generally incommensurate with the driving frequency, and the system's behavior can be accurately modeled using a pseudo-potential framework with U(1) symmetry.

Conclusion: This work paves the way for dissipation as a powerful control parameter in nonlinear phononics, offering new avenues for manipulating nonequilibrium states in quantum materials and potentially enabling novel applications in quantum technologies.
...
### [183] [Overcoming intrinsic material limitations through cavity feedback](https://arxiv.org/abs/2603.13515)
*M. Ebrahimi,Y. Huang,V. A. S. V. Bittencourt,A. Rashedi,A. Metelmann,J. P. Davis*

Main category: cond-mat.mes-hall

TL;DR: 该研究通过反馈调节抑制谐波抑制，实现了强耦合放大器，极大提升超声与磁性介导光子的相互作用，迈出了抗干扰实际操作的一步


<details>
  <summary>Details</summary>
Motivation: 该研究基于光、声、磁三体相互作用的复杂性，试图填补少见多种量子分立子之间相互作用的空白， especially在微纳系统中，该研究具有实际应用价值

Method: 我们实现了光导体与磁性介导光子的反馈和文章

Result: 通过高频微波反馈调节环，实现了超声线宽显著缩小，因而在磁性介导光子与谐波之间的相互作用被仔细调控

Conclusion: 该结果应在现实应用中带来光子、声子和磁性介导子之间的较强相互作用，尤其在光谱精美的微纳米系统中是极其有用的
...
### [184] [Observation of two-component exciton condensates in an excitonic insulator](https://arxiv.org/abs/2603.15443)
...
### [185] [Spin qubit gates via phonon buses in electron nanowires](https://arxiv.org/abs/2603.13519)
...
### [186] [First-principles modeling of electrostatics and transport in 2D topological transistors](https://arxiv.org/abs/2603.13714)
*Hyeonseok Choi,Yosep Park,Subeen Lim,Yeonghun Lee*

Main category: cond-mat.mes-hall

TL;DR: 模拟了二维 Topological绝缘体场效应晶体管（2D TIFETs）的电静止和输运特性，基于第一性原理的密度泛函理论（DFT）。


<details>
  <summary>Details</summary>
Motivation: 了解2D TIFETs在不同电场和电势下的行为

Method: 使用了密度泛函理论（DFT）进行模拟，结合Landauer-Büttiker公式和局部势纹

Result: DFT中基函数和对称性限制了临界电场强度，并通过比较kp模型展示了DFT在模拟实际边界面态中的必要性

Conclusion: DFT方法为2D TIFETs的微级输运提供了高效且严格的模拟方法。
...
### [187] [Tight-Binding Device Modeling of 2-D Topological Insulator Field-Effect Transistors With Gate-Induced Phase Transition](https://arxiv.org/abs/2603.06021)
...
### [188] [Eccentricity valley Hall effect](https://arxiv.org/abs/2603.13801)
*Jin Cao,Shen Lai,Cong Xiao,Qian Niu,Shengyuan A. Yang*

Main category: cond-mat.mes-hall

TL;DR: Monolayer GeS2 exhibits a completely universal, highly robust, intrinsic effect: Eccentricity Valley Hall Effect (eVHE) which naturally emerges across all 25 topological crystalline insulators (TCIs) in 25 layer groups.


<details>
  <summary>Details</summary>
Motivation: Valleytronics has become a promising platform for spinless electronics and efficient spin-orbit coupling mechanisms. However, the majority of studies have focused on systems with valleys at time-reversal-breaking points, leaving unexplored the behavior of valleys at time-reversal-invariant points (TRIVs). This paper shifts the paradigm and investigates the novel phenomenon of Eccentricity Valley Hall Effect (eVHE) in systems with TRIVs.

Method: The research primarily employs first-principles calculations to analyze the electronic band structure and transport properties of monolayer GeS2, a material known to host TRIVs.

Result: The calculations predict that monolayer GeS2 exhibits a significant valley Hall angle of 0.74 when subjected to first-principles calculations. Additionally, measurements based on nonlocal transport exhibit characteristic scaling behavior, confirming the existence of eVHE.

Conclusion: The discovery of eVHE represents a critical overlooked aspect of valley Hall physics, expanding our understanding beyond the established VHE paradigm and offering new avenues for advancing valleytronics.
...
### [189] [Designing Extremely Low-Power Topological Transistors with 1T'-MoS2 and HZO for Cryogenic Applications](https://arxiv.org/abs/2603.07902)
*Yosep Park,Yungyeong Park,Hyeonseok Choi,Subeen Lim,Yeonghun Lee*

Main category: cond-mat.mes-hall

TL;DR: This paper suggests that cryogenic topological transistors with negative-capacitance topological insulator field-effect transistors (NC-TIFETs) have an extremely steep transfer curve and ultra-high transconductance at low drain voltage, making them ideal for minimizing power dissipation in large-scale quantum computing systems.


<details>
  <summary>Details</summary>
Motivation: Large-scale quantum computing systems rely on cryogenic electronic controllers, but current technologies face high power dissipation issues. This paper aims to address such challenges.

Method: The authors propose NC-TIFETs by combining a gate-field-induced two-dimensional 1T'-MoS	extsubscript{2} topological channel with a hafnium-zirconium oxide (HZO) ferroelectric gate insulator. They theoretically analyze the electrical properties of these devices.

Result: NC-TIFETs exhibit an extremely steep-slope transfer curve and ultra-high transconductance at low drain voltage ($V_{	ext{D}}$).

Conclusion: NC-TIFETs provide a promising solution for reducing power dissipation in cryogenic electronic interfaces, which are essential for large-scale quantum computing systems.
...
### [190] [Extracting the Anyonic Exchange Phase from Hanbury Brown-Twiss Correlations](https://arxiv.org/abs/2603.13898)
...
### [191] [Finite-Time Braiding Dynamics within Topological Nanowire Qubits](https://arxiv.org/abs/2603.14064)
*Adrian D. Scheppe,Michael V. Pak*

Main category: cond-mat.mes-hall

TL;DR: This paper explores topological quantum computing using Majorana fermions in $p$-wave topological superconducting nanowires. The authors establish a dynamical framework to design finite-time gates that extend beyond the traditional adiabatic regime, specifically for shuttling domain edge bound Majoranas, using two methods involvingspatiotemporally-dependent onsite and hopping parameters. The paper's framework is then generalized to the T-qubit model, providing a more practical framework for realistic experimental setups. The authors aim to facilitate the development of fault-tolerant, scalable quantum systems and universal gate design.


<details>
  <summary>Details</summary>
Motivation: The paper's motivation seems to stem from the growing interest in Majorana fermions as a platform for topological quantum computing, a promising candidate for building a scalable and fault-tolerant quantum computer. The study focuses on leveraging the properties of Majorana fermions in $p$-wave topological superconductors for implementing quantum gates, which is a significant aspect of quantum information science.

Method: The authors use a combination of theoretical modeling and dynamical analysis to investigate the methods of shuttling Majorana fermions. They propose two distinct approaches to manipulate the Majorana bound states within a single nanowire system through spatiotemporally dependent onsite and hopping parameters. The dynamical analysis involves solving the time-dependent Hamiltonian of the system to predict the behavior of the Majorana fermions under different control parameters.

Result: The main results of the paper are the successful derivation of finite-time gates that operate beyond the adiabatic regime. The authors demonstrate how specific spatiotemporally optimized control parameters can be used to shutt widely-separated Majorana bound states, significantly enhancing the practicality of implementing topological quantum gates. They also extend their framework to the T-qubit model, which simplifies the gate design and provides a pathway for scalable quantum computing.

Conclusion: The paper concludes by emphasizing the importance of this work in advancing the understanding of non-Abelian anyons in topological quantum computing and suggests that the framework developed can be extended to additional platform realizations. The authors highlight the potential of their work to contribute to the development of scalable and fault-tolerant quantum systems, which is a critical step toward the realization of quantum computing technologies.
...
### [192] [Imaging Harmonic Generation of Magnons](https://arxiv.org/abs/2603.14082)
*Anthony J. D'Addario,Kwangyul Hu,Maciej W. Olszewski,Daniel C. Ralph,Michael E. Flatté,Katja C. Nowack,Gregory D. Fuchs*

Main category: cond-mat.mes-hall

TL;DR: The paper investigates magnonic harmonic generation using a nonlinear spin-wave framework and experimentally confirms the mechanism underlying the generation through magnonic imaging. The study reveals that edges and domain walls in the microstripe act as localized, anharmonic potentials, facilitating the generation of higher-order harmonics with increasing wavevector and spin selectivity.


<details>
  <summary>Details</summary>
Motivation: Understanding and controlling magnonic harmonic generation is crucial for advancing nonlinear spin-wave systems, which could enhance memory devices, sensors, and next-generation electronic circuits.

Method: The researchers developed a nonlinear spin-wave framework and combined it with scanning nitrogen-vacancy magnetometry to image and quantify magnonic harmonics in a Ni81Fe19/Pt microstripe. They observed a systematic increase in wavevector at higher harmonics and measured spin-selective responses, confirming the theoretical model.

Result: The study demonstrates that edge and domain wall potentials drive azimuthal anharmonic confinement, leading to higher-order harmonic generation with systematically increasing spin-wavenumber order up to the fifth harmonic. The results align with theoretical predictions.

Conclusion: This work provides a detailed microscopic understanding of magnonic harmonic generation and opens new avenues for designing higher-order nonlinear functionality in magnonic systems, which is essential for advanced spin-based devices.
...
### [193] [Ab Initio Transfer Length Method Simulations of Tunneling Limits in 2D Semiconductors](https://arxiv.org/abs/2603.14296)
*Tae Hyung Kim,Juho Lee,Yong-Hoon Kim*

Main category: cond-mat.mes-hall

TL;DR: As semiconductor devices approach the sub-2 nm technology node, achieving reliable contact resistance is critical, but the transition between thermionic emission and direct tunneling is not well understood. This paper presents a systematic framework using first-principles and large-scale DFT calculations to analyze the contact resistance and tunneling limits of various metal-2D semiconductor interfaces. Ab initio transmission line model (TLM) analyses for different metals and geometries reveal a universal transition from direct tunneling to thermionic emission, providing a physically grounded metric for contact quality and informing optimal contact strategies for next-generation 2D transistors.


<details>
  <summary>Details</summary>
Motivation: The transition from thermionic emission to direct tunneling in deep nanoscale metal-2D semiconductor interfaces is a critical yet unresolved challenge, which this paper addresses through a novel computational framework.

Method: The paper employs a systematic first-principles approach, combining large-scale multi-space density functional theory (DFT) calculations with ab initio transmission line model (TLM) analyses for various metals and geometries.

Result: The study reveals a universal transition in resistance scaling between direct tunneling in the sub-10 nm regime and thermionic emission at longer channel lengths, and identifies optimal contact strategies based on metal work functions for n-type and p-type operations.

Conclusion: This framework provides a rigorous metric for evaluating contact resistance and informs practical guidelines for engineering low-resistance, scalable contact technologies for next-generation 2D transistors, advancing the development of reliable and efficient sub-2 nm semiconductor devices.
...
### [194] [Nanoscale mapping of internal magnetization dynamics reveals how disorder shapes heat generation in magnetic particle hyperthermia](https://arxiv.org/abs/2603.14566)
...
TL;DR: 微觉场加热中单个磁颗粒的微观机制还不太清楚。结合起来用AC磁性测量实验和动态微磁学模拟，发现了微粒级的不均匀热源分布。粒径大小影响了热释放的量和时空分布。优化了纳米花结构，建立起微结构与微觉场加热机制的关系，明确了优化纳米 flower 的设计原则。


<details>
  <summary>Details</summary>
Motivation: 微觉场加热在生物医学中有广泛应用，如何优化微觉场热致敏材料的微观结构，提高加热效率和技术性能，具有重要的理论和实践意义。

Method: 结合AC磁性测量实验和动态微磁学模拟，分别研究微粒结构、磁化动力学和宏观热散失等。通过微观尺度观察热源分布，揭示了粒径大小对热释放量和时空分布的调制作用。

Result: 通过粒径大小调整，实现了热释放量和时空分布的优化。纳米花结构在粒径较大时，热释放量更大，而粒径较小时，热源分布更均匀且时间窗口延续更长。

Conclusion: 粒径大小调节是影响微觉场微结构热释放的关键因素，这为设计优化微觉场加热器提供了理论依据和实践指导。
...
### [195] [Quantifying quasiparticle chirality in a chiral topological semimetal](https://arxiv.org/abs/2603.14640)
...
### [196] [Composite boson theory of Hall crystals and their transitions to Wigner crystals](https://arxiv.org/abs/2603.14680)
...
### [197] [Spatiotemporal Magnonic Vortex Beams with Alternating Transverse Orbital Angular Momentum](https://arxiv.org/abs/2603.14713)
*Muyang Xie,Chenchen Liu,Jian Huang,Zhenyu Wang,Xinwei Dong,Ruifang Wang*

Main category: cond-mat.mes-hall

TL;DR: 研究发现磁性纳米条中的时空磁性漩涡束有独特动量和结构，与光、声类似但有显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索不同物理领域中的时空漩涡束及其应用前景。

Method: 通过理论建模和实验验证在磁性纳米条中的时空磁性漩涡束的存在。

Result: 发现了磁性纳米条中的时空磁性漩涡束，包含固定的相位位错，且横 factions的轨道角动量呈现出空间交替的特性。

Conclusion: 发现时空磁性漩涡束在磁性材料中的独特性质，为光、声、磁等不同领域提供了新的研究方向。
...
### [198] [Universal tuning of quantum electrodynamic interactions from power laws to exponential screening and logarithmic antiscreening](https://arxiv.org/abs/2603.14780)
*Michael N. Leuenberger,Daniel Gunlycke*

Main category: cond-mat.mes-hall

TL;DR: 这篇论文提出了一种通用的平台来调节量子电动力学相互作用的范围和强度，从幂律衰减到指数衰减和对数反-screening。


<details>
  <summary>Details</summary>
Motivation: 随着量子技术和微电子技术的发展，理解并控制量子电动力学相互作用在不同域的行为对科学研究和技术应用具有重要意义。这种跨领域的研究有助于开发更高效的材料和器件。

Method: 通过利用绿色函数方法和多反射级数求和，作者在介电层夹层的两条可调二维导体之间，精确地描述了电磁环境，得出了相互作用的定量模型。

Result: 结果显示，相互作用可以通过调节结构的反射系数在幂律、指数和对数反-screening之间切换，实现了 corsConstants 的电控调节。

Conclusion: 这项研究为理解不同相互作用的量子行为提供了一个通用平台，同时为设计和实现电控功能的纳米器件提供了理论基础。
...
### [199] [Moiré Ferroelectricity-Driven Band Engineering in Twisted Square Bilayers](https://arxiv.org/abs/2603.15140)
*Kejie Bao,Rui Shi,Huan Wang,Linghao Huang,Jing Wang*

Main category: cond-mat.mes-hall

TL;DR: Cu2WS4 and GeCl2 realize moiré ferroelectricity and tunneling effects as potential correlated materials.


<details>
  <summary>Details</summary>
Motivation: Investigating novel correlated electronic states in twisted bilayers is crucial for advancing topological materials science.

Method: Theoretical analysis of layer groups combined with ab initio calculations for material realization.

Result: Ferroelectricity and tunneling can independently control moiré effects, enabling precise electronic manipulation.

Conclusion: Twisted square homobilayers open a new avenue for exploring complex electronic states with practical applications in nanoelectronics.
...
### [200] [Spin-valley physics in anomalous thermoelectric responses of the spin-orbit coupled $α$-$T_3$ system with broken time-reversal symmetry](https://arxiv.org/abs/2603.15236)
*Lakpa Tamang,Tutul Biswas*

Main category: cond-mat.mes-hall

TL;DR: The paper investigates the spin-valley physics in the anomalous Hall and Nernst responses of a spin-orbit coupled α-T_3 system with time-reversal symmetry breaking. It demonstrates how the interplay between the spin-orbit interaction (SOI), a staggered magnetization, and a model parameter α enables the tuning of spin- and valley-dependent Hall and Nernst signals. The paper also explains the peak-dip features of the Nernst responses through their corresponding Hall responses using the Mott relation. The magnetization is shown to provide highly tunable spin and valley polarizations, which are calculated from spin- and valley-resolved Nernst conductivities. Both spin and valley polarizations can reach nearly complete polarization over an extended parameter space. The α-T_3 lattice is highlighted as a promising platform for spin and valley caloritronic applications.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for spin and valley-controlled devices has led to significant interest in studying spin-valley physics in materials with strong spin-orbit coupling. The α-T_3 lattice has emerged as a promising candidate for such applications, but its behavior under external influences like magnetization is not fully understood. This paper aims to explore how external parameters like magnetization and model parameters can tune spin and valley responses, providing insights for potential device applications.

Method: The paper uses a combination of analytical derivations and numerical simulations. The theoretical framework involves solving the Dirac equation for the α-T_3 lattice under the influence of SOI and a staggered magnetization. Numerical methods such as density functional theory (DFT) or tight-binding models are employed to calculate Hall and Nernst conductivities and spin-valley polarizations. The Mott relation is utilized to link Hall and Nernst responses, and parameter space exploration is performed through simulations.

Result: The key findings of the paper include the following: 1) The interplay between SOI, magnetization, and α allows for efficient tuning of spin- and valley-dependent Hall and Nernst signals. 2) The peak-dip features of the Nernst responses are well-explained through their corresponding Hall responses via the Mott relation. 3) The magnetization introduces highly tunable spin and valley polarizations, with both spin and valley polarizations reaching nearly complete polarization over an extended parameter space. 4) The α-T_3 lattice demonstrates strong potential for spin and valley caloritronic applications, with both spin and valley contributions being efficiently controlled by external parameters.

Conclusion: This paper provides a comprehensive study of spin-valley physics in the α-T_3 lattice, particularly in the presence of time-reversal symmetry breaking from a staggered magnetization. The results highlight the α-T_3 lattice as a versatile platform for spin and valley-controlled applications, offering a promising route for the development of spin-based devices such as spin-transport sensors, magnonic devices, or spin-caloritronic components.
...
### [201] [Anomalous and Topological Hall Effects in Antiferromagnetic EuSn2As2 Nanostructures](https://arxiv.org/abs/2603.15277)
...
### [202] [Low-frequency noise as a probe of microscopic disorder in CVD-grown graphene](https://arxiv.org/abs/2603.15327)
...
### [203] [Umklapp-Enhanced Interlayer Valley Drag in Moiré Bilayers](https://arxiv.org/abs/2603.15520)
*Ritajit Kundu,Mandar M. Deshmukh,Herbert A. Fertig,Arijit Kundu*

Main category: cond-mat.mes-hall

TL;DR: Van der Waals materials can be combined to form moiré patterns with large unit cell sizes, leading to unique material behaviors. This paper demonstrates that interlayer valley drag in lattice-matched moiré bilayers can be significantly enhanced due to umklapp scattering. Unlike traditional two-dimensional systems, this effect appears at first order in the interlayer interaction and persists at low temperatures. The authors suggest an experimental setup that could detect and characterize this phenomenon.


<details>
  <summary>Details</summary>
Motivation: The phenomenon of interlayer valley drag in moiré bilayers presents an intriguing and potentially exploitable effect in condensed matter physics. The unique properties of moiré patterns, such as large unit cell sizes, create opportunities for studying and harnessing novel electronic interactions that might not be observed in conventional materials. Understanding and controlling such drag effects could lead to advancements in material science, electronics, or other related fields that rely on the manipulation of electron behavior in layered systems.

Method: The authors employ a theoretical analysis combined with computational modeling to investigate the interlayer valley drag in lattice-matched van der Waals bilayers. They derive their results using equations based on the interactions between the layers, considering factors such as the size of the unit cells and the hopping of electrons between layers. Additionally, they propose an experimental setup involving specific lattice-matched van der Waals materials and describe the conditions under which the predicted effects could be observed.

Result: The study reveals that interlayer valley drag occurs at first-order in the interlayer interaction, which is a significant finding compared to more conventional systems. Furthermore, the drag effect remains present at low temperatures, indicating its robustness and potential for practical applications. The proposed experimental geometry, using state-of-the-art fabrication techniques, offers a pathway to observe and study this effect in real systems, providing a bridge between theoretical predictions and experimental verification.

Conclusion: This work opens up new avenues for exploring and exploiting the unique electronic properties of van der Waals moiré bilayers. The findings suggest that interlayer valley drag could be a versatile and controllable phenomenon, potentially leading to new classes of materials with tailored electronic and optical properties. Furthermore, the proposed experimental framework provides a compelling platform for further research, which could lead to the development of novel devices and technologies leveraging these effects.
...
### [204] [Spin-Transfer Torque on Curved Surfaces: A Generalized Thiele Formalism](https://arxiv.org/abs/2603.15545)
*J. I. Costilla,M. Castro,K. V. Yershov,D. Altbir,V. L. Carvalho-Santos,V. P. Kravchuk*

Main category: cond-mat.mes-hall

TL;DR: 研究了曲率对自旋 Magneto tunneling junction中的磁性螺旋结构的影响，提出了扩展的Thiele方程并探讨了曲率与自旋转矩之间的耦合关系。


<details>
  <summary>Details</summary>
Motivation: 磁性螺旋结构在固态物理和微纳技术中有广泛应用，研究曲率对其稳定性以及动力学行为具有重要意义。

Method: 推导了扩展的Thiele方程，纳入了张-李项以考虑曲面的影响，通过数值模拟分析了不同曲率条件下磁性螺旋的运动。

Result: 发现曲率与电流之间存在新的相互作用，磁性螺旋的运动轨迹受到曲率的显著影响，特别在高曲率区域出现了额外的Hall效应。

Conclusion: 本研究为曲率效应在磁性螺旋结构中的应用提供了新的理论框架，为微纳设备的设计与优化提供了理论支持。
...
### [205] [Flat-Band Generation in InAs/GaSb Quantum Wells through Vertically Engineered Heterostructures](https://arxiv.org/abs/2603.15593)
...
TL;DR: 该论文利用纵结构的III-V半导体 heterostructures 技术成功设计出高质量的高能带材料，克服了传统方法的重组、应变等问题，具有可扩展性。


<details>
  <summary>Details</summary>
Motivation: 研究高能带材料对于理解新的宏观量子效应具有重要意义, 同时.rstrip.rstrip 传统方法的重组问题是一个重要的进步。

Method: 使用分子束外 epitaxial生长技术, 通过视觉设计的垂直结构实现高质量的高能带材料制造。

Result: 成功通过四层InAs/GaSb量子阱在磁转运和红外磁光光谱学中观察到高能带。

Conclusion: 该方法提供了一种可扩展的、高度可重复的高能带材料制造途径。
...
### [206] [Probing the Penetration Depth of Topological Surface States by Magnetic Impurity Scattering in V-doped Sb$_2$Te$_3$](https://arxiv.org/abs/2603.15601)
...
TL;DR: 研究发现，范adium杂质（最多0.25%）在碲化物中可以成对靠近并excite Dirac表面态，从而导致能隙。通过范adium杂质的定位散射探针，他们测量出碲化物的Dirac表面态穿透深度在亚纳米级。


<details>
  <summary>Details</summary>
Motivation: 为了确切量化Dirac表面态的穿透深度，特别是在碲化物这种无机晶体中，需要一种直接的方法，而不需要制备不同厚度的多片。

Method: 使用扫描隧道显微镜和光谱学，研究了碲化物中稀有范adium杂质的作用，发现这些杂质可以在不同深度定位散射，从而测定了Dirac表面态的穿透深度。

Result: 首次在 bulk 物质中用单一杂质精确测量 Dirac 表面态的穿透深度，结果在亚纳米级。

Conclusion: Dirac表面态在碲化物中的穿透深度可以精确测量，且稀有范adium杂质是有效的探针，这为该领域提供了新方法。
...
### [207] [Classical linear oscillator in classical electrodynamics with classical zero-point radiation](https://arxiv.org/abs/2603.13446)
*Timothy H. Boyer*
...
Abstract: A classical linear oscillator is treated in the small amplitude limit so that it will be approximately relativistic. The oscillator involves a charge particle in a linear potential in classical zero-point radiation. It is found that the ground state is energy balanced with the power lost in radiation emission equal to the average power gained from resonance with the classical zero-point radiation. Also the oscillator is found to have resonant excited states where the energy emitted as dipole radiation is balanced on average by the energy gained from the zero-point radiation when the action variable of the mechanical system is given by J=(n+1/2)(h/2pi).

</details>


### [208] [Relativistic hydrogen in classical electrodynamics with classical zero-point radiation](https://arxiv.org/abs/2603.13448)
*Timothy H. Boyer*
...
Abstract: Classical electrodynamics including classical electromagnetic zero-point radiation leads to a ground state and resonant excited states for a charged particle in a Coulomb potential. These resonant states correspond to integer values of the action variables analogous to those appearing in the Bohr-Sommerfeld theory of the hydrogen atom. The work on classical zero-point radiation reported here is a continuation of the analysis reported in 1975, but with the addition of the ideas of relativity and resonance between the charged-particle orbit and classical zero-point radiation.

</details>


### [209] [Zeeman effect in hydrogen treated in classical physics with classical zero-point radiation](https://arxiv.org/abs/2603.13449)
*Timothy H. Boyer*

Main category: physics.class-ph

TL;DR: 这篇论文使用经典的电动力学和零点辐射来分析氢原子低能级的塞曼效应。


<details>
  <summary>Details</summary>
Motivation: 研究塞曼效应对氢原子低能级的影响，探索不同能量状态下的量子效应。

Method: 采用经典电动力学和零点辐射模型来处理问题。

Result: 通过该方法，得出了塞曼效应的相关结果，并考虑了空间量化、相对论效应和施门德格拉赫实验。

Conclusion: 该研究为理解氢原子低能级的塞曼效应提供了新的经典方法。
...
### [210] [About the dissipative Newton equation](https://arxiv.org/abs/2603.13989)
*Peter Ván*
...
### [211] [Self-Force of a Dirac String: An Explicit Calculation](https://arxiv.org/abs/2603.14429)
*Alberto G. Rojo*

Main category: physics.class-ph

TL;DR: A Dirac string with fixed flux Φ experiences an axial self-force quantified as F = Φ²/(2πμ₀a²), diverging as the string radius a approaches zero.


<details>
  <summary>Details</summary>
Motivation: The paper investigates the self-force on a Dirac string, originally hypothesized by P.A.M. Dirac, and follows a simplified derivation using the approach by McDonald.

Method: The authors treat the string as a stack of current loops, calculating the force due to the generated radial magnetic field from the solenoid's remaining part.

Result: The derived self-force is F = Φ²/(2πμ₀a²), which diverges as the string radius a approaches zero.

Conclusion: This explicit calculation confirms the non-vanishing, divergent self-force on the Dirac string, aligning with Dirac's original insight and emphasizing its singular nature.

Abstract: A Dirac string can be modeled as a semi-infinite solenoid carrying a fixed magnetic flux. Dirac pointed out that such a string should experience a nonvanishing and divergent self-force, but explicit calculations are rarely shown. Motivated by a recent comment by McDonald, we present a direct and elementary derivation of this self-force. Treating the string as a stack of current loops, we compute the axial force produced by the radial magnetic field generated by the rest of the solenoid. The resulting force, $F=Φ^2/(2πμ_0 a^2)$, diverges as the solenoid radius $a\to0$ with flux $Φ$ fixed, making explicit the singular nature of the Dirac string.
...
### [212] [The ATLAS Trigger System](https://arxiv.org/abs/2603.13409)
*Leonardo Toffolin*

Main category: physics.ins-det

TL;DR: The ATLAS Trigger system at CERN's LHC experiment was crucial for processing the huge amount of data produced during proton-proton collisions, reducing the event rate from 40 MHz to manageable levels for analysis. During Run-3, which ran from 2022 to 2026, significant upgrades were implemented both in hardware-based Level-1 (L1) Trigger and software-based High Level Trigger (HLT) systems to handle increased luminosity and pile-up conditions. The paper discusses the key features of the ATLAS Trigger System, its performance during Run-3, and its essential role in conducting precise measurements and conducting research on new physics.


<details>
  <summary>Details</summary>
Motivation: The motivation behind studying the ATLAS Trigger System likely stems from the need for efficient data processing in high-energy physics experiments, particularly in the context of the increasing luminosity and complexity of data generated at the LHC.

Method: The authors probably conducted a comprehensive analysis of the existing literature and technical specifications related to the ATLAS Trigger System. They likely examined the upgrades implemented during Run-3, including how these upgrades affected the system's performance, and evaluated the system's role in enabling precision measurements and new physics discoveries.

Result: The findings of the study would likely indicate that the ATLAS Trigger System successfully met its design objectives for data reduction during Run-3, played a pivotal role in enabling precise measurements, and contributed to advancing research in new physics.

Conclusion: In conclusion, the ATLAS Trigger System is a vital component of the ATLAS experiment at the LHC, effectively handling the data reduction challenges posed by Run-3's increased luminosity and pile-up conditions. It has been instrumental in facilitating precise measurements and exploring new physics possibilities.
...
### [213] [Development of a high-field pulsed magnet and optical fiber coupled cryostat system for magneto-photoluminescence measurements](https://arxiv.org/abs/2603.13436)
*Deepesh Kalauni,Kingshuk Mukhuti,Tao Peng,Bhavtosh Bansal*

Main category: physics.ins-det

TL;DR: 详细描述了用于磁光测量的高场强脉冲磁系统，利用电解液电容器驱动磁极，达到35特斯拉，用4K氦循环冷却是磁光实验的最佳低温环境。


<details>
  <summary>Details</summary>
Motivation: 光测量需要强大的磁场来增强信号，而传统方法可能温度高或磁场小，导致实验结果不准确或不可行。因此，开发这种新型磁系统和冷却系统尤为重要。

Method: 使用电解液电容器驱动磁极，结合4K氦循环冷却是磁光测量的关键。

Result: 实现了35特斯拉最大磁感应强度，实验条件达到理想要求。

Conclusion: 成功构建了高场强、低温度下的磁光测量系统，为相关实验提供了可靠基础。
...
### [214] [Ultra Fast Calorimeter Simulation with Generative Machine Learning on FPGAs](https://arxiv.org/abs/2603.13490)
...
### [215] [Evaluation of polymer-metal-hybrid bonded wafer-stacks and sensor wafers for ultra-thin hybrid silicon detectors](https://arxiv.org/abs/2603.13992)
...
### [216] [Universal method of selective detection of a wide range of pollutants in liquids using conductance quantization](https://arxiv.org/abs/2603.14140)
...
### [217] [Beam Test of a SiPM-on-Tile ZDC Prototype with 5.3 GeV Positrons at Jefferson Laboratory](https://arxiv.org/abs/2603.14167)
...
### [218] [Towards a Reflective PICOSEC detector?](https://arxiv.org/abs/2603.14295)
*A. Breskin*

Main category: physics.ins-det

TL;DR: PICOSEC是一款超快粒子探测器，使用光栅护罩和气体avalanche乘数器，实现超快检测。


<details>
  <summary>Details</summary>
Motivation: 研究超快粒子探测器以提高粒子物理实验中的检测精度。

Method: 结合光栅护罩和气体avalanche乘数器，利用Cherenkov效应产生光电子并进行乘数检测。

Result: 成功实现了超快检测，部分探测器在气压为毫巴的情况下工作。

Conclusion: PICOSEC展示了高效、可靠的超快粒子探测能力，为未来粒子物理研究奠定基础。
...
### [219] [T-DAQ-P: a portable tablet-form multi-stream data acquisition and contextual telemetry platform based on COTS modules and a custom integration layer](https://arxiv.org/abs/2603.14902)
*D. Tagnani,M. Andreotti*

Main category: physics.ins-det

TL;DR: The paper introduces T-DAQ-P, a compact portable data acquisition and telemetry platform for laboratory and field deployments. It integrates event streaming, slow-control telemetry, and operator-oriented tools into a single unit. The system uses a Raspberry Pi 5 for hosting and a WiFi Arduino UNO R4 for sensor acquisition. It incorporates COTS modules with an engineered interface, providing power, power distribution, logic-level adaptation, and I2C-SPI-ADC connectivity. The telemetry layer uses embedded firmware for automatic I2C discovery, sensor handling, and message framing for NMEA-like parsing. The system includes fault recovery actions and a multi-process host software with message queues and logging. It also features simulation mode and rapid commissioning capabilities. The paper details the system's architecture, electronics, firmware, software, and validation procedures.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is likely to provide a lightweight, reliable, and scalable solution for deploying data acquisition systems in challenging environments, such as those with partial hardware availability or field failures. This is essential for applications where continuous monitoring and data collection are critical, such as environmental monitoring, industrial automation, or research deployments in remote locations.

Method: The methodology involves designing and building the T-DAQ-P platform, integrating and testing its components, including electronics, firmware, and software. The system's architecture is modular, with distinct hosts, sensors, and telemetry units. The electronics integration ensures that COTS components are compatible and function properly through tailored interfaces. The firmware is designed to handle sensor acquisition and telemetry communication, providing robust error handling and recovery mechanisms. The host software employs a multi-process architecture to manage data streams, message queues for data isolation, and time-stamped logging for record-keeping. Validation is performed through functional testing, including simulation mode and rapid commissioning procedures.

Result: The result is a functional and validated portable data acquisition and telemetry platform that can handle partial hardware failures and field deployments. The system is shown to provide accurate data acquisition, telemetry communication, and fault recovery. The paper likely demonstrates the system's capabilities through specific use cases or test scenarios. The detailed architecture and validation procedures suggest that the design is robust and suitable for various deployment environments.

Conclusion: In conclusion, the T-DAQ-P platform successfully integrates necessary components for a portable, reliable data acquisition and telemetry solution. It is designed to address challenges in deployment environments, providing a adaptable and scalable infrastructure for continuous monitoring applications.
...
### [220] [A Modular Datalogger and Slow-Control Platform for Physics Experiments with Time-Series Telemetry and Web Dashboards](https://arxiv.org/abs/2603.14913)
*D. Tagnani,N. Toniolo,B. Gongora,T. Marchi,A. Goasduff*

Main category: physics.ins-det

TL;DR: The paper outlines a NIM 2U slow-control system for monitoring environmental and facility parameters in nuclear physics experiments, incorporating advanced channels, metrological uniformity, and signal processing modules. The system features a robust architecture with expandability, precise measurement accuracy, and integration with visualization tools. Ongoing work focuses on stability and accuracy characterization.


<details>
  <summary>Details</summary>
Motivation: Nuclear physics experiments require precise monitoring of environmental conditions and facility parameters, necessitating a reliable and scalable control system to ensure data integrity and experiment reliability.

Method: The system is designed with a base controller board and extension boards, using 16-bit SAR ADCs with a 2.5 V reference for uniform measurements across diverse probe types. It supports PT100/PT1000 probes with a wide temperature range and includes modules for standard industrial signals and current measurements with automatic gain adjustment. The software integrates time series logging and visualization via Grafana over Ethernet.

Result: The system achieves 1% design accuracy, provides reliable and accurate data acquisition, and integrates well with existing data management systems. The metrological uniformity across heterogeneous channels ensures consistent performance despite differences in sensor types.

Conclusion: The developed system offers a scalable, accurate, and reliable solution for environmental monitoring in nuclear physics experiments, with ongoing work to assess its long-term stability and accuracy performance. The integration of advanced signal processing and visualization capabilities enhances its utility for data analysis.
...
### [221] [The Super Fine-Grained Detector for the T2K neutrino oscillation experiment](https://arxiv.org/abs/2603.14921)
...
TL;DR: 文章描述了升级后的近核探测器ND280中Super FGD探测器的结构、性能及其在T2K实验中的应用，强调了其在提高中微子核反应测量中的系统atics不确定性.


<details>
  <summary>Details</summary>
Motivation: 研究中微子核反应对精确测定中微子振荡参数具有重要意义，因此需要提升探测器性能以减少系统uncertainties.

Method: 使用了分段塑料闪烁材料探测器，结合三维成像和高灵敏度检测技术，实现了向中微子探测领域的突破.

Result: 探测器实现了三维成像，分辨轻子和中子，并通过微秒级时间分辨率实现了Michel电子的精确识别.

Conclusion: 该探测器的成功部署为T2K实验提供了关键的技术支撑，提升了中微子振荡研究的精度.
...
### [222] [Decay-Resolved Charge Changes from Radioactive Decays in Levitated Microparticles](https://arxiv.org/abs/2603.14979)
*Jiaxiang Wang,T. W. Penny,Yu-Han Tseng,Benjamin Siegel,David C. Moore*

Main category: physics.ins-det

TL;DR: 该论文通过测量微球电荷变化，Characterizes单次放射性衰变的电荷变化，并发现了α衰变伴随的电荷 shower现象。


<details>
  <summary>Details</summary>
Motivation: 研究放射性衰变的电荷变化及其机制，为材料科学和放射性环境研究提供新方法。

Method: 利用光 Levitated 微球实时监测电荷变化，结合闪烁探测器阵列的多通道读出。

Result: 直接将电荷变化与放射性衰变关联，揭示α和β衰变电荷释放模式不同，发现因α衰变产生的电荷 shower。

Conclusion: 提出了一种单次放射性衰变研究的新方法，发现α衰变的电荷 shower现象，丰富了放射性衰变机制研究。
...
### [223] [Design and operation of a spark chamber for vacuum ultraviolet light production](https://arxiv.org/abs/2603.15273)
...
TL;DR: Noble liquids like argon and xenon are used as both detectors and targets in dark matter and neutrino physics experiments. Scintillation of noble liquids emits vacuum ultraviolet light, which is detected by specialized sensors. A significant focus in the detector development community is improving the precision of these light sensors. This paper describes the development and testing of a flash lamp to generate light at the wavelengths typically observed in noble liquid detectors, as well as presenting results from a spark chamber prototype operated at room temperature.


<details>
  <summary>Details</summary>
Motivation: The use of noble liquids in dark matter and neutrino experiments highlights their importance in modern physics research. The need for precise light detection has driven the development of specialized detectors. This paper addresses the technical challenges in generating and detecting the required light wavelengths, which is crucial for the effectiveness of these experiments.

Method: The paper details the methodology used to design and construct a flash lamp specifically for testing light sensors. The wavelength range of the light generated by the lamp is matched to the expected emissions from noble liquids used in experiments. Additionally, a spark chamber prototype was developed and operated at room temperature to collect and analyze the light output from the sensors.

Result: The authors successfully designed and built a flash lamp tailored for testing noble liquid detectors. The prototype spark chamber operated at room temperature demonstrated the feasibility of detecting light wavelengths relevant to noble liquid experiments. The results provide insights into sensor performance and potential future optimizations.

Conclusion: This paper contributes to the development of precise light sensors for noble liquid detectors, which are essential for dark matter and neutrino physics experiments. The proposed flash lamp and the experimental results provide a valuable tool for improving detector technology in this field.
...
### [224] [Characterization of Passive CMOS Strip Detectors After Proton Irradiation](https://arxiv.org/abs/2603.15524)
...
TL;DR: Passive CMOS strip detectors can be manufactured with multiple reticles without degrading performance.


<details>
  <summary>Details</summary>
Motivation: The project aimed to fabricate strip detectors in a CMOS foundry using different reticles to investigate stitching effects, with the goal of enabling mass production and potentially active strip sensors.

Method: Fabrication process involved producing passive CMOS strip detectors using 150 nm node technology on a 150 um FZ wafer. Three geometries were tested with varying reticle stitching (3 or 5 reticles) and strip lengths (2.1 cm and 4.1 cm). Detectors were irradiated with 24 GeV protons at CERN and tested in different setups.

Result: No adverse effects were observed from the reticle stitching process, indicating the feasibility of the technology.

Conclusion: The findings validate the production of high-efficiency passive strip detectors with multiple reticles, paving the way for large-scale manufacturing and integration into active sensors in CMOS foundries.
...
### [225] [Microscopic geometric theory for gapped excitations in fractional topological fluids](https://arxiv.org/abs/2603.13489)
*Yuzhu Wang,Bo Yang*

Main category: cond-mat.str-el

TL;DR: Research introduces a geometric method to describe gapped excitations in fractional quantum Hall phases, revealing a duality between Hilbert spaces of graviton-like excitations and quasihole states, and constructs microscopic wave functions for higher-spin neutral modes. Experimental detection methods are also proposed.


<details>
  <summary>Details</summary>
Motivation: Understanding gapped excitations in fractional quantum Hall (FQH) phases is crucial for advancing knowledge in condensed matter physics and seeking potential applications in quantum computing. This paper presents a novel geometric approach to tackle this problem, which could lead to new insights into the nature of these exotic states of matter.

Method: The authors employ a geometric description of the ground state and its excitations, leveraging concepts from conformal field theory and higher-order spin deformations. They establish a duality between the Hilbert spaces of specific excitations and use this to construct wave functions for higher-spin neutral modes. The method also involves analyzing the scattering properties of these excitations to predict their behavior and detectability.

Result: 1. A duality between the Hilbert spaces of graviton-like spin-2 excitations in the Laughlin phase and quasihole states of the irrational Haffnian CFT is identified. 2. A framework for constructing microscopic wave functions for higher-spin neutral modes in the Laughlin phase is developed. 3. The authors show that the many-body Hilbert space of the FQH effect can be generated from geometric deformations of the ground state, providing a comprehensive geometric interpretation of gapped excitations. 4. Scattering properties and potential for long lifetimes are analyzed, suggesting these excitations could be experimentally detected.

Conclusion: This work offers a geometric framework to understand and describe gapped excitations in fractional quantum Hall systems, revealing deep connections between different quantum states and proposing new methods for their experimental detection. The findings not only advance theoretical understanding but also pave the way for future experimental studies of these exotic quantum phases.
...
### [226] [Effective band-projected description of interacting quasiperiodic systems](https://arxiv.org/abs/2603.13548)
*Flavio Riche,Raul Liquito,Bruno Amorim,Eduardo V. Castro,Pedro Ribeiro,Miguel Gonçalves*

Main category: cond-mat.str-el

TL;DR: 研究了一维窄能带系统中电子相互作用与准周期性之间的相互作用，发现了一种新的有效哈密顿方法来处理这类系统。


<details>
  <summary>Details</summary>
Motivation: 准周期材料在现代材料科学中具有重要意义，试图理解它们的电子性质有助于开发新功能材料。

Method: 通过在二阶相互作用项中引入额外的能带贡献，开发了二阶能带投影方法。

Result: 成功地用二阶方法定量地再现了DMRG计算得到的相图，发现了Luttinger液和载波密度波之间的转变以及强准周期性下的准分形载波密度波。

Conclusion: 二阶能带投影是一种有效的方法来研究准周期窄带系统，可为更高维度的准周期和莫瑞材料提供新方法。
...
### [227] [Superradiant strongly correlated quantum states in cavity Hubbard model](https://arxiv.org/abs/2603.13657)
*Kang Wang,Wei-Xuan Chang,Cheng-Yu Bi,Zi Cai,Zi-Xiang Li*

Main category: cond-mat.str-el

TL;DR: 该研究在腔量子材料中发现了一种新的量子相变和量子相图，展示了通过光子辅助增强电子之间的相关性，可以产生新的量子相和相变。


<details>
  <summary>Details</summary>
Motivation: 研究者致力于通过量子蒙特卡罗方法探索复杂量子系统的行为，发现新的量子相和相变有助于理解量子物理中的复杂相互作用。

Method: 研究者开发了一个无符号的费米子-光子混合量子蒙特卡罗算法，并用于系统地研究二维腔Hubbard模型的基态相图。

Result: 发现多种量子相如超辐射反铁磁态和李善兰态以及不同相变如超辐射相变和格ross-neveru类连续性相变。

Conclusion: 该研究展示了通过光子辅助增强电子相关性可以发现新的量子相和相变，为量子材料和量子计算提供理论基础和方法支持。
...
### [228] [Entanglement Rényi Negativity across the Finite-Temperature Transition in the O(3) Universality Class](https://arxiv.org/abs/2603.13711)
*Dong-Xu Liu,Yi-Ming Ding,Zhe Wang,Zheng Yan*

Main category: cond-mat.str-el

TL;DR: 研究利用大型量子蒙特卡洛模拟探索量子纠缠在有限温度相变中的命运，特别是连续对称破缺的情况。他们研究了三维Heisenberg反铁磁材料中的三阶Rényi负性，发现这一相变是一个热临界点。第一个发现热临界点下量子纠缠被完全摧毁的研究。他们的结果表明，物质波动完全破坏了零温度下的长程纠缠。结果还显示，尽管纯面积定律的存在，Rényi负性的温度导数遵循与特定热指数相同的缩放关系。这些发现加强了量子纠缠与经典相变之间可能存在的联系。


<details>
  <summary>Details</summary>
Motivation: 探索量子纠缠在有限温度相变中的命运，特别是连续对称破缺的情况，尤其是利用热临界点进行研究。

Method: 使用大型量子蒙特卡洛模拟研究三阶Rényi负性在三维Heisenberg反铁磁材料中的行为，特别是在O(3)相变点。

Result: 量子纠缠在有限温度相变下崩溃，Néel序 parameter 被完全破坏。Rényi负性显示出纯面积定律，并且温度导数遵循与特定热指数相同的缩放关系。

Conclusion: 量子纠缠对热临界点不敏感，但其导数携带关键指数信息。这揭示了量子纠缠与经典相变之间的潜在联系。此外，Rényi负性仍然可以在连续对称中有效地屏蔽经典相关性。
...
### [229] [Pressure induced redistribution of oxygen hole states in La$_{4}$Ni$_{3}$O$_{10}$](https://arxiv.org/abs/2603.13808)
...
### [230] [Crystal structure, magnetic and resonant properties of decorated spin kagome system (CsCl)Cu$_5$As$_2$O$_{10}$](https://arxiv.org/abs/2603.13865)
...
### [231] [Disentangling Tensor Network States with Deep Neural Network](https://arxiv.org/abs/2603.14425)
...
### [232] [Variance reduction for forces and pressure in variational Monte Carlo](https://arxiv.org/abs/2603.14521)
...
### [233] [Microwave spin resonance in epitaxial thin films of spin liquid candidate TbInO3](https://arxiv.org/abs/2603.14545)
*Sandesh S. Kalantre,Johanna Nordlander,Margaret A. Anderson,Julia A. Mundy,David Goldhaber-Gordon*

Main category: cond-mat.str-el

TL;DR: This paper investigates the quantum spin liquid candidate TbInO3 using superconducting resonators to measure magnetic properties at low temperatures, finding evidence of extreme frustration in magnetic order and identifying the ground state through crystal field analysis.


<details>
  <summary>Details</summary>
Motivation: Understanding complex quantum states of matter, particularly quantum spin liquids, is a key goal in modern condensed matter physics. This paper contributes to that understanding by probing the magnetic properties of a spin liquid candidate using an innovative technique.

Method: The study uses coplanar superconducting resonators, adapted from circuit quantum electrodynamics, to measure magnetic excitations in thin films of TbInO3. Data analysis involves microwaves and crystal field analysis.

Result: Measurements reveal extreme frustration in TbInO3's magnetic order down to 20 mK, far below the Curie-Weiss scale, and identify the ground state doublet eigenstates, influenced by Tb moments' distinct g-factors due to local crystal fields.

Conclusion: The research successfully demonstrates a new method for probing thin films of frustrated magnets and applies it to TbInO3, advancing our understanding of its unique magnetic properties and quantum state.
...
### [234] [Synergistic doping and stabilization of magnetically tunable LnTi$_3$(Sb,Sn)$_4$ (Ln:Ce--Gd) kagome metals](https://arxiv.org/abs/2603.14571)
...
TL;DR: 研究引入了(Sb,Sn)合金化方法，提高了Knightsite化合物的稳定性并调控了磁性和电子性能。


<details>
  <summary>Details</summary>
Motivation: Knightsite结构在磁性材料中有重要作用，探索其合金化方法对其性能调控具有重要意义。

Method: 使用DFT和COHP计算，研究(Sb,Sn)在Knightsite合金中的协同效应。

Result: 合金化通过调整费米能、能态分布和磁性相互作用增强了Knightsite的稳定性。

Conclusion: 此方法为Knightsite材料的样品制备和性能调控提供了新途径。
...
### [235] [Evidence for ferroaxial order in 1T-TiSe$_2$ via elastoresistivity measurements](https://arxiv.org/abs/2603.14613)
...
### [236] [Ferroaxial and nematic transitions in the charge density wave phase of 1T-TiSe$_2$](https://arxiv.org/abs/2603.14614)
...
TL;DR: The paper investigates the nature of charge density waves (CDWs) in 1T-TiSe₂, challenging the notion of chirality in the CDW phase. Symmetry analysis and transport measurements reveal a ferroaxial symmetry-breaking state rather than a chiral one. Elastoresistivity and calorimetry provide evidence for this centrosymmetric state, which preserves inversion symmetry while breaking vertical mirror symmetry. The findings reconcile conflicting surface observations with bulk symmetry constraints.


<details>
  <summary>Details</summary>
Motivation: The paper addresses a long-standing debate about the symmetry properties of the CDW phase in 1T-TiSe₂, aiming to resolve whether the observed CDW is chiral or whether it is a ferroaxial state.

Method: The authors use symmetry-resolved elastoresistivity and elastocaloric measurements to analyze the symmetry-breaking in the CDW phase.

Result: The results show that the CDW breaks mirror symmetry while preserving inversion symmetry, indicating a ferroaxial state, rather than exhibiting chiral properties.

Conclusion: The proposed chiral CDW is actually a centrosymmetric ferroaxial state, aligning bulk measurements with surface-sensitive experiments.
...
### [237] [Giant anomalous Hall conductivity in frustrated magnet EuCo2Al9](https://arxiv.org/abs/2603.14682)
...
TL;DR: 研究发现一种超大异常霍尔效应（AHE），在错乱的Eu基磁性体中表现出异常霍尔导电性的巨大值和异常霍尔角度，超越传统机制两个数量级。通过多组实验数据验证，意味着这种情况是由不稳定自旋扭曲散射引起的，同时普言和电子的交叉耦合引发巨大的换位分裂，为开发新奇的自旋etrize系统提供了框架。


<details>
  <summary>Details</summary>
Motivation: 随着自旋电子学的发展，异常霍尔效应深入研究对于开发新奇的自旋etrize材料和器件具有重要意义，尤其是通过复杂磁性结构实现超大效应。

Method: 使用了多种实验手段，包括磁阻效应，量子振荡，中子散射和ab initio计算，构建了材料的复杂机制。

Result: 发现了由RKKY间接相互作用引发的不稳定自旋扭曲散射机制，导致了自旋扭曲微粒的形成，是主导量子效应的核心原因。

Conclusion: 该研究为开发新奇自旋电子学材料和器件提供了物理机制，同时也为通过复杂磁性结构实现人工控制的自旋效应提供了设计框架。
...
### [238] [Non-Abelian fractional Chern insulators from an exactly solvable two-body model](https://arxiv.org/abs/2603.14722)
...
### [239] [Neural network backflow for ab-initio solid calculations](https://arxiv.org/abs/2603.14775)
*An-Jun Liu,Bryan K. Clark*

Main category: cond-mat.str-el

TL;DR: 神经网络能有效模拟周期性材料，但存在局限性。提出了一种两阶段剪枝策略，扩展了神经网络势场方法，使其在固体态材料中的应用更加高效和精确。该方法在1D氢链、2D石墨体和3D二氧化硅等领域表现优异，尤其是在强关联断裂的活性环境中表现突出。


<details>
  <summary>Details</summary>
Motivation: 随着计算资源的不断扩展，高效、精确模拟固体态材料变得愈发重要，尤其是周期性系统的扩展和复杂性增加，这方面的研究仍需进一步探索。

Method: 提出了两阶段剪枝策略，结合了物理约束的重要性指标和精确的神经网络势场评估，以优化资源分配，提升整体计算效率。

Result: 在多个基准测试中，该方法在1D氢链、2D石墨体和3D二氧化硅等方面的性能均优于现有 competitors，尤其是在强关联断裂场景中表现出更好的鲁棒性。

Conclusion: 该研究展示了神经网络势场方法在固体态材料模拟中的潜力，特别是在处理大规模系统方面，但仍有优化空间。
...
### [240] [Study of the triangular-lattice Hubbard model with constrained-path quantum Monte Carlo](https://arxiv.org/abs/2603.14808)
...
### [241] [Lee-Yang Zeros and Pseudocritical Drift in J-Q Néel-VBS Transitions](https://arxiv.org/abs/2603.15116)
*Chunhao Guo,Zhe Wang,Danhe Wang,Zenan Liu,Haiyuan Zou,Zheng Yan*

Main category: cond-mat.str-el

TL;DR: 研究利用随机系列展开量子蒙特卡洛方法分析平方晶格J-Q模型中Néel反铁磁体与柱状瓦尔ken布固相之间的量子相变，发现有体系尺寸与Lee-Yang零点的缩放关系，支持弱一阶相变解释。


<details>
  <summary>Details</summary>
Motivation: 量子相变的研究极具前沿性，尤其在材料科学及量子计算领域有重要应用价值。本研究提供了一个有效的分析工具，弥补了平方晶格量子Heisenberg模型缺乏明确相变结论的空白。

Method: 研究应用了随机系列展开量子蒙特卡洛方法，通过引入复源场来计算Lee-Yang零点的缩放行为。该方法克服了带有相位问题的量子模型分析难题，尤其适用于SO(5)对称性相变的研究。

Result: 研究结果表明，在J-Q模型相变中，Lee-Yang零点的缩放行为显示出系统的弱一阶特征。零点的间距与系统尺寸呈现反时空体积标度关系，这符合弱一阶相变的理论预期。

Conclusion: 该研究为理解与检测相变类型提供了新方法。其结果暗示着通过有限体积零点缩放可区分相变阶数，具有潜在的普遍性和重要性。该发现为未来研究相变动力学及材料新相态的性质提供了理论依据。
...
### [242] [Nanoscale electronic variations in altermagnetic $α$-MnTe](https://arxiv.org/abs/2603.15225)
*Zeyu Ma,Yidi Wang,Gal Tuvia,Kevin Hauser,Jiaqiang Yan,Jennifer E. Hoffman*

Main category: cond-mat.str-el

TL;DR: $α$-MnTe exhibits significant electronic non-uniformity, as shown by mapping the local electronic landscape using low-temperature scanning tunneling microscopy and spectroscopy. Two distinct regions were identified, with region A showing chemical potential near the valence-band edge and an incommensurate charge modulation. This finding highlights the importance of nanoscale characterization for the reliable use of α-MnTe in electronic applications.


<details>
  <summary>Details</summary>
Motivation: Understanding the electronic properties of novel materials like α-MnTe is crucial for applications in spintronics and other advanced technologies. The inconsistency in reported bulk signatures calls for a deeper understanding of the material's electronic structure at the nanoscale.

Method: We used low-temperature scanning tunneling microscopy and spectroscopy to map the local electronic landscape of α-MnTe on atomically flat cleaved single crystals. The method allowed us to resolve two distinct electronic regions with different chemical potential distributions and detect an incommensurate charge modulation in region A.

Result: The study reveals that α-MnTe can exhibit significant electronic non-uniformity, with incommensurate charge modulation observed exclusively in region A. This suggests that nanoscale characterization is essential for the reliable use of α-MnTe in electronic applications.

Conclusion: The findings of this work demonstrate that α-MnTe's electronic properties are highly non-uniform at the nanoscale, and further studies are needed to understand and harness this behavior for practical applications.
...
### [243] [Motivic GUT Part I: Grand Unified Theory of Topological Order](https://arxiv.org/abs/2603.15361)
*Masahiko G. Yamada*

Main category: cond-mat.str-el

TL;DR: The paper lays the foundation for a unified framework of gapped topological phases across dimensions, using advanced categorical methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a higher-categorical framework to better classify gapped topological orders, potentially leading to a Copernican perspective in the field.

Method: Uses Grothendieck-Kitaev-Lurie motivic yoga and advanced higher-categorical structures.

Result: A unified definition in terms of unitary fusion (∞,d)-categorical data for gapped topological order, reducing to unitary MTCs in 2D.

Conclusion: This suggests categorifying previous classifications and a need for ∞-categorical objects in future studies
...
### [244] [Single-Crystal Growth and Magnetic, Electronic Properties of the FCC Antiferromagnet Ba_2CoMoO_6](https://arxiv.org/abs/2603.15485)
*A. R. N. Hanna,M. M. Ferreira-Carvalho,S. H. Chen,C. F. Chang,C. Y. Kuo,A. T. M. N. Islam,R. Feyerherm,L. H. Tjeng,B. Lake*

Main category: cond-mat.str-el

TL;DR: 研究Ba₂CoMoO₆的结构、磁性和电子性质，发现其具有面心立方晶格，抗磁性和磁动力学特性，显示出强烈的自旋-轨道相互作用，为自旋tronics和能量转换技术提供潜在的材料基础。


<details>
  <summary>Details</summary>
Motivation: 了解双垂Take石材料的磁性机制对其应用前景具有重要意义。

Method: 通过浮法走线和Czochralski生长晶体，结合X射线衍射、磁测、热容分析、熵分析、XAS和表面光伏光谱等手段进行研究。

Result: Ba₂CoMoO₆具有抗磁性，磁变临界温度在20.1 K。自旋-轨道耦合导致能隙约为2.65 T。XAS表明Co²+的自旋-轨道分裂和晶场分裂。表面光伏光谱显示强烈光学响应，特征吸收峰位于2.65 eV。这些发现丰富了面心立方结构抗磁材料的自旋-轨道相互作用理论，并为材料在自旋tronics和能量转换中的应用提供了支持。

Conclusion: Ba₂CoMoO₆材料的研究为理解具有强烈自旋-轨道相互作用的面心立方结构抗磁体的磁性特征提供了新的数据，并为开发自旋tronics和高效的能量转换设备提供了潜在的技术基础。
...
### [245] [Pseudogapped Fermi liquids from emergent quasiparticles](https://arxiv.org/abs/2603.15560)
...
### [246] [Lagrangian Identity and Mass Evolution of Particle-like Objects in Nonminimally Coupled Gravity](https://arxiv.org/abs/2603.13408)
...
### [247] [A new approach to the calculation of extreme-mass-ratio inspirals with a spinning secondary](https://arxiv.org/abs/2603.13482)
*Viktor Skoupý*

Main category: gr-qc

TL;DR: 这个研究提供了一个实用的框架来计算具有显著角动量和轨道倾角的极端质量比-inspiral（EMRI）的引力波辐射，精确到第一个半一致极限阶，利用了Kerr空间中自旋体的轨迹分析和渐近辐射率的简化计算。


<details>
  <summary>Details</summary>
Motivation: 未来的空间基波形探测器（如LISA）依赖于高精度的引力波模式来识别和分析极值质量比合并系统。为了最大化科学潜力，需要对具有显著自旋的偏心且非赤道轨道的EMRI系统进行高精度建模。

Method: 研究采用了Kerr外空域中自旋体的轨迹分析和渐近辐射率的简化计算，结合了最近发现的解析解，以及使用了渐近的Teukolsky振幅和现有的纯测地函数来表达运动常数（如Carter-Rüdiger常数）的变化率。

Result: 结果表明，该框架在近于球形的合并轨道中表现良好，计算出的自旋引起的相位偏移是不依赖于参考系的。

Conclusion: 该框架为高精度计算EMRI的引力波模式提供了一种有效的方法，有助于利用未来引力波探测器的最大科学潜力。
...
### [248] [Topological Phase Transitions in Superfluids Near Black Hole Horizons](https://arxiv.org/abs/2603.13544)
*Cristian R. Ghezzi,Paulo S. Custodio*

Main category: gr-qc

TL;DR: 研究了黑洞与超流体碰撞引发的拓扑相变


<details>
  <summary>Details</summary>
Motivation: 结合宇宙学背景研究创新

Method: 将二维XY模型扩展到弯曲时空

Result: 黑洞邻近视界和事件视界出现互生对

Conclusion: 该研究揭示了黑洞物理与超流体动态的新见解
...
### [249] [Totally geodesic null hypersurfaces and constancy of surface gravity in Finsler spacetimes](https://arxiv.org/abs/2603.13551)
*Ettore Minguzzi*

Main category: gr-qc

TL;DR: 研究了洛伦兹-芬斯勒空间中完全测地的类时超曲面，证明了在某种弱引力方程下，类时超曲面的Ricci 1形式消失，从而扩展了洛伦兹-芬斯勒几何中的重要结果。


<details>
  <summary>Details</summary>
Motivation: 推广洛伦兹几何中的结果，探索Finsler几何在引力中的应用。

Method: 使用了纯洛伦兹几何的方法，减少了洛伦兹-芬斯勒分析，引入了一个技巧。

Result: 在所述条件下，连通紧致完全测地的类时超曲面具有恒定的表面重力，获得了一些拓扑分类结果。

Conclusion: 这项工作为某些特殊Finsler引力方程提供了物理证据，因为它与温度恒定的零定律有关，可以从支配能量条件中推导出这些结果。因此，Ricci 1形式消失被选为主观引力方程。
...
### [250] [One-dimensional subspaces of the $SL(n,\mathbb{R})$ Chiral Equations](https://arxiv.org/abs/2603.13623)
*I. A. Sarmiento-Alvarado,Petra Wiederhold,Tonatiuh Matos*

Main category: gr-qc

TL;DR: The paper discusses solutions of the ($n+2$)-dimensional Einstein Field Equations (EFE) with $n$ commuting Killing vectors in vacuum, providing a method to simplify solving complex differential equations through algebraic means.


<details>
  <summary>Details</summary>
Motivation: Studying EFE solutions is important for understanding gravitational theories and spacetime structures, especially in higher dimensions and with symmetry.

Method: The authors use commuting Killing vectors to reduce the EFE, employing an ansatz where the metric component $g$ depends on a function $ξ$ satisfying a generalized Laplace equation. This converts differential equations into algebraic problems, simplifying solution finding.

Result: The method allows for straightforward construction of exact solutions with desired physical properties, offering a valuable tool in gravitational physics and spacetime studies.

Conclusion: This approach provides a systematic way to find exact solutions of the EFE, facilitating advancements in theoretical physics, particularly in higher-dimensional gravity and engineering solutions with specific symmetries.
...
### [251] [Analytical derivation of long-term dephasing caused by phase transitions in the context of Kerr black holes](https://arxiv.org/abs/2603.13637)
...
### [252] [Multi-Field Dilaton Screening Beyond the Thin-Shell Mechanism](https://arxiv.org/abs/2603.13986)
...
### [253] [Further Results on Null and Force-free Electromagnetic Fields](https://arxiv.org/abs/2603.14046)
*Govind Menon,Rakshak Adhikari*

Main category: gr-qc

TL;DR: 本文探讨了无源费米动力学中叶的分布以及通过叶 foliation 方法来构建叶的方法。


<details>
  <summary>Details</summary>
Motivation: 作为理论物理领域中的一个重要问题，了解无源场的分布及其在不同时空几何中的表现具有重要意义。这也为后续研究提供了明确的方向和目标。

Method:  paper通过一系列的数学建模方法，首先定义了无源场的二维叶，然后探讨了这些叶在不同时空结构中的分布情况，最后通过具体的例子验证了方法的有效性。

Result: 研究发现了无源Gravitational场可以以不同的形式存在，并且这些形式可以通过适当的叶 foliation 来描述。研究还给出了具体的参数化方法，用于构造这些叶。此外，纸还给出了几个具体的例子来说明这些方法的具体应用。

Conclusion: 该研究为无源场的 foliation 方法提供了一个系统性的框架，并且展示了该方法在不同时空结构下的应用潜力。这些成果对于理论物理的研究具有重要的意义。
...
### [254] [Born-Infeld AdS Black Holes Surrounded by Perfect Fluid Dark Matter](https://arxiv.org/abs/2603.14159)
*Behzad Eslam Panah,Bilel Hamil,Manuel E. Rodrigues*

Main category: gr-qc

TL;DR: This paper explores exact charged AdS black hole solutions in Einstein-Λ gravity with Born-Infeld nonlinear electrodynamics and Perfect Fluid Dark Matter (PFDM). They analyze how BI and PFDM parameters affect the event horizon, compute conserved and thermodynamic quantities, and verify the first law of thermodynamics. Thermal stability is examined in canonical and extended phase spaces, showing how PFDM and BI parameters influence stability regions. They also construct thermodynamic Efficiency and demonstrate critical behavior using Ehrenfest equations, indicating second-order phase transitions. The study includes heat engine constructions and investigations into the geodesic structure, highlighted by orbital modifications caused by PFDM and weaker BI corrections.


<details>
  <summary>Details</summary>
Motivation: Black hole thermodynamics remains a complex yet fascinating field, prompting extensive research. The interplay of different gravitational theories, field theories, and modifications to Einstein's equations, coupled with modern concepts like dark matter, presents both challenges and opportunities for exploration. This paper aims to contribute to understanding thermodynamic properties, stability, and applications of black holes in diverse contexts, which are not only theoretically important but also potentially impactful in astrophysics and cosmology.

Method: Exact solutions in Einstein-Λ gravity are sought with Born-Infeld nonlinear electrodynamics and Perfect Fluid Dark Matter. Thermodynamic and stability analyses are conducted, computational methods for conserved quantities and thermodynamic quantities are applied, heat capacity and Helmholtz free energy are utilized for stability assessment, and extended phase space approaches treat Λ as thermodynamic pressure. Analytical verification of the first law, computation of Efficiency, Ehrenfest equations are demonstrated, and thermodynamic behavior of black holes with NI fields and fluids is explored.

Result: The paper successfully obtains exact charged AdS black hole solutions incorporating PFDM and BI effects, thoroughly analyzes the influence of these parameters on black hole thermodynamics, stability, and Efficiency. The first law is verified, both canonical and extended phase space thermal stabilities are assessed, and critical Behavior is demonstrated. Analytical methods confirm the Ehrenfest equations and phase transitions are second order. Heat engines are constructed, Efficiency dependencies are determined, and the geodesic structure analysis shows PFDM's stronger impact on orbital stability.

Conclusion: This comprehensive study on charged AdS black hole solutions in Einstein-Λ gravity with BI and PFDM effects provides significant insights into their thermodynamic properties, stability, Efficiency, and related phenomena. The findings enhance our understanding of black hole thermodynamics in modified gravitational theories and contribute to practical applications in theoretical physics and astrophysics.
...
### [255] [All-sky Searches for Continuous Gravitational Waves from Isolated Neutron Stars in the Data from the First Part of the Fourth LIGO-Virgo-KAGRA Observing Run](https://arxiv.org/abs/2603.14168)
...
### [256] [CETOmega: The Causal-Informational Completion of Gravity](https://arxiv.org/abs/2603.14200)
*Christian Balfagon*

Main category: gr-qc

TL;DR: CETOmega is a unified framework that integrates gravity and quantum mechanics in a four-dimensional, nonlocal, and causal model. It uses a discrete causal network to derive an analytic kernel that manages gravitational and scalar propagations. A scalar field named 'texon' explains dark matter and dark energy without extra degrees of freedom or breaking locality. The formalism ensures analyticity, spectral positivity, and holographic completeness via a Stieltjes representation. Cosmologically, it aligns with Planck observations and predicts stable inflationary dynamics. For astrophysical black holes, causal corrections are controlled and negligible within a specific scale. CETOmega offers a complete causal and informational foundation for spacetime, extending general relativity's validity into quantum and cosmological realms.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to reconcile two fundamental theories in physics—general relativity and quantum mechanics—by introducing a new framework that operates within a strictly four-dimensional, nonlocal, and causal context.

Method: The method involves developing a unified framework called CETOmega, which utilizes a discrete causal network to derive a kernel for gravitational and scalar sector propagations. A scalar field known as 'texon' emerges as an effective excitation of causal connectivity, responsible for dark matter and dark energy.

Result: The result of this paper is the introduction of CETOmega, a framework that successfully unites gravity and quantum mechanics. Key findings include the derivation of a kernel ensuring analyticity, spectral positivity, and holographic completeness, as well as the explanation of dark matter and dark energy via the 'texon' scalar field. The framework also predicts stable inflationary dynamics consistent with observations and manageable corrections for black holes.

Conclusion: In conclusion, this paper presents CETOmega as a groundbreaking framework that unites gravity and quantum mechanics within a causal, four-dimensional model. By incorporating a discrete causal network, it derives a kernel that manages both gravitational and scalar propagations, with a scalar field 'texon' accounting for dark matter and dark energy. The framework's theoretical underpinnings and predictions offer a comprehensive solution to long-standing problems in theoretical physics.
...
### [257] [Isomorphism between the local Poincare generalized translations group and the group of spacetime transformations (x LB1)4](https://arxiv.org/abs/2603.14364)
*Alcides Garat*

Main category: gr-qc

TL;DR: 论文探讨了洛伦兹子群（Poincare subgroup）与LB1群之间的直接关系，该群由SO(1,1)和两种离散变换组成。文中还涉及了平移在规范理论中的广泛研究，并试图将此推广至局部平移，特别研究了 Bondi-Metzner-Sachs subgroup 的超平移情形。论文中引入一套微分方程组和各种场（如标量、矢量、自旋or、引力场等），这些场构成了用于构建一种新型局部四面体的结构，从而实现了上述理论的证明。已有与类似但非完全相同的四面体结构的成果对本文的构建和论证有重要作用。该论文表明平移及其推广至与LB1群同构的张量积并非平凡结果。


<details>
  <summary>Details</summary>
Motivation: 该论文探讨了洛伦兹群与标架变换之间的关系，以及将平移推广至局部变换的可能性，这在相对论和规范理论中具有重要意义。

Method: 本文使用了微分方程组和多类场的组合，构建了一种新型的局部四面体，从而证明了Poincare subgroup与LB1群之间的关系。此外，文中还提到了已有相关研究对其方法有辅助作用。

Result: 论文成功构造了局部四面体，证明了Poincare subgroup与LB1群的同构关系，并推广了平移的概念，探讨了其在局域情况下的表现。

Conclusion: 本文得出结论，洛伦兹群和特定标架变换群之间存在直接关联，并成功将平移理论推广至局域情况，为后续研究提供了新的理论基础。
...
### [258] [Periodic orbits and gravitational waveforms of black holes in bumblebee gravity](https://arxiv.org/abs/2603.14413)
*Zijian Shi,Xiangdong Zhang,Yunlong Liu*

Main category: gr-qc

TL;DR: This paper investigates the dynamics of massive particles and gravitational waves in the spacetime of charged and uncharged black holes under Einstein-Bumblebee gravity, highlighting the role of a coupling parameter $l$ and electric charge $Q$ in influencing bound orbits and gravitational waveforms.


<details>
  <summary>Details</summary>
Motivation: The potential for bumblebee gravity to leave imprints in gravitational waves is intriguing.

Method: Geodesic equations and effective potential analysis are employed to study bound orbits, while periodic orbits are classified using whirl, zoom, and vertex taxonomy. Gravitational waveforms are computed using the quadrupole formula.

Result: The analysis reveals that the Lorentz-violating parameter $l$ and electric charge $Q$ significantly influence the confinement of bound orbits and the properties of gravitational waveforms. In the uncharged case, the properties of periodic orbits differ qualitatively from the Schwarzschild case, even though static potential properties are degenerate. The calculated waveforms demonstrate that $l$ and $Q$ introduce distinct phase-shifting effects, suggesting measurable imprints of bumblebee gravity in future space-based gravitational wave observations.

Conclusion: Bumblebee gravity may produce detectable effects in gravitational-wave signals, offering a potential observational signature for this theory.
...
### [259] [Cosmic anisotropic hair of nonlocal RT gravity](https://arxiv.org/abs/2603.14488)
*Jiajun Zhou,Shuxun Tian,Zong-Hong Zhu*

Main category: gr-qc

TL;DR: 该论文研究了非局部RT引力下各向异性比-1型宇宙的动态演化，并发现随着时间推移，宇宙中的各向异性反而在增长，这与非局部RT引力理论中没有毛发定理相矛盾。


<details>
  <summary>Details</summary>
Motivation: 该研究可能为理解宇宙加速膨胀和引力约束提供了新的视角，同时也可能对非局部RT引力理论的完善有所帮助。

Method: 论文采用了动态系统分析和相空间分析的方法，引入了六个无量纲变量来构建模型。

Result: 研究揭示了，在非局部RT引力框架下，宇宙的各向异性随着时间增加而增强。这与不带毛发定理在非局部RT引力中的传统结论相矛盾。

Conclusion: 非局部RT引力理论中的各向异性增长与不带毛发定理相矛盾，这可能表明理论需要进一步修改或某个关键假设不正确。该研究提出了需要进一步探索的问题，可能会对引力理论和宇宙学的发展产生重要影响。
...
### [260] [Scalaron excitation by topological vortices in quadratic $f(R)$ gravity on a BTZ black hole background](https://arxiv.org/abs/2603.14580)
*C. A. S. Almeida,F. C. E. Lima*

Main category: gr-qc

TL;DR: 研究三维反德西特不变理论如何通过二次f(R)引力激活引力标量激发的具体机制。


<details>
  <summary>Details</summary>
Motivation: 探索次弦理论和高曲率修正对三维反德西特不变理论的影响，特别是在包括爱德华·威滕在内的理论物理学家的研究背景下。

Method: 使用二次f(R)理论框架，分析三维反德西特不变度量的BTZ黑洞背景中由局部化Maxwell-Higgs漩涡激发的引力标量激发的传播和行为。

Result: 计算得出，在旋涡源的静力BTZ黑洞中，通过二次f(R)重力理论，标量excitation的的行为和传播特征得以量化和分析，由此得到根据施加的源的结构，带有离散谱参数的Green函数得以构造，同时发现标量excitation表现出随着距离增加而渐近衰减更快的那种特性。

Conclusion: 研究结果为理解三维理论下的更高曲率修正对引力标量激发的机制提供了一个明确的实例如何正确量子化引力标量激发的理论框架，同时支持了爱德华·威滕关于高阶层理论如何在较低维系统中包括本地引力自由度的猜想。
...
### [261] [Can wormhole spacetimes in Unimodular Gravity be supported by ordinary matter? A general proof of the exotic matter requirement](https://arxiv.org/abs/2603.14718)
...
### [262] [Dark Matter Induced Scalarization as a Possible Solution to the Hyperon Puzzle](https://arxiv.org/abs/2603.14731)
*Suchana Adhikari,Teruaki Suyama*

Main category: gr-qc

TL;DR: 研究中，将非极小量标量场与里奇标量耦合，导致中子星内部出现-ifan状态，推导出现丰富褪变现象。最大质量超过2 $M_	ext{sun}$，并且考虑到超子存在（可减轻压力、降低最大质量）。当非极小量耦合强度增大时，导致多个解出现。并分析了有效势结构。此外，加入标量自相互作用项后，中子星特性发生变化。


<details>
  <summary>Details</summary>
Motivation: 爱因斯坦广义相对论作为描述引力的框架在现代天体物理和宇宙学中具有重要意义。研究中试图找到更丰富的物理现象以及更强大的数学模型，以更好地理解中子星等极端天体。

Method: 在爱因斯坦引力理论中引入标量场，非极小量与里奇标量分开，形成独特的场方程。然后通过对条件求解方程获得中子星配置。

Result: 发现了一些新的现象，例如最大质量超过2 $M_	ext{sun}$(甚至加上超子的情况下)。对于更高的耦合强度，方程组得到多重解。有效势以及其他特性也得到了分析。

Conclusion: 引入了非极小量与里奇标量的标量场摆脱中子星形成新的解和丰富的弧线。在考虑标量相互作用时，中子星特性发生变化，这为未来研究提供新的方向和途径。
...
### [263] [Logarithmically enhanced hyperbolic square-root deformation of Starobinsky inflation](https://arxiv.org/abs/2603.14743)
*Andrei Galiautdinov*

Main category: gr-qc

TL;DR: 研究提出了一种改进的双曲平方根（HSQRT）形变，应用于Starobinsky模型的$f(R)$引力中。该研究通过对微分结构的形变，解决了负曲率空间的强烈相互作用奇点问题，同时保留了大正曲率区域的指数缓慢滚动特性。研究者基于最近的天体物理学观测数据，提出了一个对数增强的结构极小化修正，保持了全局自由度和量子稳定性。革新后的模型在爱因斯坦框架中被精确解析，显示出标量势从指数型渐近行为平滑过渡到逆幂渐近行为。研究者推导了精确的解析表达式，计算了慢滚观测指标，并成功将谱指标引入了观测推荐的范围。结果表明，该模型预测了一个显著的小运行值和一个可以被下一阶段 cosmic microwave background观测所探测的可控制目标。


<details>
  <summary>Details</summary>
Motivation: 近年来的天体物理学观测数据（如ACT DR6和DESI）表明，标量谱指数出现上移，并且倾向于偏离纯指数缓慢滚动行为。因此，研究者提出了一个结构极小化的对数增强方案，以适应这些新的观测数据。

Method: 研究者通过引入对数项到双曲平方根形变中，并对爱因斯坦框架进行精确解析，开发出一个精确的参数化公式。这允许研究者详细分析标量势在不同曲率区域的行为，并推导出慢滚观测指标的解析表达式。

Result: 研究结果表明，模型的谱指数直接进入最新的观测推荐范围（$n_s $970	ext{--}0.975$），同时模型预测了一个显著小的运行值，即$	ext{α}_s $-0.00060到-0.00042，这些结果为下一阶段的 cosmic microwave background 观测提供了一个明确的参数控制目标。

Conclusion: 该研究通过改进双曲平方根形变，成功结合了Starobinsky模型的指数缓慢滚动行为和量子稳定性，并通过新的观测数据测试，验证了模型的有效性。未来研究可以进一步探索更多参数和修正项，以更精确地描述宇宙的早期演化。
...
### [264] [Cosmological angular momentum from quantum rotation](https://arxiv.org/abs/2603.14814)
*Bo-Qiang Lu*

Main category: gr-qc

TL;DR:  paper proposes a new mechanism generating cosmic angular momentum from quantum fluctuations during inflation through a spectator scalar field, leading to primordial black holes with significant spin detectable via gravitational waves.


<details>
  <summary>Details</summary>
Motivation:  understanding the origin of cosmic angular momentum is fundamental in structure formation and relates to theories of cosmic evolution and black hole physics.

Method:  the study uses quantum field theory during inflation, introduces a spectator scalar field with U(1) symmetry, and explores nonspherical gravitational collapse effects to transfer momentum.

Result:  the paper shows that primordial black holes can have dimensionless spins as high as χ∼0.1-1, significantly exceeding predictions from tidal torque theory.

Conclusion:  this mechanism provides a testable connection between inflation, primordial perturbations, and the distribution of black hole spins, offering new insights for gravitational wave observations.
...
### [265] [Spectral Bifurcations in Quasinormal Modes of Regular BTZ Black Holes](https://arxiv.org/abs/2603.14872)
*Kartheek Hegde,Tajron Jurić,A. Naveena Kumara*

Main category: gr-qc

TL;DR: 研究了正则施瓦茨赤层黑洞关于洛ewick修正的准测地能量子数谱的标量场传播，使用了莱弗的连分式方法和霍罗威兹-hardware级数方法，发现两者一致，并讨论了正则化保持线性稳定性的同时，如何重新排列了量子数谱


<details>
  <summary>Details</summary>
Motivation: 探讨正则施瓦茨赤层黑洞关于洛ewick修正的量子数谱是理论物理中的重要课题，为理解正则黑洞的稳定性及其 late-time 行为提供了新视角

Method: 主要采用了连分式和级数展开方法，比较了两种不同方法的优劣，验证了结果的一致性

Result: 正则化施瓦茨赤层黑洞在参数空间内保持线性稳定性，同时通过调节修正参数，导致了量子数谱的质变，如根与虚轴的碰撞以及分立的纯虚数根的有序产生

Conclusion: 正则施瓦茨赤层黑洞提供了研究三维准测地爱因斯坦空洞几何下量子数谱行为的精确平台，为探索正则性条件如何影响量子数谱及黑洞最终振荡状态提供了重要理论依据
...
### [266] [Parametrizing superfluid dark matter with rational approximations](https://arxiv.org/abs/2603.15053)
*Francesco Lottatori*

Main category: gr-qc

TL;DR: 研究发现，标量背景场可以改变超流暗物质中的声子传播，通过调整相互作用常数和场配置，可以使暗物质表现出从刚性到尘埃状的行为，这对于理解暗物质结构和形态有重要意义。


<details>
  <summary>Details</summary>
Motivation: 暗物质作为一个未直接观测到的物质，其性质和行为一直是天体物理和粒子物理中的重要研究课题。通过研究标量背景场对声子传播的影响，可以帮助我们更深入地理解超流暗物质的宏观行为和可能的结构。

Method: 本研究通过构建一个简单的一维模型，引入标量场的有理径向分布（Padé profiles），计算了不同相互作用常数下的声子传播速度和质量关系，得出了声子速度随空间分布的变化规律。

Result: 主要结果表明，标量场的调制导致了暗物质的有效质量和传播速度发生变化。当相互作用常数较大时，暗物质的行为趋近于尘埃状态，声子传播速度显著下降。

Conclusion: 这些结果为超流暗物质的结构和动力学提供了新的理论框架，同时也为未来通过观察暗物质分布来探测超流性质提供了参考方向。
...
### [267] [The peculiar case of the Viaggiu holographic dark energy](https://arxiv.org/abs/2603.15067)
*Somnath Saha,Subhajit Saha,Nilanjana Mahata*

Main category: gr-qc

TL;DR: This paper examines a holographic dark energy (HDE) model using a generalized entropy form proposed by Viaggiu in 2014, studying the evolution of the universe with dark matter and HDE within different horizon frameworks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore a new HDE model and compare its behavior with existing models, potentially leading to new insights into the universe's fate.

Method: This is a theoretical study that mathematically derives the evolution equations using horizon entropy, analyzing various scenarios with different horizons.

Result: The study uncovers a possible cosmic doomsday scenario in the future when the universe might collapse due to accelerated expansion. Some findings differ significantly from other HDE models.

Conclusion: The research suggests that the generalized entropy form can lead to distinct cosmological outcomes compared to conventional HDE models.
...
### [268] [Viaggiu holographic dark energy in light of DESI DR2](https://arxiv.org/abs/2603.15178)
*Amlan K. Halder,Andronikos Paliathanasis,Stefano Viaggiu,Abdulla Al Mamon,Subhajit Saha*

Main category: gr-qc

TL;DR: The study examines the cosmological viability of the VHDE model using various late-time observational datasets, including Type Ia supernovae, cosmic chronometers, and BAO data. The analysis finds that the VHDE model performs similarly to the standard ΛCDM model across all considered dataset combinations. The Hubble constant (H0) value obtained aligns closely with the ΛCDM predictions, with the matter density parameter (Ωm0) constrained to approximately 0.24. The model's additional parameter, δ, is found to have a mean value within the range π/3 δ ∼ 0.27-0.33. Statistical tests using AIC and Bayesian evidence suggest that the two models are statistically indistinguishable, though ΛCDM is slightly preferred in most dataset combinations. However, the study concludes that the VHDE model remains a viable alternative for describing the late-time accelerating expansion of the universe.


<details>
  <summary>Details</summary>
Motivation: Understanding the nature of dark energy and the accelerated expansion of the universe is a fundamental goal in modern cosmology. Consequently, exploring alternative models to explain this phenomenon, such as the holographic dark energy model, is crucial for expanding our knowledge of the universe’s evolutionary history.

Method: The research employs a comprehensive analysis using a range of late-time cosmological datasets. The authors utilize the following methods: 1) Applying Type Ia Supernovae from multiple catalogues, 2) Incorporating Cosmic Chronometers for measuring Hubble expansion, 3) Using Baryon Acoustic Oscillations from the DESI DR2 data to constrain cosmological parameters, and 4) Implementing AIC and Bayesian evidence to compare the VHDE model with the ΛCDM scenario. This rigorous approach ensures robust parameter estimation and model comparison.

Result: The findings indicate that the holographic dark energy model proposed by Viaggiu (VHDE) exhibits excellent cosmological viability when tested against various late-time observational datasets. The model’s free parameters are effectively constrained, and its performance is on par with the standard cosmological model. Specifically, the Hubble constant (H0) derived from the analysis is consistent with ΛCDM predictions, and the matter density (Ωm0) is limited to approximately 0.24, which is marginally lower than ΛCDM estimates. The additional parameter δ of the VHDE model is found to have a mean value within the range π/3 δ ∼ 0.27-0.33. Additionally, statistical tests (AIC and Bayesian evidence) reveal that both models are statistically indistinguishable in their explanatory power for the observed data. However, slight preferences for ΛCDM are evident in most dataset combinations.

Conclusion: The main conclusion of this research is that the holographic dark energy model (VHDE) offers a plausible and viable explanation for the late-time accelerating expansion of the universe. Through extensive analysis of multiple datasets, the model’s predictions effectively align with observations, and it remains a feasible alternative to the standard cosmological model. Nevertheless, the preference for ΛCDM suggests areas where future research could further refine our understanding of dark energy and improve model discrimination.
...
### [269] [Plasma impact on black hole shadow and gravitational weak lensing for Schwarzschild-like black hole](https://arxiv.org/abs/2603.15241)
*Weiqiang Yang,Mirzabek Alloqulov,Ahmadjon Abdujabbarov,Bobomurat Ahmedov,Chengxun Yuan,Chen Zhou*

Main category: gr-qc

TL;DR: 研究了萨 culd 像黑洞的观测性质，发现光动力学、阴影大小和透镜效应都受到等离子体影响。


<details>
  <summary>Details</summary>
Motivation: 探索等离子体环境对黑洞观测性质的影响，尤其是在萨 culd 像黑洞中。

Method: 使用哈密顿形式研究了光动力学，分析了阴影大小和透镜效应。

Result: 等离子体频率增加导致光球半径减小，阴影半径减小。透镜效应和放大效应随等离子体参数增加而增强。

Conclusion: 等离子体环境显著影响了萨 culd 像黑洞的观测性质，尤其是在光动力学、阴影和透镜效应方面。提供了解释实验结果的理论框架。
...
### [270] [Approximate Models for Gravitational Memory](https://arxiv.org/abs/2603.15442)
*Q-L Zhao,P. -M. Zhang,M. Elbistan,P. A. Horvathy*

Main category: gr-qc

TL;DR: 这个摘要讨论了通过Carroll对称性构造的长距离引力波 sandwich模型，使用Pöschl-Teller势点近似了引力波中的粒子运动，并利用Stern-Liouville方程的第二解进行了分析。类似的结果也适用于高斯势函数。


<details>
  <summary>Details</summary>
Motivation: 引力波的研究在现代物理中非常重要，尤其是在引力波天文学的发展中，精准地理解引力波中的粒子运动对于后续的研究具有重要意义。

Method: 该摘要采用了Carroll对称性作为基础，结合了Pöschl-Teller势点和Stern-Liouville方程的第二解来分析引力波中的粒子运动。

Result: 研究结果表明，这种 sandwich 模型在长距离范围内具有良好的解析近似，并且结果适用于Pöschl-TellerPot和高斯势函数。

Conclusion: 通过这种 Sandwich 模型，可以更准确地描述引力波中的粒子运动，为后续引力波天文学的研究提供了有用的方法。
...
### [271] [Direct Waves in Black-Hole Binary Mergers: Insights from the Backwards One Body Model](https://arxiv.org/abs/2603.15474)
*Anuj Kankani,Sean T. McWilliams*

Main category: gr-qc

TL;DR: BOB 模型能准确描述黑洞双星合并和回环辐射, BOBParametersModifications能捕获非站定振荡模式。


<details>
  <summary>Details</summary>
Motivation: 研究黑洞双星合并和事件视界 telescope望远镜附近区域的辐射特性, 特别是对 prompt emission和回环辐射的解释。

Method: 用 Backwards One Body (BOB) 方法建模合并辐射, BOBParametersModifications识别非驻定振荡波段; 利用 Pöschl--Teller势恢复 BOB 振幅演化, 并用有理滤波器分离 BOB 和数值相对论波形中的非驻定振荡内容。

Result: BOB 模型能高精度捕捉回环辐射中的直接波段, BOB频率在事件视界随着轨道角动量增加而增加, 但直接波频率与黑洞 Event Horizon 的频率在高自旋情况下显著不同, 更接近峰值时刻的 news 波动频率。

Conclusion: 研究证实了 BOB 模型在描述黑洞双星合并和回环辐射中的直接波段的准确性, 并展示了直接波频率与黑洞视界频率的关系。
