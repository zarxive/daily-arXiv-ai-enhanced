...
### [1] [A matter-wave Fabry-Pérot cavity in the ultrastrong driving regime](https://arxiv.org/abs/2601.21122)
...
### [2] [Generating persistent-current superpositions in Bose-Einstein condensates using dynamic optical potentials](https://arxiv.org/abs/2601.21144)
*Renzo Testa,Donatella Cassettari*

Main category: cond-mat.quant-gas

TL;DR: 该论文提出了一种基于时间依赖光场的方法来精确操控超冷原子的动量状态，展示了在环形 traps 中 engineer 稳定的电流叠加态，并通过实验验证了该方法的高效性。


<details>
  <summary>Details</summary>
Motivation: 精确操控超冷原子的动量状态对于量子传感和量子计算等领域具有重要意义。该论文通过引入时间依赖的光场，提供了一种简单且高效的实验方法。

Method: 论文介绍了一种基于时间依赖光场的方法，通过调控光场参数来调节原子的动量状态。该方法利用现有技术中的光 sculpting 实现。

Result: 通过数值模拟和实验研究，他们成功实现了环形 traps 中的动量叠加态，并验证了该状态的高保真度。此外，他们还分析了自相互作用对状态演化的影响，并提出了一个两态分析模型。

Conclusion: 该研究为超冷原子的动量状态工程提供了一种实用的有效方法，并为未来研究量子系统提供了新思路。
...
### [3] [Bound-state-free Förster resonant shielding of strongly dipolar ultracold molecules](https://arxiv.org/abs/2601.21928)
*Reuben R. W. Wang*

Main category: cond-mat.quant-gas

TL;DR: 该论文提出了一种利用直流和微波电场抑制强极性旋转激发超冷分子碰撞损失的方法。通过调整两个激发态分子旋转态的直流场使它们发生Forster共振，同时利用微波场使分子间发生跃迁，从而消除所有长期束缚态，在接近零的两体和三体碰撞损失通道中实现了几乎完全的抑制。该方法允许调制的偶极和反偶极相互作用，且不依赖于改变光的碰撞（这是两原子碰撞的主要损失来源），并实现了分子玻色-爱因斯坦凝聚（BEC）。使用钠铯（NaCs）作为例子，数值计算表明该方法可以在100纳开尔文下实现弹性到损失率比值约为10^6。该工作为实现大量长寿命的强相互作用分子气体样本提供了新的途径。


<details>
  <summary>Details</summary>
Motivation: 捕获超冷分子在高温下接近零碰撞损失的可能性被视为一种重要手段，特别是在 condensed-matter 哲学、量子统计物理、量子信息处理等方面。该研究旨在开发新的技巧以实现这种状态。

Method: 通过使用混合的直流和微波电场来调节分子状态。具体来说，利用直流电场将两激发态旋转态调制为Forster共振，然后通过微波电场使分子间发生能级跃迁，消除长期束缚态并抑制碰撞损失。

Result: 计算表明，在100纳开尔文温度下，弹性到损失的比率可以达到约10^6，这表明该方法在 crushing loss 领域表现出色。

Conclusion: 该研究提供了一种全新方法来实现禁锢态自由的弹性碰撞抑制，为超冷分子气体研究开辟了新的可能性。
...
### [4] [Two-shot learning of multiple strange attractors](https://arxiv.org/abs/2601.21117)
*Daniel Köglmayr,Miralem Spahic,Andrew Flynn,Christoph Räth*

Main category: nlin.CD

TL;DR: The paper introduces a machine learning technique combining NGRC and ERT to process, store, and recall multiple strange attractors, achieving accurate reconstruction through a two-shot learning approach that filters unnecessary features and avoids extensive hyperparameter optimization. It successfully reconstructs short- and long-term dynamics of chaotic attractors by introducing feature space separation and identifies that short-term memory defects can lead to long-term memory failures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a machine learning system that can effectively handle both short- and long-term memory tasks, inspired by biological memory mechanisms and recent advancements in learning techniques.

Method: The method involves integrating a next-generation reservoir computer (NGRC) with extremely randomized trees (ERT) to process multiple strange attractors. A two-shot learning approach is used, which filters out unnecessary features and avoids extensive hyperparameter optimization.

Result: The results show that the NGRC+ERT system can accurately reconstruct the short- and long-term dynamics of both the Lorenz and Halvorsen chaotic attractors when using an exponential filtering scheme. The system is also validated for reconstructing multiple pairs of attractors and up to 16 different attractors. Feature space separation is shown to suppress unwanted switching dynamics, stabilizing long-term memory recall. Additionally, it is found that defects in short-term memory processing can cause failure modes in long-term memory recall, leading to confabulation.

Conclusion: The paper demonstrates a successful integration of NGRC and ERT for multi-attractor processing and storage, highlighting the importance of feature space separation and the consequences of short-term memory defects on long-term recall.
...
### [5] [From Basins to safe sets: a machine learning perspective on chaotic dynamics](https://arxiv.org/abs/2601.21510)
...
### [6] [Integrating prior knowledge in equation discovery: Interpretable symmetry-informed neural networks and symbolic regression via characteristic curves](https://arxiv.org/abs/2601.21720)
*Federico J. Gonzalez*
...
### [7] [Cosmological Expansion from Machian Phase Normalization by Horizon Constraints](https://arxiv.org/abs/2601.20889)
*Maurice H. P. M. van Putten*

Main category: gr-qc

TL;DR: 该论文通过马式阶段归一化和Hamilton约束，提出了一种新的宇宙学模型，解释了ΛCDM及其变形行为以及引起的后期宇宙矛盾。


<details>
  <summary>Details</summary>
Motivation: 宇宙学中的矛盾，如宇宙常数问题和暗能量导致的加速膨胀，是近年来研究的热点。该论文尝试通过新的理论框架来解决这些矛盾。

Method: 作者使用了马式阶段归一化和Hamilton约束来重新定义引力路径积分，并提出了新的宇宙模型。

Result: 论文得出Λ=R/6是热平衡条件下的精确解，并说明了模型如何解释暗能量效应以及如何避免新自由度。

Conclusion: 该研究为理解ΛCDM及其变形提供了新的视角，可能为解决后期宇宙矛盾提供新思路。
...
### [8] [Generalized ModMax and the Early Universe](https://arxiv.org/abs/2601.20908)
*M. Sabido,V. Sierra*

Main category: gr-qc

TL;DR: 通过广义ModMax非线性电动力学，我们提出一个不含初值奇点的宇宙模型，并且该模型能够满足当前的Planck数据的不变量$N$,$n_s$和$r$。


<details>
  <summary>Details</summary>
Motivation: 探索不含奇点的宇宙模型与满足当前观测数据的能力。

Method: 基于广义ModMax非线性电动力学，通过调整理论参数，计算和分析宇宙演化。

Result: 模型中通过适当选择参数能消除初始奇点，且拥有与观测相符的早期 inflate阶段。

Conclusion: 广义ModMax是一种有效且可以预测的理论框架，能构建不含初值奇点且具备合理早期inflation的宇宙模型。
...
TL;DR: 研究了4维反德西特AdS黑洞在爱因斯坦引力与非线性幂律电磁场（p=3/4）作用下的动ationally不稳定性与thermodynamic相变之间的关系。在canonical系综中，发现了介电荷临界值Q_c，将相变分成小/大黑洞（SBH/LBH）一级Transitionsex区和单相区，后者只允许热动力学稳定相。在SBH/LBH共存区，对于无质量及有质量探针，温度下的Lyapunov指数λ(T)在Transitions点有有限Discontinuity。随着Q趋向于Q_c，一级Transitions在二级临界点消失。near临界，Lyapunov的突然变化遵循均值场临界指数1/2的普适法则。对于无质量探针，临界影响参数b_c表现出相同的多值性和临界行为。此外，系统在相变点时的热力学特性表现出相同的陡峭特性。结果表明，Lyapunov指数是捕捉黑洞热力学一级相存和二级临界行为的统一的动态探测器。


<details>
  <summary>Details</summary>
Motivation: 理解反德西特AdS黑洞在不同电磁场作用下相变的热力学性质和动ationally行为。研究动ationally稳定性与相变之间的联系有助于深入理解黑洞热力学与量子引力的潜在关系。

Method: 使用热力学分析和数值计算研究爱因斯坦-非线性电磁场的4维AdS黑洞相变和动ationally不稳定性。通过分析相图、Lyapunov指数、热力学量的临界行为以及影响参数的行为来探索相变的类型和动ationally特性的变化。

Result: 发现了一级和二级相变之间的契合点，以及Lyapunov指数作为捕捉相变和动ationally特性的统一工具的作用。

Conclusion: 研究揭示了在4维AdS黑洞中，动ationally不稳定性与相变密切相关，通过Lyapunov指数可以统一描述不同类型的相变。这为理解黑洞热力学和量子引力提供了新的视角。
...
TL;DR: 在五维真空引力中，我们数值证明了从旧施瓦茨child黑洞出发，以有限时间形成一个极值旋转黑洞的存在性解，这反驳了真空引力中第三个定律。


<details>
  <summary>Details</summary>
Motivation: 该研究探索了真空引力中黑洞的形成机制，彰显了经典广义相对论的局限性，为黑洞物理和量子引力研究提供了新视角。

Method: 采用数值模拟的方法，在适当的初始条件下求解五维的爱因斯坦场方程，通过细致的不稳定性分析，证明了所需的解的存在性。

Result: 首先，该研究首次揭示了在纯引力（无物质场）条件下，存在一种机制使施瓦茨child黑洞在有限时间内转化为极值旋转黑洞。其次，我们还发现了一种不依赖于初始黑洞存在的形成这样极值黑洞的新机制。

Conclusion: 这意味着三种定律中的一种——此处指第三个定律——不成立，是经典广义相对论的重要突破，也为量子引力的探索提供了新方向。
...
TL;DR: 这篇论文通过引入新的坐标系和推广Rindler坐标，揭示了加速框架与全局时空曲率之间的独特几何关系，并提出了Spherical Rindler度量，用于研究黑洞和宇宙学度量。


<details>
  <summary>Details</summary>
Motivation: 研究背景与方法：论文的出发点是探索加速框架与时空全局曲率的关系，通过推广Rindler坐标，引入新的坐标系，如紧凑区域的Minkowski时空和推广的Spherical Rindler度量，以分析黑洞和宇宙学解。

Method: 介绍了新型Rindler型坐标系统，包括循环坐标，扩展了Rindler框架，并对近地坐标与Schwarzschild度量进行了分析，以此构造Spherical Rindler度量。运用这个新框架成功得到了黑洞和平行宇宙的解。

Result: 通过Spherical Rindler度量，论文成功地导出了黑洞和平行宇宙的解，揭示了新的几何性质，强化了加速框架与全局时空关系的理解。

Conclusion: 这种方法为研究加速参考系与时空全局结构提供了新的工具，有助于更深入地理解基本物理理论之间的联系，特别是涉及到时空加速与全局曲率时。
...
TL;DR: 研究计算了引力波信号中离振态的偏差，发现低频率动力潮的振荡性态在双子星或utron星黑洞合并中可见。 tidal力在二星系统中引起振荡模式时，通过耗尽能量至振动模式，从而将能量传递给二星系统。这一过程使二星系统的能量耗散，加速了微波斯运动。最终，耗尽了能量使余波继续传播。为了确定该效应的可测性，使用了现成的Current和Planned检测器进行计算，并采用了准分析方法和优化后的标准数值匹配函数。研究结果表明，在高级LVK探测器下，时间前后偏差为1毫秒，能量飞行波占引力波的几百分之一时，该效应是可测量的。与先前的模型不同，他们认为该效应是通过相位变化或周期差异来描述的。实际证实，动力潮引起的效应主要是时间微分变化，而常用在文献中的一频近似显著高估了该效应的可测性。


<details>
  <summary>Details</summary>
Motivation: 引力波探测是当前天体物理研究的重要领域，其中二星系统中动力潮的效应是影响信号探测的因素之一。为了更准确地评估这种效应，需要改进模型和计算方法。

Method: 准解析方法和优化后的数值匹配方法

Result: 在高级探测器下，该效应是可测量的，时间前后偏差为1毫秒

Conclusion: 动力潮主要引起时间微分变化，与先前的相位差异或周期差异模型相比，一频近似显著高估了该效应的可测性。
...
TL;DR: 在强引力源中产生的引力波量子化是广义相对论与量子力学一致性的要求。


<details>
  <summary>Details</summary>
Motivation: 研究者希望探索经典物理（如强引力）与量子物理（如引力波量子化）之间的联系，这有助于理解广义相对论的量子力学相容性。

Method: 研究者通过分析引力波的量子化效应及其与牛顿极限（经典近似）的关系，构造了一个gedanken实验的方法，利用广义相对论和量子力学的理论框架进行推导。

Result: 研究者得出结论：在经典引力理论下（牛顿极限），为了保持广义相对论与量子力学的相容性，引力波必须量子化。 

Conclusion: 通过 gedanken 实验分析，研究者表明在强引力源中的引力波量子化是广义相对论与量子力学一致性的必要推论。
...
TL;DR: 研究发现，标准的标量场理论中无法实现由phantom到quintessence的态相位变化，但通过 ghost condensate（k-essence）理论，可以通过消除 ghost不稳定性并调整参数，实现这一态相位转变。


<details>
  <summary>Details</summary>
Motivation: 探索不同标量场理论中态相位变化的实现条件，有助于理解暗能量演化历史，特别是在早期宇宙中的表现。

Method: 通过详细分析标准标量场理论的局限性，然后研究k-essence理论的特性及其如何避免ghost不稳定性，进而实现所需的态相位转变。利用数学推导和理论分析的方法。

Result: 研究证实，k-essence理论在适当参数配置下，可以通过消除 ghost不稳定性，实现由phantom到quintessence的态相位变化。

Conclusion: k-essence理论为解决phantom到quintessence态相位变化提供了可行的方法，无需过多的fine-tuning，且自然地嵌入Modified Gravity框架中。
...
TL;DR: 这篇论文提供了一个精确的解析解，用于计算自旋物体在引力波作用下的运动偏差，并且可以用于多种与引力波有关的效应研究。


<details>
  <summary>Details</summary>
Motivation: 研究引力波中自旋物体的运动偏差对于理解引力波效应和天体物理学具有重要意义。

Method: 论文采用平行运输四维标架和利用平面引力波的平移守恒量，简化求解马西森-帕帕etz基-狄克方程，获得六个保守量来确定动量、自旋演化和世界线。

Result: 获得了一个闭合形式的积分表达式，描述了引力波信号中自旋-曲率引起的横向和纵向运动，为高精度空间探测器和理论研究提供了统一的模型。

Conclusion: 该解析解为研究自旋依赖的引力波效应，如引力记忆和佩恩极限几何，以及未来的空间探测器提供了强大的工具。
...
### [19] [Leveraging rapid parameter estimates for efficient gravitational-wave Bayesian inference via posterior repartitioning](https://arxiv.org/abs/2601.21630)
*Metha Prathaban,Charlie Hoy,Michael J. Williams*

Main category: gr-qc

TL;DR: 该研究提出了一种快速贝叶斯推断方法，结合前向分段和后分采样技术，显著加速了贝叶斯分析，特别适用于信号信噪比（SNR）大于20的双星系统。


<details>
  <summary>Details</summary>
Motivation: 随着引力波探测器的不断改进，高效的数据分析方法变得愈发重要。为了提升分析效率，开发快速贝叶斯推断技术具有重要意义。

Method: 研究提出了一种结合前向分段（simple-pe algorithm）和后分采样（posterior repartitioning）的快速贝叶斯推断方法。该方法通过引导后采样器聚焦于参数空间中概率最高的区域，以提高效率。

Result: 实验表明，该方法在SNR小于150时能带来2.2倍的加速效果。此外，该方法在保持统计推断严谨性的同时，显著提升了分析效率。

Conclusion: 该方法为贝叶斯分析提供了高效且可靠的工具，特别适用于高SNR的引力波信号分析，有助于降低探测器分析成本。
...
### [20] [Elementary blocks of Loop Quantum Gravity](https://arxiv.org/abs/2601.21644)
...
### [21] [Beyond Kasner Epochs: Ordered Oscillations and Spike Dynamics Inside Black Holes with Higher-Derivative Corrections](https://arxiv.org/abs/2601.21658)
*Mei-Ning Duan,Li Li,Yu-Xuan Li,Fu-Guo Yang*

Main category: gr-qc

TL;DR: 研究显示，在强引力环境中，如接近黑洞的区域，经典的话持纳克动力学被修正，出现了新的动力学行为，如振荡和增长振幅的峰。


<details>
  <summary>Details</summary>
Motivation: 研究者发现，在施加高曲率或量子引力效应后，经典动力学被打破，揭示了黑洞内部更为复杂的动态。

Method: 通过引入修正项并结合标量场，研究者分析了不同的动力学阶段，得出了三种新的动态行为。

Result: 研究发现了三种新的动态模式：修正的 Kasner 髓期、持续的周期振荡和增长振幅的峰形振荡。这些结果显示经典动力学仅在特异条件下适用。

Conclusion: 该研究为理解极端引力环境中的引力非线性提供了全面的方法，揭示了经典持有者克尔动力学的局限性。
...
TL;DR: 该论文探讨了带有标量场和宇宙常数的无奇性宇宙学解决方案的稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究者希望探讨宇宙学解决方案在扰动下的稳定性，以增强理论模型的可靠性。

Method: 使用了投影法分析5维的Kaluza-Klein理论及其与4维时空的关系，引入了标量场和约束条件来研究稳定性。

Result: 构造了无奇性解并分析了其基本特性，稳定性分析提供了重要线索。

Conclusion: 提出了在不同-cardinality和标量场背景下的稳定解决方案，可用于解释宇宙加速膨胀等现象。
...
TL;DR: Bulk flows appear faster and deeper than expected under the ΛCDM model. Relativistic studies attribute this to stronger peculiar velocity growth, possibly due to gravitational effects not captured in Newtonian models. The author aims to compare Newtonian and relativistic treatments and recover relativistic results from Newtonian setups by including additional terms in the Poisson equation.


<details>
  <summary>Details</summary>
Motivation: The discrepancy between observed large-scale peculiar motions and ΛCDM predictions raises questions about the cosmological model's adequacy and highlights the need to understand the role of relativistic effects in structure formation.

Method: The author compares Newtonian and relativistic treatments of peculiar velocity growth. This involves a covariant comparison, including additional source-free terms in the Poisson equation to bridge the two formalisms. The analysis likely involves cosmological simulations or analytical derivations to evaluate growth rates.

Result: The study demonstrates that relativistic effects significantly enhance peculiar velocity growth compared to Newtonian predictions. Additionally, the author shows how to recover relativistic growth rates within a Newtonian framework by appropriately including certain terms in the Poisson equation, providing a unified approach to analyzing these effects.

Conclusion: The findings suggest that relativistic gravitational inputs play a crucial role in driving peculiar velocity growth. By reconciling Newtonian and relativistic treatments, the author offers a more comprehensive understanding of bulk flow dynamics and provides a method to incorporate relativistic effects into Newtonian models, potentially improving cosmological simulations and tests of the ΛCDM paradigm.
...
### [25] [Action integrals for quantum BTZ black holes](https://arxiv.org/abs/2601.21779)
...
### [26] [Melvin-Zipoy-Voorhees Spacetime and Circular Orbits](https://arxiv.org/abs/2601.21793)
*Haryanto M. Siahaan*

Main category: gr-qc

TL;DR: 研究者通过施加磁荷，对齐普斯-沃عد齐次度量进行了广义化，得出了外尔活性度radius和光环轨道半径变化趋势。


<details>
  <summary>Details</summary>
Motivation: 广义相对论中外磁场对时空结构和光粒运动的影响是研究热点，本研究探讨外磁场对齐普斯-沃_ADDRESS齐次度量的影响。

Method: 应用磁增变换到具有四极形变形的静态种子度量，推导了外尔度和洛伦兹力影响下的度量，并计算了运动方程。

Result: 外磁场b引起的有效角动量变化，成功抑制势垒，使内稳定环轨道（ISCO）靠近中心，光环半径轻微外移。

Conclusion: 外磁场显著改变静态度量的结构，增强磁场促进特征轨道半径内缩。
...
### [27] [Compact Stars Sourced by Perfect Fluid Dark Matter Halos](https://arxiv.org/abs/2601.21848)
*Yuan Yue,Yong-Qiang Wang*

Main category: gr-qc

TL;DR: dark matter halos can support regular black holes or compact stars as isotropic perfect fluids, galactic dark matter profiles are used, einasto and dehnen models are applied, numerical solutions show non-singular, horizonless compact star solutions that are stable against axial perturbations and satisfy the dominant energy condition.


<details>
  <summary>Details</summary>
Motivation: the research is motivated by the exploration of dark matter halos supporting black holes or compact stars, which could contribute to our understanding of galactic structures and gravitational physics.

Method: the study employs galactic dark matter profiles, specifically the einasto and dehnen models, as the mass-energy density source and numerically solves the einstein field equations to find non-singular, horizonless compact star solutions.

Result: the research successfully finds a class of non-singular, horizonless compact star solutions supported by dark matter halos as isotropic perfect fluids. these configurations satisfy the dominant energy condition and are stable against axial perturbations.

Conclusion: this work extends previous findings on anisotropic energy-momentum tensors by considering isotropic perfect fluids, providing new insights into the possible astrophysical configurations supported by dark matter halos.
...
### [28] [Numerical simulations of primordial black hole formation via delayed first-order phase transitions](https://arxiv.org/abs/2601.21878)
*Zhuan Ning,Xiang-Xi Zeng,Rong-Gen Cai,Shao-Jiang Wang*

Main category: gr-qc

TL;DR: 研究了超音域虚无域（FVD）喷气对称的数值模拟，分析了引力-标量-流体相互作用中的超级多重态相变，提出了形成原黑洞的条件。


<details>
  <summary>Details</summary>
Motivation: 研究超音域虚无域喷气对称的数值模拟，解释早期宇宙中的原黑洞形成过程，特别是延迟的一阶相变中的相变机制，是当前宇宙学的重要研究方向。

Method: 通过自适应网格细化方法对超音域虚无域喷气对称进行全非线性数值模拟，识别了三种动力学结果：类型B超临界PBH形成，类型A次临界PBH形成和不形成。

Result: 通过时间尺度比$t_	ext{H}/t_	ext{V}$和局部密度对比$	ext{δ}(t_	ext{H})$作为PBH形成判据，研究得出$t_	ext{H}/t_	ext{V}$是一个更鲁棒的判据预测标志，能够区分类型B和类型A的PBH形成情况。当存在清晰的薄壁FVD边界时，$	ext{δ}(t_	ext{H})$也可以区分不同的结果。

Conclusion: 研究提供了FVD喷气 collapse的新见解，定量化了PBH 形成的阈值，并为从延迟的 FOPT 中精确预测 PBH 百分比奠定了基础。这对于我们理解早期宇宙的结构和形成原黑洞过程具有重要意义。
...
### [29] [Stückelberg inspired approach for avoiding singular Hamiltonians in Lorentz violating models of antisymmetric tensor field](https://arxiv.org/abs/2601.22007)
*Sandeep Aashish,Md Saif*

Main category: gr-qc

TL;DR: 研究发现通过引入辅助矢量场修复了洛伦兹违背模型的真空流形上存在的奇异哈密顿问题。


<details>
  <summary>Details</summary>
Motivation: 研究希望探索如何保存大量物理定律的对称性，尤其是在量子引力和宇宙学中的应用。

Method: 研究采用Dirac-Bergmann方法进行约束分析，引入辅助矢量场（类似于Stückelberg机制）来恢复拉格朗日的对称性。

Result: 通过引入这种辅助场，哈密顿变得更加平滑，允许在量子引力条件下对早期宇宙进行有意义的研究。

Conclusion: 该方法为解决量子引力中的洛伦兹违背问题提供了新途径，具有潜力应用于早期宇宙模型。
...
### [30] [Decomposition of Schwarzschild Green's Function](https://arxiv.org/abs/2601.22015)
*Junquan Su,Neev Khera,Marc Casals,Sizheng Ma,Abhishek Chowdhuri,Huan Yang*

Main category: gr-qc

TL;DR: 该论文通过球面分解的方法，详细描述了非旋转黑洞Green's函数的组成部分，包括直接部分、 quasinormal 模和尾部。通过对比频域和时域计算，验证了各组成部分的一致性，并与 Schwarzschild 空间中的早期研究成果相符。该研究对二黑洞合并的振幅阶段和非线性相互作用具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 理解黑洞合并过程中振荡阶段的Green's函数 played 出关键作用，尤其是在 ringdown阶段的直接波和 quasinormal 模识别方面。

Method: 论文采用频域分析，通过 branch cut 积分和 quasinormal 模的极点分析，对 Schwarzschild 时空中的 Green's函数进行了分解。同时，利用时间域的 Regge-Wheeler 方程求解验证了各部分的一致性。

Result: 论文确认了 Green's函数的直接部分、quasinormal 模和尾部，并通过与 Schwarzschild 空间的已有研究结果对比，确认了其一致性，为后续振幅阶段和非线性相互作用的研究奠定了基础。

Conclusion: 该研究为黑洞振荡阶段的Green's函数分析提供了一种全面的方法，并验证了其own结果与已有研究的一致性。这项工作有助于更好地理解黑洞合并过程中的复杂物理现象，尤其是振荡阶段的 directly wave 和 quasinormal 模的行为。
...
### [31] [Optimal cross-correlation technique to search for strongly lensed gravitational waves](https://arxiv.org/abs/2601.22138)
*Anirban Kopty,Sanjit Mitra,Anupreeta More*

Main category: gr-qc

TL;DR: The paper introduces a new computational method, OCCAM, which efficiently identifies strongly lensed gravitational wave event pairs with high sensitivity but higher computational cost than current methods.


<details>
  <summary>Details</summary>
Motivation: As gravitational wave observatories improve, detecting lensed event pairs becomes feasible, but existing methods are computationally expensive and require prior source knowledge. This study introduces OCCAM to address these limitations.

Method: OCCAM is an optimal, model-dependent approach using cross-correlation analysis on strain data from multiple detectors for compact binary coalescence events.

Result: OCCAM achieves 97% lensed event detection at a false positive rate of ~13% for a single LIGO detector with an SNR greater than 10, and even lower FPR for improved networks.

Conclusion: OCCAM significantly outperforms other computationally inexpensive methods and becomes more viable when combined with detector networks and sky-localization data to reduce false positives, making it efficient and suitable for rapidly searching among numerous events.
...
### [32] [Varying Newton constant, entropy and the black hole evaporation law](https://arxiv.org/abs/2601.17162)
*Julia Haba,Zbigniew Haba*

Main category: gr-qc

TL;DR: 研究了变种牛顿常数和宇宙常数对爱因斯坦方程的影响，发现如果能量-动量张量不守恒，则必有某个常量随时间变化；应用热力学将这些常量的变化与熵联系起来，导出了有变种牛顿常数的施瓦茨沙特尔 Black 孔 Boltzman 方程的变形，证明其寿命比常数 G 的情况下长 9/5 倍；平均密度保持恒定时，黑洞的半径趋于零。


<details>
  <summary>Details</summary>
Motivation: 通过热力学和爱因斯坦方程，研究变种物理常数对黑洞蒸发的影响。

Method: 利用爱因斯坦方程、Bianchi 恒等式和热力学理论分析变数常数随时间的变化。

Result: 导出改变量牛顿常数和宇宙常数的施瓦茨沙特尔方程变形，平均密度恒定时黑洞半径趋于零，变种常数的黑洞寿etimes比有定常数的更长。

Conclusion: 变数常数可能导致黑洞蒸发过程放缓，同时黑洞密度的恒定特征在黑洞蒸发到极小值时保持稳定。
...
### [33] [Trapping and cooling mechanisms in blue-detuned magneto-optical traps of molecules](https://arxiv.org/abs/2601.21097)
...
### [34] [Applicability of the Dirac-Fock method combined with Core Polarization in calculations of alkali atoms](https://arxiv.org/abs/2601.21723)
...
### [35] [Statistics and systematics of electron EDM searches with BaF](https://arxiv.org/abs/2601.21781)
...
TL;DR: The study introduces experiments aiming to measure the electric dipole moment of an electron in Barium monofluoride, employing spin precession methods. The current sensitivity is 2(3) x 10^-25 e·cm, limited by statistical data within 34 hours. Systematic biases from electric field E and laser intensity in the experimental setup are also quantified through data analysis. Upgrades like a cryogenic buffer gas source for laser-cooled beams and improved fluorescence detection could enhance sensitivity.


<details>
  <summary>Details</summary>
Motivation: The search for the electron's electric dipole moment is a fundamental test of parity violation in the Standard Model, with implications for our understanding of the symmetry of nature.

Method: The paper describes a technique using spin precession in the presence of a static electric field to measure the electron's electric dipole moment. The method involves monitoring the precession of BaF molecules exposed to laser fields while accounting for potential systematic errors.

Result: The experimental limits on the electron's electric dipole moment are 2(3) x 10^-25 e·cm. The study identifies the electric field strength and laser intensity in the experimental volume as primary sources of systematic bias, which are quantified and minimized through data analysis.

Conclusion: The successful measurement of the electron's electric dipole moment represents a significant advancement in precision tests of fundamental physical symmetries, with further upgrades potentially reducing measurement uncertainties and enhancing sensitivity.
...
### [36] [Spectral properties of high-order harmonic radiation enhanced by XUV-driven electron-hole dynamics](https://arxiv.org/abs/2601.21825)
...
### [37] [Coherent vibrational dynamics in molecular bond breaking: methyl radical umbrella mode probed by femtosecond x-ray spectroscopy](https://arxiv.org/abs/2601.21949)
...
TL;DR: 该论文报告了通过光解离甲基碘得到甲基基态 radicals，并通过甲基基态的ν₂“上 percentages”振动能激发高能态。动力学过程中由于对称性，实空间振动主要表现为x射线能谱在ν₂进阶频率差异处的变化，同时也在基频观察到ν₂模式。通过完全量子力学模型，实现了对相干叠加态的严格表征和实空间运动轨迹的重建。研究结果表明，轨迹的主要特征是由高度相干激发和强负非调谐性ν₂模式驱动的量子beat现象。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在通过光解离和 applicant实空间激发机制†来探索分子振动的动态行为，尤其是甲基基态 radicals的量子动力学特性。

Method: 该研究利用 x 射线吸收 spectroscopy (XAS) 实时态测量技术，结合量子力学模型，对甲基 radicals 的相干振动动力学进行表征。

Result: 研究表明，甲基 radicals 在 ν₂ 振动模式下表现出强量子 beat，其动力学行为由高度相干激发和强非调谐性主导。

Conclusion: 该研究为理解分子基态 radicals 的量子动力学提供了新的实验证据，并展示了 x 射线光谱技术在研究分子振动动力学中的应用潜力。
...
### [38] [Spin angular momentum transfer in the Einstein-de Haas effect](https://arxiv.org/abs/2601.20946)
*Xin Nie,Wenhao Luo,Kun Cao,Dao-Xin Yao*

Main category: cond-mat.mes-hall

TL;DR: 研究分析了爱因斯坦-德·汉斯效应中磁性晶体中自旋角动量的传递，发现了自旋角动量如何同时传递给声子和刚体旋转，表现出不同的方式，声子吸收大部分动能。研究还表明，伪轴向形和德zyaloshinskii-Moriya相互作用对自旋角动量的传递有不同的控制。增强伪轴向形会增加传递的自旋角动量总量，而增强德zyaloshinskii-Moriya相互作用会加快传递率并增加声子的自旋角动量比例。这项研究有助于 clarification 磁性晶体中爱因斯坦-德·汉斯效应的微观机制，为致力于角动量的可控移动提供了可能性。


<details>
  <summary>Details</summary>
Motivation: 研究爱因斯坦-德·汉斯效应中的角动量传递机制对磁性晶体的应用可能有助于增强微ancial和Timing器的设计能力，从而提升相关技术性能。

Method: 使用Eckart框架分析了声子和刚体旋转之间的角动量分配，观察了角动量和能量如何不同地转移，分析了伪轴向形和德zyaloshinskii-Moriya相互作对方程式的影响力。

Result: 结果表明，伪轴向形增强会增加传递的角动量总量，而德zyaloshinskii-Moriya增强会使转移率加快和声子的角动量比例增加。

Conclusion: 这项研究清晰地描绘了爱因斯坦-德·汉斯效应的微观机制，并为磁性微器件中的角动量控制提供了理论依据。
...
### [39] [Deeply nonlinear magnon-photon hybrid excitation](https://arxiv.org/abs/2601.21549)
*Dinesh Wagle,Anish Rai,M. Benjamin Jungfleisch*

Main category: cond-mat.mes-hall

TL;DR: 该论文研究了微波功率对磁单极子-光偶合的影响，发现无量纲的自旋波相互作用通过微波功率下磁静模式的耗散抑制了偶合，表明在较低微波功率下存在较强的偶合，而随着功率的增加，偶合逐渐减弱直至消失，特别是在低于特定外部磁场时，证实了Suhl的一阶不稳定性导致的消亡。


<details>
  <summary>Details</summary>
Motivation: 研究微波功率对磁单极子-光偶合的影响，旨在揭示磁单极子与光之间的相互作用机制及其在非线性磁性设备中的应用潜力，如频率转换、开关等。

Method: 采用实验和理论相结合的方法，研究了yttrium iron garnet-sphere/split-ring-resonator杂合系统的磁单极子-光偶合行为，利用微波功率变化观察磁单极子和光的相互作用随功率的演变。

Result: 随着微波功率的增加，磁单极子和光的相互作用逐渐减弱，甚至在特定外部磁场下完全消失。这表明，在较低功率下，磁单极子-光偶合具有强耦合性，而高功率下则表现为弱耦合性。

Conclusion: 该研究表明，微波功率显著影响磁单极子-光偶合的强度和稳定性，为设计高效的非线性磁性设备提供了理论依据和实验指导。
...
### [40] [Topological Acoustic Diode](https://arxiv.org/abs/2601.20951)
*Ashwat Jain,Wojciech J. Jankowski,M. Mehraeen,Robert-Jan Slager*

Main category: cond-mat.mes-hall

TL;DR: 该论文展示了某些三维拓扑相可以作为声波二向分光镜，实现非线性的奇声致动效应。我们发现了拓扑引起的异常声二次谐波生成和偏转，并展示这种非线性响应如何由量子态Hilbert空间几何中的非度量性张量唯一捕捉。此外，我们的研究补充了量子几何观测量在二次响应区的分类，并揭示了使用形变θ空位的拓扑绝缘器来实现声波单向透镜的新实验途径。


<details>
  <summary>Details</summary>
Motivation: 声波在不同介质中的传播特性一直是材料科学中一个重要的研究方向，本研究通过拓扑相的性质，揭示了声波在非平凡拓扑序中的独特响应，具有潜在的实验实现价值。

Method: 通过分析声波在不同拓扑相中的传播特性，并结合量子几何学中的非度量性张量，使用理论建模和实验验证的方法。

Result: 发现了拓扑引起的异常声二次谐波生成和偏转现象，并提出了使用形变θ空位的拓扑绝缘器来实现声波单向透镜的新实验方案。

Conclusion: 该研究扩展了声波在拓扑序中的行为理解，为声波“控制与工程”提供了新的理论框架和实验基础。
...
### [41] [Rocket-like dynamics of ferrimagnetic domain walls in graded materials](https://arxiv.org/abs/2601.21068)
...
### [42] [Tunneling probe-based identification of the sp${}^3$ dangling bond on the H-C(100):$2\times1$ surface](https://arxiv.org/abs/2601.21154)
...
### [43] [One-Dimensional Electronic States in a Moiré Superlattice of Twisted Bilayer WTe2](https://arxiv.org/abs/2601.21228)
...
TL;DR: 研究总结：使用密度泛函理论和扫描电镜观察到层状堆叠引起的1D摩尔 interference的重构，揭示了控制层叠宽度和电磁性能的关键机制。


<details>
  <summary>Details</summary>
Motivation: 探索1D摩尔干涉超晶格对二维材料如WTe2的潜在应用和影响。

Method: 密度泛函理论计算和高角度圆环暗场扫描电镜实验。

Result: 层状重构显著影响摩尔条纹，诱导了具有强沿条纹方向散射的准一维电子态，可由内层摩尔势主导。

Conclusion: 层状重构是1D摩尔超晶格中一维电子态的关键机制，为设计类似系统提供了理论基础。
...
### [44] [Stimulated Magnonic Frequency Combs](https://arxiv.org/abs/2601.21370)
*Xueyu Guo,Tianci Gong,Guibin Lan,Mengying Guo,Xiufeng Han,Guoqiang Yu,Peng Yan,Qi Wang*

Main category: cond-mat.mes-hall

TL;DR: Magnonic frequency combs can be achieved through a new mechanism that overcomes the limitations of three-magnon scattering, enabling their practical application in spintronic devices.


<details>
  <summary>Details</summary>
Motivation: Understanding and controlling magnonic frequency combs improves information processing and sensing technologies.

Method: The approach combines theoretical modeling, micromagnetic simulations, and experimental validation to achieve precise control over the comb properties.

Result: The method allows for the feasible generation of magnonic frequency combs with adjustable properties, marking a significant advancement in magnon spintronics.

Conclusion: This breakthrough opens new pathways for integrating magnonic combs into practical applications for spintronic devices.
...
### [45] [Quantum Otto cycle in the Anderson impurity model](https://arxiv.org/abs/2601.21546)
*Salvatore Gatto,Alessandra Colla,Heinz-Peter Breuer,Michael Thoss*

Main category: cond-mat.mes-hall

TL;DR: 研究单杂质安德erson模型中周期量子外 cycle 的热力学性能，使用时间演化生成器的最小损耗分解方法和精确的分层运动方程方法，分析量子热机械装置的操作 regimes 和 Coulomb 相互作用、系统-热库强耦合和能级对齐等影响因素。结果表明 Coulomb 相互作用能改变操作 regimes 并可能提高效率。


<details>
  <summary>Details</summary>
Motivation: 量子热力学领域的研究近年来变得非常重要，理解量子热机的操作 regimes 和效率提升因素对于实现高效量子热机设计至关重要。

Method: 本研究结合了最小损耗分解方法和精确的分层运动方程（HEOM）方法，对单杂质安德erson模型中的周期量子外 cycle 进行了详细的数值分析。

Result: 研究结果表明，Coulomb 相互作用不仅能够改变量子热机的工作 regimes，还可能导致效率的提高。

Conclusion: 通过引入 Coulomb 相互作用，量子热机的效率得到了提升，这一发现为设计更高效的量子热机提供了新的思路。
...
### [46] [Intrinsic Nonlinear Gyrotropic Magnetic Effect Governed by Spin-Rotation Quantum Geometry](https://arxiv.org/abs/2601.22019)
*Neelanjan Chakraborti,Snehasish Nandy,Sudeep Kumar Ghosh*

Main category: cond-mat.mes-hall

TL;DR: 研究发现，周期性磁场驱动的非线性磁响应揭示了超越常规电场驱动的量子几何效应。详细阐述了量子几何张量在非线性磁响应中的关键作用。实现了对二维系统中二次磁流的理论推导，揭示了几何分离现象及其对称性在不同量子态中的作用。这些结果为在光电子学和自旋电电子学中工程设计非线性磁响应提供了理论依据。


<details>
  <summary>Details</summary>
Motivation: 量子几何学因磁场驱动而非线性磁响应的研究具有重要的理论和应用价值，特别是其在光电子学和自旋电电子学中的潜在应用。通过解析该研究的工作内容和结论，有助于理解磁场驱动下的量子几何现象及其在材料科学中的应用。

Method: 通过量子动力学框架分析磁场对磁流的影响，推导了二次磁流的表达式，并通过不同数值模拟材料实例说明了结果的适用性。

Result: 二次磁流的结构与几何张量和自旋旋转变换量子几何张量密切相关，呈现出几何分离现象，当材料满足特定对称性条件时，可以在导电和位移 channel中实现不同偏置下的表现。

Conclusion: 研究填补了磁场驱动的二次磁响应理论空白，展示了自旋角动量量子失真在几何量中的作用，对于开发具有优异非线性磁响应的自旋相关电子设备具有指导意义。
...
### [47] [Magnetic texture modulated superconductivity in superconductor/ferromagnet shells of semiconductor nanowires](https://arxiv.org/abs/2601.22051)
...
TL;DR: In 1D ferromagnet-superconductor nanowires, magnetism typically suppresses superconductivity except in specific cases like domain wall superconductivity (DWS) or multi-domain-averaged superconductivity (MDAS), where the net magnetization averages to nearly zero.


<details>
  <summary>Details</summary>
Motivation: The paper investigates the interplay between magnetism and superconductivity in 1D ferromagnet-superconductor nanowires, particularly in InAs/EuS/Al systems, to understand when and where superconductivity can persist despite the presence of magnetism.

Method: The study employs scanning SQUID magnetometry and transport measurements on full-shell InAs/EuS/Al nanowires to analyze the conditions under which superconductivity occurs in the Al shell.

Result: Superconductivity in the Al shell only occurs when the EuS layer is in a multi-domain state, as indicated by both DWS and MDAS phenomena. In the saturated single-domain state, superconductivity is absent.

Conclusion: The findings suggest that the multi-domain state of the EuS layer is crucial for enabling localized superconductivity in the Al shell along the nanowire, which could have implications for topological qubits, Andreev spin qubits, superconducting logic, and memory devices.
...
### [48] [Photoionization in KTN deflectors by light in the near-infrared imaging window](https://arxiv.org/abs/2601.21029)
*Samuel Stanek,Harishankar Jayakumar,Christopher Warkentin,James Leger,Aaron Kerlin*

Main category: physics.optics

TL;DR: KTN-based electro-optical deflectors (EODs) are effective in optical microscopy but have limitations in the visible light range. While they can be used with near-infrared light for biological imaging, there is a lack of understanding of photon-induced charge escape (photoionization) in this range. This paper presents measurements of photoionization in NIR windows and provides quantitative guidance for selecting imaging wavelengths and duty cycles for KTN EODs.


<details>
  <summary>Details</summary>
Motivation: The paper addresses a gap in understanding the applicability of KTN-based EODs in near-infrared imaging, which is important for deep tissue and nonlinear microscopy.

Method: The study uses a two-beam polarization interferometer to measure trapped charge density as a function of photon fluence across NIR windows. The data shows that the photoionization rate decreases with wavelength and exhibits multi-exponential decay, suggesting the presence of multiple trap species or recapture effects.

Result: The findings provide critical quantitative data for optimizing KTN-based EODs in near-infrared applications, particularly highlighting the wavelength dependence of photoionization and the complexity of charge dynamics.

Conclusion: This work advances the understanding of KTN EODs in near-infrared imaging and provides guidance for their optimal use in biological and nonlinear microscopy applications.
...
### [49] [Wavelength-selective nonlinear wavefront control in resonant thin-film lithium niobate metasurfaces](https://arxiv.org/abs/2601.21382)
*Madona Mekhael,Timo Stolt,Helena Weigand,Kiia Arola,Rachel Grange,Patrice Genevet,Mikko J. Huttunen*

Main category: physics.optics

TL;DR: 该论文提出了一种基于亚波长选择性谐波生成的新型非线性 metasurface，通过工程化的相位控制实现精确的波形重塑。


<details>
  <summary>Details</summary>
Motivation: 当前的非线性 metasurface 基于几何相位，缺乏波长分辨率，导致非线性响应缺乏动态调节。开发具有良好波长选择性的新型非线性材料和结构具有重要的应用前景。

Method: 该研究通过设计相位工程化的两区域结构，利用 Mie 型谐振器的光谱调谐特性，实现了谐波的精确控制，实现了同时的频率转换和空间模式重塑。

Result: 实验结果表明，该方法可将 1100 nm 的高斯能 pumps 调谐为 550 nm 的第一阶 Hermite-Gaussian 模式，同时保持了泵波的形态。该结构为超紧凑可调谐非线性元件提供了新的设计思路，可用于非线性衍射和相关应用。

Conclusion: 该研究 breakthrough 促进了超紧凑可调谐非线性元件的开发，为非线性衍射技术和应用开辟了新的途径。
...
### [50] [Hybrid Barium Titanate Waveguide Designs For Efficient Nonlinear Frequency Conversion](https://arxiv.org/abs/2601.21427)
...
### [51] [Optically reconfigurable canalization of exciton-polaritons in a non-hyperbolic perovskite](https://arxiv.org/abs/2601.21443)
...
### [52] [Measurement of stray light in the LISA instrument](https://arxiv.org/abs/2601.21501)
...
### [53] [Non-secular polariton leakage and dark-state protection in hybrid plasmonic cavities](https://arxiv.org/abs/2601.21597)
*Marco Vallone*
...
### [54] [Reverse Energy Flows in Two-Dimensional Photonic Crystals and Analogies with Vortex Formation and Analogous Flows in Hydrodynamics](https://arxiv.org/abs/2601.21704)
*Andrey Pryamikov*

Main category: physics.optics

TL;DR: 通过光子晶体中波导管结构对反向电磁能量流动的存在与否和方向进行了分析。


<details>
  <summary>Details</summary>
Motivation: 关于高钻率材料和反向能量流动的研究，具有重要的理论意义和潜在的应用价值。

Method: 利用分形曲线微分变换法和有限差分频域法对耗尽型光栅和快钱型光栅进行了数值模拟。

Result: 通过改迂数值模拟方法，首次成功地对两种二维光栅的波纹损伤进行了研究，尤其是耗尽型和快钱型光栅在不同孔径比下的损伤特性。

Conclusion: 本研究通过采用分形曲线微分变换法和有限差分频域法，首次成功地对两种二维光栅的波纹损伤进行了研究，尤其是耗尽型和快钱型光栅在不同孔径比下的损伤特性。
...
### [55] [Mode Switching Through Exceptional Points Induced by Lasing-Inversion Coupling](https://arxiv.org/abs/2601.21907)
*Xingwei Gao,Cheng Guo,David Burghoff*

Main category: physics.optics

TL;DR: บทความที่ display 学期ิแส demonstrating a soliton-based threshold mechanism for mode switching in lasing systems with closely spaced exceptional points proposing a natural explanation for controlling photonic systems including adaptive optical communication and dual-comb spectroscopy.


<details>
  <summary>Details</summary>
Motivation: The paper discusses an interesting phenomenon in lasing systems involving exceptional points and mode switching, which could have implications for adaptive optical communication and sensing.

Method: Derives a Bloch coupled-mode theory from first principles to explain the mechanism of mode switching and identifies the conditions under which the second threshold occurs.

Result: The theory successfully predicts the behavior of the system, showing that the addition of extra exceptional points allows for mode switching between single-mode lasing and frequency combs with adjustable repetition rates.

Conclusion:  nightly achievement in understanding mode switching in lasing systems with EPs and opening new possibilities for photonic applications.
...
### [56] [Broadband Second Harmonic Generation using Fixed-Period Periodically Poled Lithium Niobate](https://arxiv.org/abs/2601.21932)
*Yongjoo Kim,Yijia Cai,Zhixin Liu*

Main category: physics.optics

TL;DR: The study focuses on using standard unpoled lithium niobate (PPLN) crystals equipped with multiple fixed poling periods to achieve broad second harmonic generation (SHG) bandwidth through angle tuning, resulting in a 1.6-fold improvement in bandwidth without the need for custom aperiodic or chirped structures.


<details>
  <summary>Details</summary>
Motivation: Broadband second harmonic generation is crucial for nonlinear optical applications, such as ultrafast optics and sources for second-order nonlinear processes. Achieving this without custom structures is practically valuable.

Method: The research involves using PPLN crystals with multiple built-in poling periods, adjusting the crystal angle relative to input beams, and measuring SHG performance across varying angles.

Result: Angle tuning enhances SHG bandwidth by 1.6 times compared to unpoled PPLN crystals, indicating a viable, cost-effective method for achieving broad bandwidth SHG.

Conclusion: This approach suggests a simple, efficient method for achieving broad second harmonic generation using standard PPLN devices without requiring complex manufacturing processes.
...
### [57] [StochasticGW-GPU: rapid quasi-particle energies for molecules beyond 10000 atoms](https://arxiv.org/abs/2601.21005)
...
TL;DR: StochasticGW开发了一个高性能的GW近似代码，并引入了并行化和GPU加速。


<details>
  <summary>Details</summary>
Motivation: 研究量子材料和凝聚态物理中的光子属性需要高效准确的计算工具。

Method: 基于sROI技巧实现并行计算，GPU加速提高了效率。

Result: 代码能计算大规模系统，如含有10001个原子、35144个电子的氢硅化合物，精度优于±0.03eV。

Conclusion: 该方法扩展了GW近似的应用范围，是计算大系统的重要工具。
...
### [58] [Better without U: Impact of Selective Hubbard U Correction on Foundational MLIPs](https://arxiv.org/abs/2601.21056)
*Thomas Warford,Fabian L. Thiemann,Gábor Csányi*

Main category: physics.chem-ph

TL;DR: 机器学习势场（fMLIP）训练在使用不同数据库时会引入一致性的不一致，这会引发系统低估结合能或产生虚假排斥效应，建议在未来的数据集中排除+U因子的应用。


<details>
  <summary>Details</summary>
Motivation: 材料科学中的机器学习势场在训练时依赖于大量材料数据，数据库中对+U的应用不一致会引起势场不一致，这会影响模型的准确性。

Method: 通过分析材料库中的+U应用不一致，研究开发了一种简单的修正方法以平滑势场不一致。

Result: 应用修正方法后，模型对氧吸附能的预测精度得到了提高。

Conclusion: 结果表明，势场不一致会引起系统性误差，未来数据集应排除+U因子。
...
### [59] [Accurate Thermophysical Properties of Water using Machine-Learned Potentials](https://arxiv.org/abs/2601.21103)
*Tobias Hilpert,Georg Kresse*

Main category: physics.chem-ph

TL;DR: 摘要通过MACE模型预测了水的密度等参数，尽管MACE模型计算更昂贵，但其更精确的误差使其可信赖地用于水分热物性质研究。


<details>
  <summary>Details</summary>
Motivation: 研究水的热物性质需要高精度的计算工具，MACE模型提供了可行方案。

Method: 使用了MACE模型，基于RPBE-D3数据库”， 

Result: MACE模型的精确性高于核势模型，可可靠用于热物性质研究

Conclusion: MACE模型可靠应用于水分热物性质研究
...
### [60] [A Deterministic Framework for Neural Network Quantum States in Quantum Chemistry](https://arxiv.org/abs/2601.21310)
*Zheng Che*
...
### [61] [Fewest-Switches Surface Hopping with Combined Deep Learning Potential and Long Short-Term Memory Network Propagator for Simulating Realistic Photochemical Processes](https://arxiv.org/abs/2601.21703)
*Zhenxing Zhu,Diandong Tang,Lin Shen,Wei-Hai Fang*

Main category: physics.chem-ph

TL;DR:  paper introduces an LSTM-FSSH framework for simulating photochemical reactions with enhanced accuracy and efficiency by integrating equivariant neural networks for constructing adiabatic potential energy surfaces, achieving accurate excited-state lifetimes and product yields using only 10 reference trajectories for training.


<details>
  <summary>Details</summary>
Motivation:  developing advanced computational methods to simulate photochemical reactions efficiently and accurately is crucial for understanding molecular processes. LSTM networks offer a promising approach for modeling complex electronic subsystems, and this work aims to improve their application in FSSH simulations.

Method:  the approach combines Fewest-switches surface hopping (FSSH) with long short-term memory (LSTM) networks. LSTM networks are redesigned to handle high-dimensional nuclear degrees of freedom effectively. Equivariant neural networks are integrated with LSTM to construct adiabatic potential energy surfaces in both ground and excited states. The training uses only 10 reference trajectories to generate a trajectory ensemble for simulations.

Result:  the proposed LSTM-FSSH framework demonstrates accuracy in predicting excited-state lifetimes and product yields for photoisomerizations of CH2NH and azobenzene. Compared to conventional FSSH simulations, it achieves similar results with significantly fewer training trajectories, making it more efficient.

Conclusion:  this study advances the application of LSTM networks in photochemical reaction simulations by leveraging their ability to model complex dynamics. The integration of equivariant neural networks enhances the construction of potential energy surfaces, and the efficiency of training with limited trajectories highlights the potential of this method for practical applications in computational chemistry.
...
### [62] [Third and fourth density and acoustic virial coefficients of neon from first-principles calculations](https://arxiv.org/abs/2601.21721)
*Robert Hellmann,Giovanni Garberoglio*

Main category: physics.chem-ph

TL;DR: This paper presents precise calculations of the third and fourth density and acoustic virial coefficients of neon using advanced quantum Monte Carlo methods and a sophisticated intermolecular potential.


<details>
  <summary>Details</summary>
Motivation: Understanding accurate virial coefficients is essential for modeling and predicting macroscopic thermodynamic properties of gases like neon, which has applications in various fields including astrophysics and atmospheric science.

Method: The study utilized the path-integral Monte Carlo (PIMC) approach within the PIMC-Pythian framework coupled with the Hellmann et al. pair potential. The potential leverages supermolecular ab initio calculations with high-level quantum chemistry methods, including relativistic and post-Hartree-Fock effects, to achieve reliable uncertainty estimates. The researchers also developed a nonadditive three-body potential and accounted for nonadditive four-body contributions.

Result: The calculated virial coefficients showed significantly smaller uncertainties compared to experimental data, enhancing the reliability of predictive models for neon's thermodynamic behavior.

Conclusion: This work advances theoretical methods for accurately calculating virial coefficients, providing a foundation for improved modeling of neon's thermodynamic properties across various temperature ranges.
...
### [63] [Excited-State Intermolecular Proton Transfer and Competing Pathways in 3-Hydroxychromone: A Non-adiabatic Dynamics Study](https://arxiv.org/abs/2601.21918)
*Alessandro Nicola Nardi,Morgane Vacher*

Main category: physics.chem-ph

TL;DR: 研究论文通过混合量子-经典非绝地动力学模拟，揭示了3-羟基卟啉（3-HC）体系中激 excited态内质子转移现象的两个不同时间尺度的超快和慢过程，并通过势能面分析和非绝地轨迹揭示了超快和慢过程的机制。


<details>
  <summary>Details</summary>
Motivation: 通过模拟和数据分析，揭示3-HC体系中激态内质子转移的两个不同时间尺度的超快和慢过程背后的根本机制。

Method: 使用混合量子-经典非绝地动力学模拟，分析分子动力学轨迹和势能面。

Result: 论文指出，3-HC体系中存在两个不同的激态内质子转移时间尺度，其中慢过程由平面上的氢键扭曲运动驱动。

Conclusion: 这一研究为理解非绝地激态动力学提供了一个统一的机制，解释了超快和慢过程的共存。
...
### [64] [Molecular structure, binding, and disorder in TDBC-Ag plexcitonic assemblies](https://arxiv.org/abs/2601.22022)
...
### [65] [Effect of Nanopore Wall Geometry on Electrical Double-Layer Charging Dynamics](https://arxiv.org/abs/2601.22048)
...
### [66] [Resource-Theoretic Quantifiers of Weak and Strong Symmetry Breaking: Strong Entanglement Asymmetry and Beyond](https://arxiv.org/abs/2601.20924)
...
### [67] [Quiver-Invariant Dualities between Brane Tilings](https://arxiv.org/abs/2601.20936)
...
### [68] [Asymptotic Safety in Generalized Proca Theories](https://arxiv.org/abs/2601.20944)
*Lavinia Heisenberg,Alessia Platania,Sara Rufrano Aliberti*

Main category: hep-th

TL;DR: Generalized Proca Theories can be consistently completed to a quantum field theory via the Proca fixed point.


<details>
  <summary>Details</summary>
Motivation: The paper explores the potential of Generalized Proca Theories as a quantum field theory framework, contributing to understanding their role as models for dark energy.

Method: Applied truncation techniques with four Proca field powers and two derivatives, analyzing ultraviolet fixed points.

Result: The Proca fixed point stands out as a viable ultraviolet completion, despite proximity to other fixed points which are problematic.

Conclusion: Proca fixed point provides a consistent quantum framework for Generalized Proca Theories, advancing their use in fundamental physics.
...
### [69] [Holomorphic structure of massive scalar fields in $\text{(A)dS}_2$](https://arxiv.org/abs/2601.20947)
*Calvin Y. -R. Chen,Lukas W. Lindwasser,Massimo Porrati*

Main category: hep-th

TL;DR: 研究$	ext{(A)dS}_{2}$中的标量场理论在整数标度维数$	ext{Δ}=k+1$时，发现它们包含一对（反）共形高阶电流。通过分析这些电流的性质，推导出理论的量子化方法与其线性和非线性对称性。特别地，区别处理$	ext{AdS}_{2}$和$	ext{dS}_{2}$情形，并指出现有模式类似于二维共形场理论的复坐标版本。利用电流的共性构造全局共性和Virasoro生成元。此外，借助电流的共性，探讨了$k>0$时完全对称性集合子代数的Chiral代数结构，并识别出$k 
eq 0,1,2$时可积变形的可能性，且更大的$k$缺乏可积变形是已知猜想的后果。


<details>
  <summary>Details</summary>
Motivation: 研究(AdS)_2和dS_2中标量场理论的对称性和可积性，尤其是在特定标度维数下，可以帮助我们理解这些空间的物理性质和发展相关的量子理论。

Method: 构造复坐标下的模式展开式，理解其若干几何对称和Chevalley-Eilenberg代数，分析Chiral代数的对称性结构及可积性。

Result: 通过考察电流的性质，推导出量子化条件，并利用Chiral代数构建了理论的完整对称性。

Conclusion: 结果表明，在整数标度维数下，$	ext{(A)dS}_{2}$标量场理论的对称性可分解为Chiral代数的子代数结构，这有助于识别可积性条件和预测可能的量子化。
...
### [70] [Spectral Form Factor of Gapped Random Matrix Systems](https://arxiv.org/abs/2601.20954)
*Krishan Saraswat*

Main category: hep-th

TL;DR: 研究随机矩阵模型中具有大量退化解且谱区间存在宏观间隙的情况，发现低温下连分式贡献主导，非退化解的贡献决定连分式行为，出现衰减振荡，连分式的坡度由正弦核主导，并与双 trumpet结果一致。。


<details>
  <summary>Details</summary>
Motivation: 理解在大量退化解和宏观间隙存在时，谱形式因数如何修改传统叙述，尤其是在低温下。

Method: 通过 Christoffel-Darboux 核分析示例，如 Bessel 模型和 $	extcal{N}=2$ Jackiw-Teitelboim 超引力，发现振荡衰减，连分式坡度由正弦核主导，与双 trumpet 结果一致。

Result: 低温下主导连分式贡献，非退化解影响连分式行为，出现衰减振荡，sin 核主导连分式坡度。

Conclusion: 在大量退化解和宏观间隙存在时，传统关于谱形式因数行为的叙述被修改：低温下连分式主导，连分式行为由非退化解的贡献决定，出现衰减振荡，并且连分式的坡度由正弦核主导。
...
### [71] [The Tensionless Lives of Null Strings](https://arxiv.org/abs/2601.20959)
*Arjun Bagchi,Aritra Banerjee,Ritankar Chatterjee,Priyadarshini Pandit*

Main category: hep-th

TL;DR: 研究综述了张力消失的弦理论，从Schild的早期工作开始，再到Isberg等人的基础性工作，最后讲述了过去十年的发展，重点分析了Carrollian共形代数和Carrollian结构在理论中的应用，包括闭合、开放以及超对称的Carroll弦理论。


<details>
  <summary>Details</summary>
Motivation: 张力消失的弦理论是弦理论的一个重要方面，研究它有助于理解极端能量下的物理规律以及与点粒子极限的区别。

Method: 综述型文章，包含了一系列理论分析和对现有工作的总结，包括经典理论和量子理论的分析。

Result: 文章得出，ILST作用在张力消失的极限下具有三个一致的量子理论。这些理论的频谱和Carroll弦的性质被详细讨论。并探讨了这些理论在接近黑洞等问题中的应用。

Conclusion: 文章全面综述了张力消失的弦理论，涵盖了理论方面的重要进展和其应用，为未来研究提供了坚实的基础。
...
### [72] [On the Integrable Structure of the SU(2) Wess-Zumino-Novikov-Witten Model](https://arxiv.org/abs/2601.20960)
...
### [73] [Extremal Love: tidal/electromagnetic deformability, logarithmic running and the weak gravity conjecture](https://arxiv.org/abs/2601.20962)
*Toshifumi Noumi,Sam S. C. Wong*

Main category: hep-th

TL;DR: The tidal Love numbers of extremal charged black holes in Einstein-Maxwell EFT with higher-derivative corrections show non-zero and logarithmically running behavior. The signs of these deformations are constrained by unitarity and the Weak Gravity Conjecture, and cross log runnings are explained via worldline effective field theory.


<details>
  <summary>Details</summary>
Motivation: This paper investigates the tidal deformability of extremal charged black holes when higher-derivative corrections are introduced. It is of interest due to potential implications for quantum gravity and black hole thermodynamics.

Method: The study involves computing the static linear response of extremal charged black holes in the presence of higher-derivative corrections. The analysis focuses on both the vector and parity-odd tensor sectors, specifically examining the $	extell=1$ and $	extell
eed$ sectors.

Result: The findings reveal non-zero tidal Love numbers with logarithmic running in several sectors. Of particular note are the constraints on the signs of the induced electric and magnetic susceptibilities in the $	extell=1$ sector, which must satisfy unitarity and the Weak Gravity Conjecture. Additionally, it is observed that the cross log runnings are identical in certain parts of the spectrum, a feature explained using worldline effective field theory.

Conclusion: The results suggest that higher-derivative corrections can lead to observable tidal deformations, whose properties are tightly controlled by fundamental theoretical principles such as unitarity and the Weak Gravity Conjecture. The findings also offer insights into possible worldline effective field theory descriptions of these phenomena.
...
### [74] [Higher-loop norm of the no-boundary state](https://arxiv.org/abs/2601.20993)
*Jordan Cotler*
...
### [75] [Fourfold path to Thermality: Inequivalent purifications of Rindler wedge](https://arxiv.org/abs/2601.21002)
*Rakesh k Jha*

Main category: hep-th

TL;DR: 研究量子场中的热行为，通过分析零位移的黎尼尔楔，在莫尔凯特时空中得出结论，零位移的黎尼尔楔构建产生非盖耳曼式热：单一顺行分量具有玻色-爱因斯坦占据数，逆行分量保持无占据状态。在组合变换路径中，莫尔凯特状态保持纯，而零位移楔 stringstream 状态是纯张量态。所观察到的热行为源于玻理波混合和模态时间演化而不是horizon引起的纠缠或盖耳曼式混合.


<details>
  <summary>Details</summary>
Motivation: 探索量子场中的热行为是理论物理中的重要课题，标题提到'分析零位移的黎尼尔楔的层次结构'，表明研究者旨在揭示热行为与纠缠的关系.

Method: 使用莫尔凯特-黎尼尔映射、空间平移和零位移的序列移动等方法、进行操作，并通过玻里布变换分析粒子内容.

Result: 比较标准的Unruh效果，零位移构造导致非盖耳曼式热：单一顺行分量具有玻色-爱因斯坦占据数，逆行分量保持无占据状态。组合变换路径下，莫尔凯特状态保持纯，而零位移楔 stringstream 状态是纯张量态。

Conclusion: 零位移构造提供了热谱的不等价纯化，明确了horizon结构、观察者依赖、玻里布变换和纠缠在相对量子场论中的作用。零位移构造展示了热谱出现而不依赖于纠缠导致的混合性，说明了热和纠缠的运作独立性。
...
### [76] [Background instability of quintessence model in light of entropy and distance conjecture](https://arxiv.org/abs/2601.21136)
*Min-Seok Seo*

Main category: hep-th

TL;DR: The paper uses entropy considerations in the covariant entropy bound to analyze the quintessence model in cosmology. It finds that certain conditions lead to an unstable background, which is related to the distance and trans-Planckian censorship conjectures and has implications for scale separation between Kaluza-Klein modes and the Hubble parameter.


<details>
  <summary>Details</summary>
Motivation: Understanding the stability of cosmological models like quintessence is crucial for studying the large-scale structure and evolution of the universe. The covariant entropy bound and swampland conjectures provide a theoretical framework for this analysis.

Method: The method involves applying entropy arguments to the quintessence model, considering the relationship between matter entropy (species number) and geometrical entropy (apparent horizon area). The distance conjecture is used to proposes a mechanism for entropy growth, leading to instability conditions.

Result: The paper concludes that accelerating backgrounds with event horizons are unstable, with their instability linked to the trans-Planckian censorship bound. Additionally, it suggests that a scale separation between Kaluza-Klein masses and Hubble parameters can exist under certain conditions related to entropy growth rates.

Conclusion: This work bridges swampland conjectures and entropy concepts, offering a comprehensive understanding of the quintessence model's stability and its implications for cosmological scenarios. It strengthens the connection between high-energy physics principles and cosmology, potentially guiding future theoretical developments in this area.
...
### [77] [Electric birefringence in Euler-Heisenberg pseudo-electrodynamics](https://arxiv.org/abs/2601.21143)
*M. J. Neves*

Main category: hep-th

TL;DR: 研究了一种伪量子电动力学的外尔-海森堡扩展，并分析了均匀电磁场中平面波的传播特性。


<details>
  <summary>Details</summary>
Motivation: 探讨电磁场非线性效应在特定介质中的传播特性，可能是为了理解材料在强电磁场下的光学行为。

Method: 通过构造包含非局部 Chern-Simons 项和非线性电dynamic 项的拉格朗日量，计算电磁波在均匀电磁场中的传播，求解色散关系和折射率。

Result: 在有均匀磁场的情况下，发现存在双折射现象；仅在电场存在时，折射率表现出特定频率和波长下的行为。

Conclusion: 该理论为理解伪量子电动力学在均匀电磁场中的波传播提供了框架，尤其是在材料的光学行为方面。
...
### [78] [Holographic Network and Traversable Parallel Universe](https://arxiv.org/abs/2601.21206)
*Yu Guo,Rong-Xin Miao*

Main category: hep-th

TL;DR: 该论文探讨了不同 CFT 之间通过 Gauss-Bonnet 引力相互连接的网络结构，提出了 holographic Noether 定理在 Net-brane 接界条件下的守恒定律，并分析了网络节点的稳定性及其参数约束，研究了网络熵的几种可能形式及其与 holographic g-定理的关系。此外，文章还计算了不同 NCFT 在不同边界的两点函数，并探讨了 AdS_3/CFT_2 紧致网络的引力对偶，最后提出了通过 AdS/NCFT 模型实现不同几何和物理定律的平行宇宙的可能性。


<details>
  <summary>Details</summary>
Motivation: 探索不同 CFT 之间的网络结构在引力理论中的相互作用及其在量子引力和场论中的应用至关重要。

Method: 利用 holographic Noether 定理、分析网络熵的稳定性、研究两点函数的计算，并通过 AdS/NCFT 模型探讨平行宇宙的可能性。

Result: 提出了 holographic Noether 定理在 Net-brane 上的守恒定律，分析了网络熵的约束条件，并展示了 AdS/NCFT 模型实现平行宇宙的能力。

Conclusion: 该网络结构在引力理论和量子场论中的研究提供了新的视角，特别是在平行宇宙的可能性上具有重要意义。
...
### [79] [BBGKY Hierarrchy for N D0-Branes](https://arxiv.org/abs/2601.21451)
*J. Kluson*
...
### [80] [Holographic Entanglement Propagation Through Wormholes](https://arxiv.org/abs/2601.21604)
...
### [81] [Numerical Computations of Entanglement Measures in Curved Space](https://arxiv.org/abs/2601.21566)
*Suresh Govindarajan,Sreehari A Padinhareveettil,Raghotham A Kulkarni*

Main category: hep-th

TL;DR: 该论文通过数值计算的方法，提出了标量场和规范场在多种情况下纠缠熵和负性度的计算方法，拓展了萨德尼克的原始计算。研究采用共变离散化的方法，并将结果与反热核系数做了对比。


<details>
  <summary>Details</summary>
Motivation: 纠缠度和负性度是对研究量子纠缠性质的指标，了解这些量在不同场和空间情况下的行为对量子场论和量子引力的研究具有重要意义。

Method: 该论文采用数值计算方法对标量场和规范场在不同情况下的纠缠熵和负性度进行了计算，并使用共变离散化的方法进行空间离散化处理。反热核系数的计算方法也被引入进行对比分析。

Result: 论文计算了标量场和规范场在曲率空间中的纠缠熵和负性度，并通过反热核系数的方法验证了结果的正确性。研究结果表明，与萨德尼克的结果相比，这些new methods显示出更好的适用性和准确性。

Conclusion: 本研究扩展了萨德尼克在平坦空间的情况，提出了计算标量场和规范场在弯曲空间中纠缠熵和负性度的新方法，结果与反热核系数计算结果一致，展示了方法的有效性。该结果为理解量子纠缠在复杂空间中的行为提供了理论支持。
...
### [82] [Gravitational amplitudes in the Regge limit: waveforms, shock waves and unitarity cuts](https://arxiv.org/abs/2601.21687)
*Francesco Alessio,Vittorio Del Duca,Riccardo Gonzo,Emanuele Rosi*

Main category: hep-th

TL;DR: 研究了两个质点与多个引力子散射的高能极限，结合量子和经典贡献，应用于凯尔BLACK repay的超螺旋分析。


<details>
  <summary>Details</summary>
Motivation: 研究高能引力相互作用及散射过程，扩展了Regge理论和BFKL方程的应用范围。

Method: 使用Regge理论、BFKL和量子场论方法，结合指数S-矩阵和冲击波形式化，采用费米速度量子化。

Result: 计算了5PM-2SF下的质量散射振幅，并与凯尔黑洞的超螺旋相联系。

Conclusion: 通过对高能引力和量子场论方法的整合，获得了新的散射幅值信息，对引力波研究有贡献。
...
### [83] [Celestial Energy-Energy Correlation in Yang-Mills Theory and Gravity](https://arxiv.org/abs/2601.21852)
...
### [84] [Defect Relative Entropy](https://arxiv.org/abs/2601.21875)
*Mostafa Ghasemi*
...
<details>
  <summary>Details</summary>
Motivation: 研究者希望探讨缺陷在某种空间内的可区分性，这可能与统计力学或量子场论中的_defects_相关。

Method: 使用了_算子乘积代数_和_ising模型_、_tricritical Ising模型_、_s^u(2)wzw模型_等。

Result: 发现了某些_ topological defects_的相对熵为零，并引入了_ defect relative sectors_的概念。

Conclusion: 这项工作为_ defects_研究提供了新的方法和概念。
...
### [85] [Loops and legs: ABJM amplitudes from $f$-graphs](https://arxiv.org/abs/2601.21980)
*Song He,Yao-Qi Zhang*

Main category: hep-th

TL;DR: ABJM amplitudes can be reconstructed from squared amplitudes using a generating function with permutation symmetry, similar to N=4 SYM. These squared amplitudes include enough information to find individual amplitudes even at odd loops and higher multiplicities.


<details>
  <summary>Details</summary>
Motivation: The non-perturbative bootstrap approach for ABJM amplitudes has been advanced by constructing a generating function for squared amplitudes at $N=n+L$ points. New loop integrand results for ABJM including up to 6 loops for $n=4$ points and 2 loops for 6 points provide evidence for the approach. The method utilized parallels from N=4 SYM, such as using DCI integrands and Yangian invariants, to extract these results.

Method: A generating function with $S_N$ permutation symmetry is employed systematically to reconstruct planar integrands. This involves expressing squared amplitudes as linear combinations of planar $f$-graphs recast as bipartite graphs. The process draws on techniques used in N=4 SYM, such as using BCFW building blocks for tree amplitudes and a suitable basis of DCI integrands for higher-point calculations.

Result: Collaborative work has successfully extracted four-point amplitudes up to 6 loops, six-point integrands at two loops, and eight-point tree amplitudes. These results support the effectiveness of the generating function approach in reconstructing ABJM amplitudes at various loop orders and multiplicities, similar to the role of $f$-graphs in N=4 SYM.

Conclusion: The findings indicate a powerful method for constructing amplitudes in ABJM theory by utilizing squared amplitudes. The success in reconstructing integrands at different loop orders and multiplicities, including matching known results in N=4 SYM where applicable, validate the approach and suggest its general applicability across different representations of scattering amplitudes.
...
### [86] [Short Strings in Three-Dimensional Anti-de Sitter Space: from Weak to Strong Coupling](https://arxiv.org/abs/2601.21992)
*Simon Ekhammar,Nikolay Gromov,Bogdan Stefański,Charles Thull*

Main category: hep-th

TL;DR: 研究通过数值方法解决AdS3×S^3×T^4中R-磁荷的量子谱曲线，从弱到强耦合范围进行分析。强耦合并的时候，能量工程形成平面弦质量等级，最高处采用平方根比例相对于弦张力，并且增加Kaluza-Klein排序在下层。弱耦合状况下，能量由最近邻的贝ted ansatz决定，下层的修正与其他因素有关，但当体积很大时会被抑制住。


<details>
  <summary>Details</summary>
Motivation: 研究者试图理解AdS3空间中弦理论的行为，特别是当R-磁荷存在时的量子性质，这对几何和弦理论的根本特性都有助。

Method: 采用量子谱曲线的数值解法，详细分析了不同耦合下的能量级，非常量化的从弱到强进行计算。

Result: 在强耦合时，能量形成一个 universality class，基准下采用以张力平方根上的尺度；弱耦合时，指数采用最近邻的贝ted ansatz。此外，在下层出现了Kaluza-Klein分裂，这种情况对能量结构有明显的影响。

Conclusion: 通过从弱到强的纵向分析，研究者发现不同耦合下能量工程表现出不同的规律：强耦合出现通用的平方根规模，弱耦合出现指数值修正。这一结果有助于更好地理解AdS/CFT对应的表达空间结构，对理论物理和弦理论的发展有重要的意义。
...
### [87] [Non-Supersymmetric String-String Dualities via Enriques Surfaces](https://arxiv.org/abs/2601.22058)
*Arata Ishige*

Main category: hep-th

TL;DR: 研究提出了关于6维N=2型II和双组超对称理论的非超对称类比，通过将K3曲面的商空间为Enriques曲面来进行构造。从对称II弦在K3曲面上给出了理论，并使用K3的对合构造出规范理论，进而识别出无质量和tachyon谱，提取局部的模空间结构。最后研究将这些构造重新解释为0A和0B弦在Enriques曲面上的场论，并且得出这些场论与最近构造的非对称性超弦不对称商理论更具对偶性。


<details>
  <summary>Details</summary>
Motivation: 该研究试图拓展HEP中关于弦理论和对偶性的理解，为探索更多的弦理论模型和对偶性提供了新的方法和见解。

Method: 该研究填补了在将K3曲面的商空间定义为Enriques曲面进行理论构造的方法空白，特别是通过使用K3的对合构造规范理论，并对这些理论的无质量和tachyon谱进行了分析。

Result: Mapping在这种构造下，得到了一系列关于非超对称的6维理论，建立了一个新的理论框架，用于对诸如II和双组超对称理论之间对偶性的新探索。

Conclusion: 该研究贡献了一种新的方法来构造非超对称的弦理论模型，并揭示了它们与非对称性对称的orbifold理论之间的对偶性。这为弦论中对偶性的多样性提供了前所未有的视角，推动了理论的发展和探索。
...
### [88] [A Reverse Black Hole Information Problem](https://arxiv.org/abs/2601.22077)
*Jan de Boer,Andrew Rolph,Jildou Hollander*

Main category: hep-th

TL;DR: 研究了AdS/CFT中黑洞的形成、检测和粗化，强调了边界幺正性和 bulk 混合态 Hawking 辐射之间的张力。通过碰撞 bulk 粒子波包在超 Planckian 能量，构造了 CFT 状态对应于黑洞形成和蒸发。提出边界探测器，能够区分微正则系综中小 AdS 黑洞和其他状态。研究了不同的粗化 prescription，在 CFT 状态上进行平均、哈密顿量和时间窗口，比较它们的纯净度与 bulk semiclassical 描述的预期。结果阐明了 semiclassical 黑洞行为如何从 exact 单一 Unitary 动态的系综平均产生，并对 single-sided 黑洞信息问题中的粗化有了更好的理解。


<details>
  <summary>Details</summary>
Motivation: 研究黑洞的形成、检测和粗化在 AdS/CFT 中的行为，有助于理解量子引力和 holography 的基本原理。

Method: 通过碰撞 bulk 粒子波包在超 Planckian 能量构造 CFT 状态，研究:black hole formation and evaporation; propose boundary probes to distinguish small AdS black holes; investigate different coarse-graining prescriptions on CFT state; compare their purities with bulk semiclassical description。

Result: 澄清了 semiclassical 黑洞行为如何从 exact 单一 Unitary 动态的系综平均产生; 对 single-sided black hole information problem 中的粗化有了更好的理解。

Conclusion: 在 AdS/CFT 中，边界幺正性和 bulk 混合态 Hawking 辐射之间的张力可以通过 Coarse-graining 解释; 边界探测器和粗化 prescription 可以区分小 AdS 黑洞和其他状态; 我们的结果为理解单边黑洞信息问题中的粗化提供了新的视角。
...
### [89] [Convergent sum of EFT corrections to Schwarzschild metric requires UV locality](https://arxiv.org/abs/2601.22105)
*Yang Liu,Alexey S. Koshelev,Anna Tokareva,Ziyue Zhu*

Main category: hep-th

TL;DR: 研究考虑了爱因斯坦场论框架中对真空黑洞解的修正，重点分析了施瓦茨西尔德解，并发现这些修正可以被所有阶次的导数和曲率项所总结，并提出了这种求和方法的收敛条件。


<details>
  <summary>Details</summary>
Motivation: 该研究是为了更加全面地理解广义相对论框架中的黑洞解及其修正，从而为量子引力理论提供新的见解。

Method: 研究采用了有效场论的方法，通过微扰展开EFT的系数，对施瓦茨西尔德解进行了系统性地分析，并将这些修正全部求和。

Result: 研究发现主导的修正能够通过所有阶次的导数和曲率项进行总结，并指出这种求和方法仅在局部可话化理论中收敛，从而揭示了引力散射振幅性质与引力理论扰动论处理之间的直接联系。此外，对数形式因子在四维时空中1环有效作用下的修正效应，发现这些修正效应比树状微扰算子效应更为显著。

Conclusion: 这些发现为扩展这一框架以考虑施瓦茨西尔德解的其他修正提供了基础，例如对库尔解的修正。
...
### [90] [Firewalls in the non-perturbative bulk Hilbert space of JT gravity](https://arxiv.org/abs/2601.22121)
*Hamed Zolfi*

Main category: hep-th

TL;DR: 这是一个关于量子霍金辐射中的黑洞与白洞隧道现象的研究论文。研究中使用了高亏格几何和非扰动内积来分析此过程，并得出了洞口熵与产物概率的关系。


<details>
  <summary>Details</summary>
Motivation: 作者受到阿·囚One普(AdS/CFT)对应性和早期霍金辐射相关研究的启发，希望通过分析黑洞蒸发中的量子效应，探索其基础理论框架。

Method: 采用了量子引力和规范场论的结合方法，运用了微分几何和量子力学原理来建模和计算。特别地，运用了波函数的路径积分方法，计算了不同几何概率随时间的演化。

Result: 在计算过程中，发现当时间到达t≈e^{S(E)}时，存在计算错误。

Conclusion: 这一研究发现，当时间到达特殊阈值时， holes生成概率趋向于常数值，这意味着在这个时间点附近，概率倾向于稳定，可能表明时间基础的量子结构在这里发生了变化。但进一步的数据和模型模拟是必要的，这个结果尚不完全可靠。
...
### [91] [Quantum fluctuations in hydrodynamics and quantum long-time tails](https://arxiv.org/abs/2601.22140)
*Akash Jain*
...
### [92] [Neural S-matrix bootstrap II: solvable 4d amplitudes with particle production](https://arxiv.org/abs/2601.22145)
...
### [93] [From Beam to Bedside: Reinforcing Domestic Supply of $^{99}$Mo/$^{99m}$Tc using Novel High-Current D+ Cyclotrons for Compact Neutron Generation and $^{99}$Mo Production](https://arxiv.org/abs/2601.20997)
*Jarrett Moon,Daniel Winklehner,Jose Alonso,Claire Huchthausen,David McClain,Janet Conrad*

Main category: physics.acc-ph

TL;DR: 该论文提出了一种基于质子（Deuterons）的轻核聚变体系的生成Tc-99m的方法，可以在非核反应堆环境中独立进行，避免了铀资源的依赖性和核安全风险。


<details>
  <summary>Details</summary>
Motivation: 随着对放射性诊断技术的依赖增加，确保核素如Tc-99m的稳定供应至关重要。传统上，这些核素是从核反应堆获取，但核反应堆的建设和运营存在诸多安全隐患。因此，开发一种无需依赖核反应堆的新供应途径具有重要的现实意义。

Method: 该论文采用了一种基于轻核聚变的体系，使用Deuterons（即质子的同位素氘）作为燃料，并使用环流器（cyclotron）将其加速到高能。环流器的结构设计基于IsoDAR实验，具有更高的电流输出能力。系统采用薄层铍靶（beryllium）来捕获这些质子，从而产生中子。中子可以用于制造Tc-99m。

Result: 模拟结果显示，该系统可以在依靠轻元素材料和普通电源的情况下，产生足够的中子来生产Tc-99m。具体的模拟结果表明，一个包括轻薄铍靶的系统可以在每秒产生约10^13个中子。这意味着该系统能够在医院等非核设施中独立生产所需的放射性核素，从而减少了对核反应堆的依赖性。此外，使用Deuterons的优点在于它们的能量较低，这可以显著减少产生 neutron activation 的风险，从而提高安全性和减少了放射性废物的产生。

Conclusion: 该研究提供了一种可能取代核反应堆为Tc-99m等放射性核素提供稳定供应的新方法。文章的方法和结果为这种新系统的可行性和安全性提供了重要的证据。该方法具有潜在广泛的医疗应用潜力，特别是为不想依赖核反应堆或对外供核资源的医疗体系提供保障，值得进一步的研究和验证。
...
### [94] [Description of electromagnetic fields in inhomogeneous accelerating sections. IV couplers](https://arxiv.org/abs/2601.21809)
*M. I. Ayzatsky*

Main category: physics.acc-ph

TL;DR: 作者提出了一种将耦合元件整合到广义耦合模式理论中的新方法。他们使用了简单的模型，将结构波导与外部RF电源和负载通过环路和 transmission lines 连接。尽管结构简单，但建立了一个复杂的 coupled integro-differential system（ Barbashin 类型，带有退化核）。由于退化核，这个系统被简化为三个独立的常微分方程组。基于开发的解析模型，他们编写了一个用于匹配环路耦合器的计算机代码，用于加速器X带的均匀加速截面。计算结果表明，不经额外匹配，输入反射系数为8E-3。


<details>
  <summary>Details</summary>
Motivation: This paper introduces a new method for incorporating coupling elements into generalized coupled mode theory. It demonstrates how even a simple model can lead to complex systems of equations and highlights the effectiveness of reducing these systems to simpler differential equations for easier solution.

Method: The authors used a generalized coupled mode theory to model the interaction of a structured waveguide with an external RF power source and load. They employed loops and transmission lines to couple the waveguide with the source and load. The coupled integro-differential system was simplified by leveraging the degenerate kernels, leading to three independent differential equation systems.

Result: The study successfully reduced the complex coupled system to three independent differential equations, enabling easier and more efficient solutions. The analytical model was used to develop computer code for matching loop couplers, achieving an input reflection coefficient of 8E-3 without additional matching.

Conclusion: This research provides a practical approach for incorporating coupling elements into generalized coupled mode theory, demonstrating its effectiveness in reducing complex systems to simpler differential equations with applications in microwave engineering.
...
### [95] [Quench spectroscopy of amplitude modes in a one-dimensional critical phase](https://arxiv.org/abs/2601.20926)
*Hyunsoo Ha,David A. Huse,Rhine Samajdar*

Main category: cond-mat.str-el

TL;DR: 研究发现，在1+1维XXZ自旋链的无能隙相中，出现了一个类似于Higgs的振幅模式。这与常规情况下的对称性破坏不同，是保持对称性下的一种阻尼激发态。通过非平衡短程跳跃光谱方法，证明了这一模式在其激发后作为U(1)对称观测值的振荡行为。通过数值模拟和贝塔答案分析，阐明了这一模式的微观机制。讨论了在易于平面的量子磁性和可编程量子模拟器中检出该模式的方式。结果显示量子短程跳跃可以作为一个强大的工具来探测集体激发，超出了线性响应的范畴。


<details>
  <summary>Details</summary>
Motivation: 研究者可能出于理解量子系统的复杂性，寻找新现象，或开发新的量子技术而进行。

Method: 研究者采用了数值模拟和贝塔答案分析两种方法。

Result: 报告的主要结果包括振幅模式的观察，其行为及其源自特定的弦激发态。

Conclusion: 研究提供了量子短程跳跃作为探测集体激发的强大工具，并扩展了线性响应之外的分析视野。
...
### [96] [When does a lattice higher-form symmetry flow to a topological higher-form symmetry at low energies?](https://arxiv.org/abs/2601.20935)
...
### [97] [Self-dual Higgs transitions: Toric code and beyond](https://arxiv.org/abs/2601.20945)
...
### [98] [Spin-orbit-induced Instability and Finite-Temperature Stabilization of a Triangular-lattice Supersolid](https://arxiv.org/abs/2601.20963)
*Seongjun Park,Sung-Min Park,Yun-Tak Oh,Hyun-Yong Lee,Eun-Gook Moon*

Main category: cond-mat.str-el

TL;DR: The paper investigates the effects of spin-orbit coupling (SOC) on a geometrically frustrated triangular-lattice magnet, revealing a mechanism where thermal fluctuations can stabilize supersolidity against SOC.


<details>
  <summary>Details</summary>
Motivation: The paper explores the role of spin-orbit coupling in stabilizing quantum phases like spin supersolids, challenging the conventional understanding that SOC destabilizes such phases by gapping their low-energy modes.

Method: The research uses spin-wave theory and infinite density-matrix renormalization group (iDMRG) simulations to map out the effects of SOC on the phase diagram of the frustrated triangular-lattice magnet.

Result: The key findings include that while infinitesimally weak SOC can drive a zero-temperature instability of the supersolid, thermal fluctuations can counteract SOC to stabilize supersolidity at non-zero temperatures. Additionally, the system transitions into distinct magnetic orders, such as a skyrmion lattice, under stronger SOC, completing a unified phase diagram.

Conclusion: This study provides a deeper understanding of how SOC and thermal fluctuations interplay to stabilize or destabilize quantum phases in geometrically frustrated systems, with implications for real materials exhibiting such properties
...
### [99] [Competing Ordering Modes in the Distorted Quantum Kagome Material Clinoatacamite Cu$_2$Cl(OH)$_3$](https://arxiv.org/abs/2601.21024)
...
TL;DR: 研究了磁性材料clinoatacamite的性质，为量子自旋液候选体herbertsmithite提供线索，发现了在零磁场条件下低温下的复杂相变序列，提出了这些现象源于受约束的磁性磁波的竞争模式，可能导致无磁性拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 材料科学，特别是量子自旋磁性领域的研究，具有重要的理论和应用价值。

Method: 密度泛函理论计算，结合热力学测量，包括μ子自旋旋转和中子散射。

Result: 揭示了clinoatacamite属于变形的kagome反铁磁体，发现了低温下的复杂相变序列，并提出了无磁性拓扑结构的可能性。

Conclusion: clinoatacamite的研究丰富了自旋 requests的复杂性，提供了理解复杂磁性材料的insight。
...
### [100] [Ultrafast Decoherence of Charge Density Waves in K$_{0.3}$MoO$_{3}$](https://arxiv.org/abs/2601.21030)
...
### [101] [Classification of non-Fermi liquids and universal superconducting fluctuations](https://arxiv.org/abs/2601.21047)
...
### [102] [Influence of Markovianity and self-consistency on time-resolved spectral functions of driven quantum systems](https://arxiv.org/abs/2601.21088)
...
### [103] [Interacting type-II semi-Dirac quasiparticles](https://arxiv.org/abs/2601.21098)
*Mohamed M. Elsayed,Taras I. Lakoba,Valeri N. Kotov*

Main category: cond-mat.str-el

TL;DR: The paper discusses the properties of Type-II semi-Dirac fermions in two dimensions, which are proposed to describe topologically nontrivial low-energy excitations in titanium/vanadium oxide heterostructures. The authors use Hartree-Fock, renormalization group, and RPA techniques to show that the quasiparticle spectrum evolves from anisotropic Dirac dispersion to a semi-Dirac 'boomerang' shape as energy increases, with the density of states changing from linear to power law with exponent 1/3. The crossover scale is determined by interaction strength and the effective Hamiltonian. The study reveals critical behavior in physical observables like Landau levels, which exhibit scale dependence described by critical exponents. The findings have implications for understanding the interplay between interactions and topology in these systems.


<details>
  <summary>Details</summary>
Motivation: The paper explores the behavior of Type-II semi-Dirac fermions under long-range electron-electron interactions in a heterostructure. The motivation stems from the potential of these quasiparticles for applications in quantum technologies, necessitating a detailed understanding of their properties under various conditions.

Method: The authors employ a combination of Hartree-Fock, renormalization group, and random phase approximation (RPA) techniques to analyze the quasiparticle spectrum and density of states. These methods allow for a comprehensive study of the system's response to interactions and energy scaling.

Result: The research reveals that the quasiparticle spectrum and density of states exhibit significant sensitivity to long-range interactions. At low energies, the system displays anisotropic Dirac dispersion, but as energy increases, it transforms into a semi-Dirac 'boomerang' shape. The density of states evolves from a linear dependence to a power law with exponent 1/3. The crossover energy scale is influenced by the interaction strength (α = e²/ħv) and the effective Hamiltonian. Furthermore, the study identifies critical behavior in physical observables such as Landau levels, which exhibit scale dependence governed by critical exponents.

Conclusion: The paper concludes that the interaction strength and the effective Hamiltonian significantly control the evolution of Type-II semi-Dirac fermions. The observed critical behavior in physical observables provides a deeper understanding of the interplay between topology and electron-electron interactions in two-dimensional systems. These findings are crucial for advancing the theoretical framework and practical applications of such materials in quantum technologies.
...
### [104] [Non-invertible translation from Lieb-Schultz-Mattis anomaly](https://arxiv.org/abs/2601.21625)
...
### [105] [Emergent Spatial Textures from Interaction Quenches in the Hubbard Model](https://arxiv.org/abs/2601.22053)
*Sankha Subhra Bakshi,Gia-Wei Chern*

Main category: cond-mat.str-el

TL;DR: interaction quenches in the half-filled hubbard model show that spatially homogeneous dynamics is typically unstable and leads to inhomogeneous states with spatial differentiation and mott-like domains forming over time.


<details>
  <summary>Details</summary>
Motivation: understanding the behavior of strongly correlated electron systems under interaction quenches is crucial for exploring nonequilibrium many-body dynamics, particularly the role of spatial structure formation.

Method: real-space time-dependent gutzwiller framework is used to investigate the dynamics of the half-filled hubbard model after interaction quenches.

Result: heterogeneous dynamics emerge from homogeneous initial states, with spatially differentiated regions and the slow coarsening of mott-like domains, highlighting the generic nature of spatial self-organization in far-from-equilibrium systems.

Conclusion: our findings demonstrate that spatial structures significantly impact nonequilibrium dynamics, revealing a fundamental limitation of spatially homogeneous theories and offering insights into the self-organization of correlated matter.
...
### [106] [Inverted anisotropy of the partially screened magnetic impurity](https://arxiv.org/abs/2601.22078)
...
### [107] [End-to-End Fidelity Analysis of Quantum Circuit Optimization: From Gate-Level Transformations to Pulse-Level Control](https://arxiv.org/abs/2601.20871)
*Rylan Malarchick*
...
### [108] [An Ontological Interpretation of Photon Wave-Particle Duality via Complex-Space Trajectories](https://arxiv.org/abs/2601.20872)
...
### [109] [Photonic Links for Spin-Based Quantum Sensors](https://arxiv.org/abs/2601.22011)
*M. Reefaz Rahman,Karsten Schnier,Ryan Goldsmith,Benjamin J. Lawrie,Joseph M. Lukens,Seongsin M. Kim,Patrick Kung*

Main category: quant-ph

TL;DR: A new RF-over-fiber (RFoF) platform for optically accessible spin qubits has been developed, with an efficiency of -0.7 dBm at 2.90 GHz, enabling low-noise, thermally isolated, and cryo-friendly quantum systems.


<details>
  <summary>Details</summary>
Motivation: The motivation for this research stems from the challenge of achieving Scalable spin-based quantum systems. Conventional spin qubits face limitations in scalability due to direct microwave delivery, which introduces noise and other issues. This work aims to address such limitations by introducing the RFoF platform.

Method: The study employed an RFoF platform that combines an electro-optically modulated telecom laser for transmitting microwave signals over fiber with a high-speed photodiode for RF waveform recovery, enabling spin transitions in nitrogen-vacancy centers in diamond.

Result: The RFoF platform achieved an impressive efficiency of 1.81% at 2.90 GHz, resulting in an output power of -0.7 dBm. This significant improvement suggests a promising development in scalable quantum technologies.

Conclusion: This breakthrough in RFoF technology provides a pathway toward more robust and scalable quantum systems, bridging the gap between traditional spin-based quantum sensing and emerging distributed quantum technologies.)
...
### [110] [A general framework for interactions between electron beams and quantum optical systems](https://arxiv.org/abs/2601.21385)
...
### [111] [Novel method for evaluating the eigenvalues of the Heun differential equation with an application to the Breit equation](https://arxiv.org/abs/2601.20873)
...
### [112] [AI Optimized Routing and Resource Allocation for Quantum Enabled Non Terrestrial Industrial Networks](https://arxiv.org/abs/2601.20877)
...
### [113] [Fast, accurate, and predictive method for atom detection in site-resolved images of microtrap arrays](https://arxiv.org/abs/2502.08511)
...
### [114] [Micro-mobility dispatch optimization via quantum annealing incorporating historical data](https://arxiv.org/abs/2601.20887)
*Takeru Goto,Masayuki Ohzeki*

Main category: quant-ph

TL;DR: 该论文提出了一种基于量子退火器的微移动车辆调度方案，利用历史数据优化调度效率，通过对比经典方法和量子退火器性能，展示了量子退火器的优势。


<details>
  <summary>Details</summary>
Motivation: 微移动车辆服务在城市交通中具有重要应用，但调度方案一直是该领域的难点，该研究通过结合量子计算方法，提出了一种高效的调度方案。

Method: 通过将调度问题建模为QUBO问题，并结合历史数据和贝叶斯方法优化，该研究利用量子退火器求解。

Result: 实验结果表明，该调度方案在优化效率和处理复杂度方面均优于传统方法，量子退火器在调度方案中显示出明显优势。

Conclusion: 该研究展示了量子退火器在微移动车辆调度中的有效应用，为未来的交通调度方案提供了新的思路。
...
### [115] [The quantum sky of Majorana stars](https://arxiv.org/abs/2601.20922)
*L. L. Sanchez-Soto,A. B. Klimov,A. Z. Goldberg,G. Leuchs*

Main category: quant-ph

TL;DR: Majorana stars offer an intuitive method to visualize quantum states. They provide insights into quantum structure, symmetries, and entanglement, serving as a bridge between abstract algebra and geometric intuition. The paper surveys the development and applications of the Majorana constellation in modern quantum information.


<details>
  <summary>Details</summary>
Motivation: The motivation for studying Majorana stars comes from the need for intuitive visualization tools in quantum mechanics, particularly in understanding complex quantum states and their properties.

Method: The method used in the paper involves a brief survey of the development and applications of the Majorana constellation. It explores how these geometric representations are relevant in modern areas of quantum information by connecting abstract algebraic formulations with intuitive geometrical interpretations.

Result: The key results include the successful demonstration that Majorana stars are a viable and intuitive method for visualizing quantum states, with applications in understanding symmetries, entanglement, and their relevance in quantum information theory. The paper highlights the growing importance of the Majorana constellation in modern quantum research.

Conclusion: In conclusion, the paper establishes the Majorana constellation as a valuable tool for visualizing and analyzing quantum states, with broad applications in quantum information science. It emphasizes the continued relevance and potential for future advancements in this geometric approach to quantum mechanics.
...
### [116] [Double-Bracket Master Equations: Phase-Space Representation and Classical Limit](https://arxiv.org/abs/2601.20925)
...
### [117] [Polaron-Polaritons in Subwavelength Arrays of Trapped Atoms](https://arxiv.org/abs/2601.21062)
...
TL;DR: 均匀分布的原子晶格易受机械振动干扰，实验发现机械运动可操控光-物质相互作用的实质。


<details>
  <summary>Details</summary>
Motivation: 探索光-力学系统中的相互作用机制，为量子调控和新 chilled  设备开发奠定理论基础。

Method: 实验中利用控温器式原子陷阱和光栅阵列存储原子，结合光和机械调谐推动相互作用研究。

Result: 机械运动显著提升亚半径激发状态的寿命，暗态传输异常稳定，在特定动量激发条件下的高反射率约为99%，证明了动量的操控。

Conclusion: 机械运动可以被有效地控制和利用，在光-物质相互作用中具有广泛应用前景。
...
### [118] [Entangling logical qubits without physical operations](https://arxiv.org/abs/2601.20927)
...
TL;DR: 文章提出了一种新的量子容错编码--- phantom codes, 该编码通过重新排列物理位进行编译,在代码块内所有逻辑位之间实现了纯粹的纠缠操作,能够完美保持 fidelity,没有任何时间和空间的开销。文章还系统性地研究了这种编码,通过数值和解析方法找到了所有可能的 codes, 并构造了高距离的 phantom-code 家族。实验结果表明, phantom codes 在 GHZ 状态制备和量子模拟任务中显著优于表面码,逻辑错误率下降了一个数量级,同时 qubit 额外开销有限。未来,该工作提供了量子计算可扩展性改进的潜在途径,并为探索更一般的量子错误纠正编码结构提供了工具。


<details>
  <summary>Details</summary>
Motivation: 提升量子计算的可扩展性是当下的一个重要研究方向。现有的量子位操作和测量精度较低,导致容错计算面临巨大挑战。 phantom codes 作为一种新的错误纠正码,通过重新排列物理位进行编译,避免了传统的错误纠正步骤,从而实现了纯粹的逻辑纠缠操作,展示了在逻辑错误率和空间时间开销方面的优势。研究这一编码有助于推动量子计算的发展。

Method: 文章通过数值和解析方法研究了 phantom codes, 包括系统地枚举 CSS 码和利用 SAT-based 方法发现新的实例。此外,还通过构造基于量子里德-毛勒码和 qudit 码的二进制化方法来扩展高距离的 phantom-code 家族。文章进一步通过噪声仿真验证了 phantom codes 在 GHZ 状态制备和量子模拟任务中的优越性,并对比了其与表面码的性能。

Result: 实验结果表明 phantom codes 在 GHZ 状态制备和量子模拟任务中显著优于表面码。在相同的 qubit 额外开销下, phantom codes 的逻辑错误率下降了一个数量级。这表明 phantom codes 作为一个新的容错编码方案,在处理密集局部纠缠结构的任务中具有明显优势。

Conclusion: 文章提出了一种新的量子容错编码--- phantom codes, 该编码能够实现全逻辑位之间的纯粹纠缠操作,并避免了传统错误纠正步骤的开销。研究结果表明 phantom codes 在 GHZ 状态制备和量子模拟任务中显著优于表面码。未来,该工作为探索更复杂的量子错误纠正编码结构提供了方法论框架,并且为量子计算的实用开发提供了新方向。
...
### [119] [Non-Equilibrium Phase Transition in a Boundary-Driven Dissipative Fermionic Chain](https://arxiv.org/abs/2601.20938)
*Hao Chen,Wucheng Zhang,Manas Kulkarni,Abhinav Prem*

Main category: quant-ph

TL;DR: 文章展示了如何利用边界局部化的周期性驱动（Floquet）在不互相作用的费米链中诱导长期相关性，即使孤立的 bulk在平凡的绝缘相中。


<details>
  <summary>Details</summary>
Motivation: 研究者可以通过在开放量子系统中使用局部驱动实现长距离相关性，这可能对该领域的发展有重要意义。

Method: 研究者通过数值模拟和理论分析探讨了驱动频率如何跨越 bulk能量间隙，以及边界驱动介入粒子-空穴激发的传播和长期相关性的产生。

Result: 研究表明，当驱动物跨越粒子-空间隙时，诱导的长期有序随 bulk配对势按幂律增长 ($χ	ext{~}γ^2$)。这种现象表明孤立的 bulk中的保持绝缘状态不会阻止通过边界驱动实现的长距离关联的存在。

Conclusion: 该研究显示，通过边界驱动实现长距离关联的可能性，这对于量子热力学和无序系统研究具有重要意义。
...
### [120] [Spatial superposition for a two-dimensional matter-wave interferometer in an inverted harmonic potential with gyroscopic rotational stability](https://arxiv.org/abs/2601.20949)
...
### [121] [A levitated nano-accelerometer sensitized by quantum quench](https://arxiv.org/abs/2601.21119)
*M. Kamba,S. Otabe,K. Funo,T. Sagawa,K. Aikawa*

Main category: quant-ph

TL;DR: We report a nanoscale accelerometer utilizing quantum ground state dynamics and demonstrate sensitivity enhanced by rapid quenching of the trapping potential. Our results align with quantum simulations based on the Fisher information.


<details>
  <summary>Details</summary>
Motivation: Nanoscale sensors are crucial for detecting minute movements, such as in space-based systems or biological assays. Achieving high sensitivity in these devices remains a significant challenge, and innovative approaches are necessary to overcome these limitations.

Method: We developed a nanoscale accelerometer based on the nonequilibrium dynamics of a nanoparticle in a quantum ground state by performing rapid quenching of the trapping potential. The method involved numerical simulations using quantum Langevin equations and compared the results to theoretical limits based on quantum Fisher information.

Result: The experimental results show that rapid quenching enhances the device's sensitivity, with measurements of the sensitivity being in line with quantum simulations. The approach approaches the theoretical limit set by the quantum Fisher information.

Conclusion: This work demonstrates a novel method to enhance the sensitivity of nanoscale accelerometers by exploiting quantum ground state dynamics and rapid quenching, paving the way for more efficient inertial sensing technologies.
...
### [122] [Parametric Quantum State Tomography with HyperRBMs](https://arxiv.org/abs/2601.20950)
...
### [123] [Quantum metrology enhanced by effective time reversal](https://arxiv.org/abs/2601.20952)
...
### [124] [3D imaging of the biphoton spatiotemporal wave packet](https://arxiv.org/abs/2601.21250)
*Yang Xue,Ze-Shan He,Hao-Shu Tian,Qin-Qin Wang,Bin-Tong Yin,Jun Zhong,Xiao-Ye Xu,Chuan-Feng Li,Guang-Can Guo*

Main category: quant-ph

TL;DR: This paper presents a method to observe the 3D spatiotemporal structure of quantum light fields created by biphotons.


<details>
  <summary>Details</summary>
Motivation: Understanding photon degrees of freedom (DoFs) is crucial for quantum information processing tasks. However, measuring the quantum counterparts of these DoFs has been technically challenging, particularly due to the weak photon flux. This paper addresses these limitations by introducing a new method.

Method: The authors developed a self-referenced, all-optical, high-efficiency 3D imaging technique called '3D imaging of photonic wave packets.' This method allows for comprehensive characterization of the spatiotemporal structure of quantum light fields.

Result: The method successfully observed the spatial-spatial, spectral-spectral, and spatiotemporal correlations of biphotons generated via spontaneous parametric down-conversion. This revealed rich local and nonlocal spatiotemporal structures in quantum light fields.

Conclusion: The new method advances the understanding of nonlinear quantum optics dynamics and enhances the potential applications of photons in quantum communication and computing.
...
### [125] [Dispersive Microwave Sensing for Quantum Computing with Floating Electrons](https://arxiv.org/abs/2601.21313)
*Yiran Tian*
...
### [126] [Universal Topological Gates from Braiding and Fusing Anyons on Quantum Hardware](https://arxiv.org/abs/2601.20956)
...
TL;DR: 研究者在非阿贝尔拓扑有序相中引入了量子计算的基本原则，展示了通过任意子的融合规则可以实现通用量子计算。


<details>
  <summary>Details</summary>
Motivation: 这是为了突破传统的阿贝尔拓扑有序相的局限性，寻找更强大的量子计算机制。

Method: 利用Quantinuum的H2量子处理器，通过融合规则来编码量子信息并实现量子门操作。

Result: 成功实现了通用的拓扑量子门集，并通过魔性态的 preparation 进一步验证了计算的可能性。

Conclusion: 非阿贝尔拓扑有序相为量子计算提供了一种简洁而强大的新框架。
...
### [127] [Active polarization stabilization of fields in an optical fiber for protective measurements](https://arxiv.org/abs/2601.20991)
...
### [128] [Rapid high-temperature initialisation and readout of spins in silicon with 10 THz photons](https://arxiv.org/abs/2601.21880)
...
### [129] [Building Holographic Entanglement by Measurement](https://arxiv.org/abs/2601.21065)
*Jonathan Jeffrey,Lucien Gandarias,Monika Schleier-Smith,Brian Swingle*

Main category: quant-ph

TL;DR: 提出了一个框架，用于通过Bulk几何中的极小曲面控制量子态的纠缠结构，通过Ryu-Takayanagi公式解释了纠缠性，并在光子和冷原子平台上实现了。


<details>
  <summary>Details</summary>
Motivation: 研究量子态的纠缠结构及其与Bulk几何的关系，特别是在量子引力模型中的应用。

Method: 通过选择一个Bulk几何，离散化为节点和边的网络，测量内部节点以产生边界上的纯态。

Result: 实例表明，在双曲盘和虫洞几何下，生成的量子态的纠缠结构与Ryu-Takayanagi公式预测的一致。

Conclusion: 提供了一个简洁的实验方案，利用高斯操作和测量，可以在光子和冷原子平台验证量子重力预言。
...
### [130] [Dynamical Casimir effect under the action of gravitational waves](https://arxiv.org/abs/2601.21075)
*Gustavo de Oliveira,Thiago Henrique Moreira,Lucas Chibebe Céleri*

Main category: quant-ph

TL;DR: 研究发现，在平面引力波作用下，一个可动镜腔会经历动态Casimir效应，通过参数放大效应导致粒子数量呈指数级增长。


<details>
  <summary>Details</summary>
Motivation: 动态效应和引力波在量子场论中的应用一直是研究热点，寻找粒子产生机制具有重要理论意义。

Method: 使用量子场论和参数放大理论，分析动镜反射、振荡运动与引力波相互作用。

Result: 在特定振荡条件下，粒子数量显著增加，识别了参数放大条件。

Conclusion: 结果显示平面引力波驱动的动态Casimir效应能有效提升粒子产生率，具有潜在应用。
...
### [131] [Hidden-Field Coordination Reveals Payoff-Free Quantum Correlation Structure in Decentralized Coordination](https://arxiv.org/abs/2601.21139)
*Sinan Bugu*

Main category: quant-ph

TL;DR: 在无监督的领域中研究多智能体协调下的分散性相互作用。


<details>
  <summary>Details</summary>
Motivation: 探索在无通信约束下的策略优化。

Method: 通过HFC模型分析协调作用。

Result: 发现增加总相关性以促进相互复制行为可以提高通站碰撞的概率。

Conclusion: 量子策略在概率几何上为反协调提供了支持。
...
### [132] [Efficient Algorithms for Weakly-Interacting Quantum Spin Systems](https://arxiv.org/abs/2601.21140)
*Ryan L. Mann,Gabriel Waite*

Main category: quant-ph

TL;DR: 研究提出了高效的算法来计算弱相互作用量子自旋系统的平衡函数，还提供了一个近似采样方案来估计热分布。这些都是基于聚类展开法和从采样近似到计数近似的标准简化。


<details>
  <summary>Details</summary>
Motivation: 量子自旋系统在量子物理和统计力学中具有重要意义，而对弱相互作用系统的平衡函数和热分布的研究有助于理解系统的宏观行为。

Method: 该研究采用了聚类展开法，并结合标准的从采样近似到计数近似的简化方法。

Result: 研究的结果是，他们提出了一个多项式时间的近似方案来计算平衡函数，并提供了近似采样方案来估计热分布。

Conclusion: 这项研究为研究弱相互作用量子自旋系统的平衡行为提供了一个有效的计算框架。
...
### [133] [Community detection in network using Szegedy quantum walk](https://arxiv.org/abs/2601.21152)
*Md Samsur Rahaman,Supriyo Dutta*

Main category: quant-ph

TL;DR: 该论文提出了一种利用Szegedy量子 walk进行社区检测的方法，并通过多个图进行了测试。


<details>
  <summary>Details</summary>
Motivation: 社区检测在复杂网络理论中具有重要意义，尤其是用于分析社交网络。

Method: 利用Szegedy的量子 walk方法进行社区检测，通过计算极限概率分布来确定顶点所属社区。

Result: 实验结果表明该方法在检测社区表现良好，适用于各种网络。

Conclusion: 该方法提供了一种有效的量子计算社区检测方法，为网络分析提供了新思路。
...
### [134] [Reflecting boundary induced modulation of tripartite coherence harvesting](https://arxiv.org/abs/2601.21240)
*Shu-Min Wu,Xiao-Ying Jiang,Xiang-Yue Yu,Zhihong Liu,Xiao-Li Huang*

Main category: quant-ph

TL;DR: This paper examines the extraction of quantum coherence via three static Unruh-DeWitt detectors interacting with a scalar field near a reflecting boundary. The key findings are: reducing the separation between detectors and the boundary degrades quantum coherence, while the same effect preserves or amplifies entanglement. Using non-identical energy gaps, coherence extraction is hindered, but entanglement harvesting becomes more efficient and persists over a wider range of separations. Quantum coherence is more accessible and robust than entanglement, though entanglement exhibits richer behavior, which can be enhanced via boundary effects and detector geometry variations.Orthogonal configurations outperform parallel ones in coherence harvesting, highlighting the role of geometry.


<details>
  <summary>Details</summary>
Motivation: This research is driven by the fundamental interest in understanding how quantum resources like coherence and entanglement can be effectively harvested and manipulated in structured quantum field configurations, particularly in the presence of boundaries that can influence quantum states through boundary effects. The practical implications could relate to quantum information processing and quantum communication technologies, where controlling and enhancing such quantum resources is essential.

Method: The study employs theoretical analysis of quantum field theory, particularly focusing on the Unruh-DeWitt detector model, examining the effects of detector separation and configuration on quantum coherence and entanglement. Simulations or analytical derivations are likely used to model the quantum field and detectors' interactions, with a focus on varying parameters such as energy gaps and detector arrangements.

Result: The main results reveal a clear hierarchy between quantum coherence and entanglement as quantum resources. Coherence is found to degrade more rapidly with decreasing separation between detectors and the boundary, whereas entanglement shows greater resilience and persistence. Additionally, the use of non-identical energy gaps not only hinders coherence extraction but also enhances entanglement harvesting efficiency and extends the range over which entanglement can be maintained. The paper also highlights that orthogonal detector configurations outperform parallel configurations in terms of coherence extraction efficiency, emphasizing the role of geometry in the harvesting process.

Conclusion: This paper provides a comprehensive analysis of quantum coherence and entanglement harvesting mechanisms using static Unruh-DeWitt detectors. The findings demonstrate that quantum coherence is more readily accessible and robust against certain configurations compared to entanglement, while entanglement exhibits richer behavior that can be enhanced through boundary effects and detector non-uniformity. The results contribute to a deeper understanding of quantum resource extraction and management in structured quantum field environments, with implications for quantum technology applications.
...
### [135] [Sampling methods to describe superradiance in large ensembles of quantum emitters](https://arxiv.org/abs/2601.21254)
*Daniel Eyles,Emmanuel Lassalle,Adam Stokes,Ramón Paniagua-Domínguez,Ahsan Nazir*

Main category: quant-ph

TL;DR: 在大型量子发光体阵列中，研究了超辐射现象的理论工具，提出了两种近似数值采样方法，并引入了偏差校正，显著提高了预测质量。这些方法适用于嵌入电磁环境的发光体阵列，能够提供上界和下界。最佳方法选择取决于发光体总数，使得两种方法在重要范围内能提供准确描述。


<details>
  <summary>Details</summary>
Motivation: 理解大型量子发光体阵列中的超辐射现象对量子光学和量子信息科学具有重要意义。

Method: 在电磁环境中嵌入发光体阵列的数值模拟方法，给出了对$g^{(2)}(t,0)$的上界和下界。

Result: 通过偏差校正显著提升了预测质量。

Conclusion: 为研究超辐射现象提供了新的理论工具。
...
### [136] [Localization and scattering of a photon in quasiperiodic qubit arrays](https://arxiv.org/abs/2601.21263)
*Xinyin Zhang,Yongguan Ke,Zhengzhi Peng,Zuorui Chen,Wenjie Liu,Li Zhang,Chaohong Lee*

Main category: quant-ph

TL;DR: 研究了单光子在准周期间隔 Array中被波导捕获的局域化和散射性质。准周期增强了，导致局域化超放射态在共振频率附近出现，形成连续带。相比完全无序的波导 QED，局域态占比高达 $(3-	sqrt{5})/2$，当调制频率为 $(1+	sqrt{5})/2$ 时。局域态与非局域态可被平坦和曲面反热带激发。当准周期增强，局域化超放射态完全阻塞单光子传输，提升全反射。在固定准周期强度下，系统尺寸的连续性在传输光谱上表现摩 belongings边缘。工作为非厄米系统局域化 Give 新见解。


<details>
  <summary>Details</summary>
Motivation: 探索准周期结构中光子局域化与散射特性，及其对单光子传输的影响，为非厄米系统局域化研究提供新视角。

Method: 理论分析与数值模拟相结合。通过准周期势展开，分析局域态和非局域态的分布，利用传递矩阵理论研究传输特性。利用分形维数评估局域性，通过系统尺寸变化观察摩 belongings边缘出现。

Result: 在调制频率为 $(1+	sqrt{5})/2$ 时，局域态占比高达 $(3-	sqrt{5})/2$，阻塞单光子传输并增强反射。局域态形成连续带，非局域态支撑超放射传输状态。考虑到大的调制周期下，局域态和非局域态与平坦和曲面反热态相关。

Conclusion: 准周期结构下单光子传输与局域化行为具有独特的物理特性，发现局域态与非局域态的相互作用极大影响传输性能，特别在强调制下阻止单光子传输，提升全反射。
...
### [137] [A Quantum-Memory-Free Quantum Secure Direct Communication Protocol Based on Privacy Amplification of Coded Sequences](https://arxiv.org/abs/2601.21265)
*Shang-Jen Su,Shi-Yuan Wang,Matthieu R. Bloch*

Main category: quant-ph

TL;DR: 该论文提出了一种基于信息论的量子密钥分发(QSDC)方案，称为QMF-QSDC，作为替代常规QKD协议与一次性密钥结合的方法，用于抗集体攻击。主要贡献包括：1) 一种仅依赖于编码序列的无密钥协议，无需传输编码；2) 一组隐私放大定理，用于从编码经典序列中提取密钥，即使存在量子侧向信息。这些成果为设计抗量子攻击的QMF-QSDC方案提供了基础。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的威胁日益加剧，寻找安全的量子通信方案变得尤为重要。该论文针对QKD协议与一次性密钥的结合提出了新的抗集体攻击方法，具有重要的理论和应用价值。

Method: 该论文采用信息论的方法，结合编码技术和隐私放大定理，提出了无需密钥交换的QMF-QSDC方案。通过数学证明，展示了该方案的有效性和安全性。

Result: 论文的结果表明，QMF-QSDC方案在抗集体攻击方面表现优异，尤其是在减少密钥长度和减少通信复杂度方面。此外，该方案在信息论的背景下为量子密钥分发提供了新的思路。

Conclusion: 该研究为量子通信领域提供了新的解决方案，特别是在抗量子攻击和降低资源消耗方面具有重要意义。未来研究可以进一步探索更广泛的应用场景和优化方案。
...
### [138] [QCL-IDS: Quantum Continual Learning for Intrusion Detection with Fidelity-Anchored Stability and Generative Replay](https://arxiv.org/abs/2601.21318)
*Zirui Zhu,Xiangyang Li*

Main category: quant-ph

TL;DR: A quantum-centric continual learning framework, QCL-IDS, efficiently adapts to new attack stages while maintaining legacy capabilities within constrained environments. It achieves superior performance through Q-FISH for stability and QGR for privacy-preserved rehearsal, outperforming traditional methods in attack detection against two datasets.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the critical challenge of continual intrusion detection under resource constraints, proposing a novel framework to enhance adaptability and stability while preserving privacy.

Method: QCL-IDS integrates Q-FISH for parameter constraints and fidelity-based anchoring, combined with QGR using frozen generator snapshots for efficient rehearsal, enabling effective adaptation without excessive storage.

Result: QCL-IDS demonstrates significant improvements in attack detection, achieving higher F1 scores with minimal forgetting compared to sequential fine-tuning methods on two distinct datasets.

Conclusion: QCL-IDS represents a robust solution for quantum-era intrusion detection, successfully balancing adaptability, stability, and privacy constraints.
...
### [139] [Robust Floquet Topological Phases and Anomalous $π$-Modes in Quasiperiodic Quantum Walks](https://arxiv.org/abs/2601.21332)
*F. Iwase*

Main category: quant-ph

TL;DR: 该论文研究了一种受斐波那契模调制硬币参数驱动的一维离散时间量子走的全局拓扑相图，利用平均色心位移（MCD）作为动力学探针，发现存在具有严格量化形变数ν=-1和指数级局域边态的稳健拓扑相，在零能量和拟能量边界E=π处出现具有相同局域化强度的拓扑边态，在准周期性无序环境下，Floquet拓扑保护机制依然有效，为光子实验中观察非平衡态异常现象提供了实证途径。


<details>
  <summary>Details</summary>
Motivation: 该文中通过研究受斐波那契模调制硬币参数驱动的一维离散时间量子走的拓扑相图，揭示了在准周期性无序环境下Floquet拓扑保护机制的存在及其在光子实验中的应用潜力，为量子信息科学和社会网络中的孤波研究提供了理论框架和实证依据。

Method: 该研究利用了傅里叶分析方法和数值模拟相结合的理论分析方法，通过计算平均色心位移（MCD）来表征拓扑相的特性。在分析拓扑相图时，采用了链状体系的有效性分析方法来确认结果的稳健性。

Result: 该研究表明，存在严格量化形变数ν=-1和指数级局域边态的稳健拓扑相。在零能量和拟能量边界E=π处出现具有相同局域化强度的拓扑边态。这些结果表明，Floquet拓扑保护机制在准周期性无序环境下依然有效。

Conclusion: 该研究不仅拓展了Floquet拓扑保护机制的研究范围，也为光子实验中潜在的非平衡态异常现象提供了理论依据，展示了量子信息科学和社会网络中的孤波现象可能具有更广泛的应用前景。
...
### [140] [Quantum steering probes energy transfer in quantum batteries](https://arxiv.org/abs/2601.21398)
*Meng-Long Song,Zan Cao,Xue-Ke Song,Liu Ye,Dong Wang*

Main category: quant-ph

TL;DR: EPR steering plays a crucial role in charge dynamics for quantum batteries.


<details>
  <summary>Details</summary>
Motivation: Quantum batteries are a promising technology for energy storage. This study investigates the role of EPR steering in optimizing their energy dynamics within a shared reservoir charging system.

Method: The researchers optimized parameter configurations in the charging system, tested across multiple low-dissipation scenarios, and analyzed the role of EPR steering in energy equilibrium and storage enhancement.

Result: EPR steering is initially stored and then used to sustain energy storage enhancement. It also acts as a witness for battery population balance and a consumable source of extractable work. The study provides insights into high-dissipation charging regimes.

Conclusion: These findings establish EPR steering as a novel and effective indicator for monitoring energy variations in quantum batteries, which will enhance their high-performance.
...
### [141] [Optimized adiabatic-impulse protocol preserving Kibble-Zurek scaling with attenuated anti-Kibble-Zurek behavior](https://arxiv.org/abs/2601.21435)
*Han-Chuan Kou,Zhi-Han Zhang,Xin-Hui Wu,Yan Zhou,Gang Chen,Peng Li*

Main category: quant-ph

TL;DR: We propose an optimized adiabatic-impulse (OAI) protocol for quantum phase transitions that achieves much shorter evolution time while preserving efficient defect formation (Kibble-Zurek scaling). The protocol combines adiabatic evolution and sudden field changes in a controlled way to minimize quantum defects. The optimal time for performing the field sweep (quench time) scales with system parameters, allowing for faster numerical simulations of phase transitions in various quantum systems, including noisy environments.


<details>
  <summary>Details</summary>
Motivation: Efficiently simulating quantum phase transitions is crucial for understanding high-dimensional quantum systems. Kibble-Zurek scaling provides a powerful framework for describing defect formation during such transitions, but conventional methods are computationally expensive. Our optimized protocol aims to streamline these simulations without sacrificing accuracy.

Method: The protocol combines two approaches: adiabatic evolution near the critical point and sudden impulse away from it. The quench time is used to determine the optimal point where these methods switch, and the scaling of this time with system parameters is analyzed. The protocol also accounts for noisy environments that can disrupt the phase transitions, affecting defect formation.

Result: Our protocol enables faster numerical simulations of quantum phase transitions by significantly reducing the evolution time while maintaining the correct scaling of defect formation. Optimal quench times are determined by the adiabatic coefficient ζ, which depends on the quench rate, and the system's parameters. In noisy environments, the scaling of optimal times shifts, reflecting the presence of anti-Kibble-Zurek behavior.

Conclusion: The optimized protocol provides a practical and efficient method for studying quantum phase transitions through numerical simulations, making it easier to analyze complex systems and phases of matter.
...
### [142] [In-situ benchmarking of fault-tolerant quantum circuits. I. Clifford circuits](https://arxiv.org/abs/2601.21472)
*Xiao Xiao,Dominik Hangleiter,Dolev Bluvstein,Mikhail D. Lukin,Michael J. Gullans*

Main category: quant-ph

TL;DR: 该论文提出了一种方法，能够在运行量子电路的同时从_syndrome数据中表征物理和逻辑错误，比只使用逻辑数据的方法更有效。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算的发展，_efficient和可靠地验证和表征量子设备对于构建大规模量子计算机至关重要。

Method: 提出了一种将故障容错性Clifford电路映射到子系统码的方法，并开发了一个从_syndrome数据估计Pauli噪声的方法。

Result: 证明了物理和逻辑错误在_people运行电路时可被_in situ表征，并通过_syndrome数据估计噪声，从而能够预测逻辑错误率。

Conclusion: 该方法需要的样本量随着多项式增长，即使在代码距离很大时也能显着提高效率。
...
### [143] [RF-free driving of nuclear spins with color centers in silicon carbide](https://arxiv.org/abs/2601.21499)
...
TL;DR: A modified divacancy center in silicon carbide enables control of nuclear spins without RF fields, using microwave pulses and hyperfine enhancements, achieving high-fidelity two-qubit tomography and scalable quantum applications.


<details>
  <summary>Details</summary>
Motivation: The paper aims to simplify and scale quantum devices by eliminating the need for RF fields, which complicates experiments and increases power consumption. This is crucial for quantum sensing and computing with applications in biology, materials science, and geophysics.

Method: The researchers used a modified divacancy center in silicon carbide. They employed microwave pulses to drive electron spins and utilized hyperfine-enhanced effects under a precisely tilted magnetic field to manipulate nuclear spins without RF fields.

Result: They achieved high-fidelity nuclear-spin control with 89% two-qubit tomography fidelity and nearly T1-limited nuclear coherence times, demonstrating a simplified and scalable approach for future quantum applications.

Conclusion: By controlling nuclear spins without RF fields, this method offers a promising route towards scalable and efficient quantum devices suitable for various applications.
...
### [144] [Quantum Simulation with Fluxonium Qutrit Arrays](https://arxiv.org/abs/2601.21507)
*Ivan Amelio,Quentin Ficheux,Nathan Goldman*

Main category: quant-ph

TL;DR: 研究了基于fluxonium超导电路实现和利用qutrit的方法，通过外部磁通偏差调节能级和矩阵元，探讨了数组中的qutrit特性及其用于模拟超奇 matter的可能性，识别了四种不同的操作 regimes，分析了近邻可调和相互作用，以及散射修正和消散，研究了qutrit数组的丰富基态相图，并提出动态实验来探测不同的 regime。结论：fluxonium qutrit arrays 是一种功能多样且易于实现的平台，用于探索超奇 matter 和非阿贝尔拓扑态等超越Bose-Hubbard范式的强相互作用玻色物。


<details>
  <summary>Details</summary>
Motivation: 研究超导电路中的qubit和qutrit特性，及其在量子模拟中的应用，尤其是在超奇 matter 和量子规范理论中的潜力。

Method: 通过调节外部磁通偏差来改变能级和矩阵元，研究了四种不同的操作 regimes，并分析了相互作用和散射修正等。

Result: 识别了四种不同的操作 regimes，发现了近邻可调和相互作用，以及散射修正和消散，研究了基态相图，并提出了动态实验。

Conclusion: 综上所述，fluxonium qutrit arrays 是一种功能多样且易于实现的实验平台，用于探索超奇 matter 和超越Bose-Hubbard范式的强相互作用玻色物。
...
### [145] [High-Coherence and High-frequency Quantum Computing: The Design of a High-Frequency, High-Coherence and Scalable Quantum Computing Architecture](https://arxiv.org/abs/2601.21528)
*Masroor H. S. Bukhari*

Main category: quant-ph

TL;DR: Plan proposes an 8-qubit transmon quantum computing architecture with frequencies exceeding 10 GHz, backed by new high-performance topologies to achieve high-fidelity, compact, and scalable quantum computing. Though transmon qubits are typically operated in the 4–6 GHz range, this advancement showcases potential for higher-frequency qubit design using advanced manufacturing techniques.


<details>
  <summary>Details</summary>
Motivation: Achieving quantum advantage in computing requires qubits with long coherence times, low errors, and high frequencies. The proposed architecture aims to push the boundaries of transmon qubit frequencies, addressing challenges in scalability and integration.

Method:  The study proposes a scalable 8-qubit transmon architecture with advancements in superconducting junction manufacturing. It integrates high-performance topologies and demonstrates feasibility through frequency extrapolation and experimental techniques.

Result: The architecture surpasses 10 GHz in qubit frequencies, achieves compact and scalable functionality, and shows viable pathways for implementation with current fabrication techniques. The theoretical coherence times and low charge noise support potential quantum advantage applications.

Conclusion: This research paves the way for scalable, high-frequency, and low-error quantum computing architectures. Further experimental validation and industrial integration will be crucial to unlocking the full potential of these advanced qubit designs.
...
### [146] [Cooperative Emission from Quantum Emitters in Hexagonal Boron Nitride Layers](https://arxiv.org/abs/2601.21544)
*Igor Khanonkin,Amir Sivan,Le Liu,Johannes Eberle,Kenji Watanabe,Takashi Taniguchi,Gadi Eisenstein,Meir Orenstein*

Main category: quant-ph

TL;DR: 本文利用 hBN 厚膜层和电子束激活的单光子辐射源实现了室温条件下的集体发光效应。直至目前尚无 reports 展现这种效应。通过共聚焦显微镜和 Hanbury Brown-Twiss 安置，识别了孤立发射体和经局域电子束激发的体群发射体。时间分辨率测量显示，紧密体群的发射强度增加速度超过线性，发射速率在 500 皮秒左右（接近实验系统的时间分辨率），相比单发射体或空间延展性体群的寿命约 1.85纳秒。二阶 photon 自相关测量揭示 出次 Poissonian 的反差峰，结论支持来自于几个不可区分发射体的发射。同时观察到发射速率增加和发射时间缩减，直接证明了室温条件下的集体发射效应。这些结果奠定了 hBN 缺陷群集在其二维材料中的光活性平台基础，为工程设计集体量子光学现象提供了实现途径，如超亮的超辐射光源和非经典的光子态，为量子科技应用开辟道路。


<details>
  <summary>Details</summary>
Motivation: 量子光学领域的集体发光现象被广泛认为是量子效应的重要体现。然而，多数人工合成的材料中难以实现集体发光效应，尤其是在室温条件下。近年来，新的二维材料（如 hBN）由于其天然的异质结构，被寄望可能成为实现新型光子器件和量子操作的优秀平台。然而，集体激发下的超辐射现象还未被证实，特别是在二维材料如 hBN 上。这是本文研究和实验的核心动机之一。

Method: 该研究利用高度集成的实验方案，主要的实验手段包括：1. 事故层 hBN 作为发光介质，其天然层间异质结构保证了具有大 Ban ginterval 的光电子束缚态。2. 利用电子束均匀且可控地激活 hBN 中的单光子辐射源，这些激发的光子具有良好的局域性。3. 共聚焦显微镜和 Hanbury Brown-Twiss 共振配置用于 识别孤立发射体和局域电子激发的体群发射体。4. 利用时间分辨率极高的显微光谱仪进行时间分辨测量，用于观察发射速率和寿命。5. 二阶 photon 自相关测量用于检测次 Poissonian 的发射统计特性。同源光强测量用于 monitor 发射体的环境依赖性和发射速率。

Result: 文章表明，在 hBN 厚膜层中，由局域电子激发引发的群体单光子源实现了室温条件下的集体辐射效应。这种效应的实现需要依赖于发射体的不可区分性（即，群集发射体的发射频率远大于单体发射体的频率），并且发射体的聚集程度与发射速率的增强和发射寿命的缩短成正比。通过光谱学和自相关测量，该研究否定了辐射体的波粒二象性（即，同时观察的为局域性和宏观集体性）。此外，该研究还通过对比不同体群发射体的统计发射特性和寿命，证实了集体辐射效应的真实性。

Conclusion: 该研究通过 hBN 厚膜层中单光子辐射体的局域电子激发在零度条件下的群体激发，验证了集体由相互作用的光子引发的超辐射效应。对于理解二维材料中的集体量子效应以及在二维材料中的超辐射应用具有重要意义。该研究结果展示了二维材料（如 hBN）为集体量子效应提供了一个新的、可控的开发平台。接着可以摆脱依赖于光谐振腔或 cryogenic 的低温条件，基于二维材料的集体量子效应探索进一步研究，为超亮高效辐射源和非经典的光子态的工程制造奠定基础。;
...
### [147] [Entanglement of quantum systems via a classical mediator in hybrid van Hove theory](https://arxiv.org/abs/2601.21555)
...
### [148] [A biased-erasure cavity qubit with hardware-efficient quantum error detection](https://arxiv.org/abs/2601.21616)
*Jiasheng Mai,Qiyu Liu,Xiaowei Deng,Yanyan Cai,Zhongchu Ni,Libo Zhang,Ling Hu,Pan Zheng,Song Liu,Yuan Xu,Dapeng Yu*

Main category: quant-ph

TL;DR: 实现了一个高效的偏态擦除准QC位，成功展示了硬件高效的偏态擦除位编码在单一微波腔中，通过使用移码辅助测量和:before mid-circuit擦除检测，实现了99.3%的泄露错误转化为擦除检测。通过后选择排除擦除，实现了逻辑去 olds relaxation和dephasing比率分别超过擦除错误率31倍和15倍，实现了比 cavity中最低两个Fock态编码更好硬件效率的偏态擦除位。开创了量子误差校正的新方法和硬件架构。


<details>
  <summary>Details</summary>
Motivation: 量子错误更正是现代量子计算中的关键议题， improve qubit 的抗干扰能力对其性能至关重要。本研究在实现硬件高效的偏态擦除qubit方面做出了重要进展，展示了通过移码辅助测量和 mid-circuit擦除检测等先进方法，显著提高错误捕获效率，为量子容错计算奠定了基础。

Method: 使用移码辅助测量和 mid-circuit擦除检测的方法， qubit 嵌入真空和两 photon Fock态，记录测后错误发生率降低到1%以下，通过后选择排除擦除，实现了高的逻辑错误率阈值，之后实现了比物理qubit更好的性能。

Result: 实现了高效的偏态擦除qubit，成功实现了硬件高效、抗干扰能力强的量子比特结构，相关技术参数包括很高的擦除比能、逻辑错误率和显著的去 old比和去 old过程中所能获得的最大倍数等。

Conclusion: 该研究为量子误差更正和量子计算中进一步优化硬件架构提供了重要思路和方法，展示了偏态擦除位在量子计算中的潜力。
...
### [149] [Reinforcement Learning for Adaptive Composition of Quantum Circuit Optimisation Passes](https://arxiv.org/abs/2601.21629)
*Daniel Mills,Ifan Williams,Jacob Swain,Gabriel Matos,Enrico Rinaldi,Alexander Koziell-Pipe*

Main category: quant-ph

TL;DR: A reinforcement learning agent has been trained to compose quantum circuit optimization pass sequences and outperforms standard sequences in reducing the number of two-qubit gates.


<details>
  <summary>Details</summary>
Motivation: With the growing complexity of quantum circuits and the demand for efficient quantum software development, optimizing circuit compositions is crucial. However, designing optimal pass sequences requires specialized knowledge, making an automated approach desirable.

Method: A reinforcement learning agent was developed to learn optimal pass sequences. The action space includes passes for two-qubit gate reduction, and the agent was trained on a diverse set of circuits.

Result: The agent achieved a mean of 57.7% reduction in two-qubit gates, outperforming the next best default pass sequence which achieved 41.8%. The median improvement was also 56.7% compared to 50.0%.

Conclusion: This work demonstrates that reinforcement learning can effectively automate the composition of quantum circuit optimization passes, improving upon standard approaches and highlighting the potential for automated optimization in quantum software development.
...
### [150] [Hierarchical quantum decoders](https://arxiv.org/abs/2601.21715)
*Nirupam Basak,Ankith Mohan,Andrew Tanggara,Tobias Haug,Goutam Paul,Kishor Bharti*

Main category: quant-ph

TL;DR: A family of hierarchical quantum decoders with a tunable trade-off between speed and accuracy is introduced, leveraging the Lasserre SOS hierarchy. The approach achieves near-optimal performance on rotated surface codes and honeycomb color codes, outperforming standard Linear Programming relaxations even at low hierarchy levels.


<details>
  <summary>Details</summary>
Motivation: Developing efficient and accurate quantum decoders is crucial for fault-tolerant quantum computing. This paper aims to bridge the gap between heuristic-based decoders and rigorous optimal decoding by introducing a novel family of decoders based on the Lasserre SOS hierarchy.

Method: The proposed decoders use the Lasserre Sum-of-Squares (SOS) hierarchy to relax the decoding problem, converting it into a sequence of Semidefinite Programs (SDPs). The method analyzes the trade-off between speed and accuracy, demonstrating that even low-level hierarchies perform nearly as well as exact decoding.

Result: The SOS decoder significantly outperforms standard Linear Programming relaxations. On rotated surface codes and honeycomb color codes, Levels 2 and 3 of the hierarchy achieve performance nearly equivalent to the exact solver.

Conclusion: This work provides a practical and rigorous approach to quantum decoding with tunable precision, advancing the field toward efficient fault-tolerant quantum computing.
...
### [151] [Quantum Random Features: A Spectral Framework for Quantum Machine Learning](https://arxiv.org/abs/2601.21746)
*Akitada Sakurai,Aoi Hayashi,William John Munro,Kae Nemoto*

Main category: quant-ph

TL;DR: Efficient quantum machine learning models using Random Features achieve high accuracy on large datasets.


<details>
  <summary>Details</summary>
Motivation: Improving quantum machine learning efficiency and scalability.

Method: Introduce Quantum Random Features (QRF) and Quantum Dynamical Random Features (QDRF) using Z-rotations and random encoding to bypass variational optimization, enhancing efficiency.

Result: Reach 89.3% accuracy on Fashion-MNIST, surpassing classical benchmarks with scalable qubit requirements.

Conclusion: QRFF and QDRF offer scalable quantum learning solutions compatible with near-term quantum hardware.
...
### [152] [A geometric criterion for optimal measurements in multiparameter quantum metrology](https://arxiv.org/abs/2601.21801)
...
### [153] [Schroedinger's principle eliminates the EPR-locality paradox](https://arxiv.org/abs/2601.21806)
*Walter F. Wreszinski*

Main category: quant-ph

TL;DR: 该论文指出，在量子力学的哥本哈根解释中，爱因斯坦的EPR悖论不存在，通过引用薛定谔的论文和反驳阿里卡和杨 Knightse的局部性测量观点，得出了这一结论。


<details>
  <summary>Details</summary>
Motivation: 探讨量子力学中局部性问题，尤其是在纠缠态测量中的局部性，具有重要意义。

Method: 研究分析薛定谔的方法，通过假设波函数 collapse 的机制和简化模型, 论述了EPR悖论的不存在。

Result: 证明爱因斯坦的EPR悖论在哥本哈根解释中不存在，不依赖于局部性测量的其他限制。

Conclusion: 强调哥本哈根解释中波函数 collapse 的独特性。
...
### [154] [Error-detectable Universal Control for High-Gain Bosonic Quantum Error Correction](https://arxiv.org/abs/2601.21838)
*Weizhou Cai,Zi-Jie Chen,Ming Li,Qing-Xuan Jie,Xu-Bo Zou,Guang-Can Guo,Luyan Sun,Chang-Ling Zou*

Main category: quant-ph

TL;DR: 保护量子信息通过量子错误校正（QEC）是未来功能性量子计算的基石，但目前QEC保护的逻辑量子位 coherence时间仅是其最好物理组成部分的两倍。我们发现， Ancilla诱导的 operational 错误成为QEC收益提升的主要阻碍，而非 cavity 内部的固有相干性。为此，我们引入了可以通过检测和丢弃 Ancilla的轨迹来实现可探测的泛型控制的 bosonic 模式控制。对于二项式代码，我们在 99.6% 的 fidelity 达到 8.33 倍的QEC 收益，这超过了平衡点。实验结果表明，通过当前先进设备，超越 10 倍的QEC 收益是可行的，为实现功能性 bosonic 量子计算奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 保护量子信息的QEC技术是未来量子计算发展的关键，但实际应用中QEC的效果并未达到理论预期，特别是在逻辑量子位的coherence时间上。”,

Method: 我们通过实验观察了 Ancilla relaxation 事件对 operational 错误的影响，并设计了Error-Detectable Universal Control的机制，可以通过检测和丢弃Ancilla轨迹来避免这些错误。对于 binomial codes，通过这种方法实现了 high-fidelity 的量子门操作，验证了QEC收益的显著提升。

Result: 实验结果表明，对于二项式代码，我们实现了超过99.6%的量子门 fidelity，并获得了8.33倍的QEC收益，超过了平衡点。这说明通过当前先进设备技术，超越10倍的QEC收益是可行的。

Conclusion: 通过Error-Detectable Universal Control的机制，我们可以显著减少 Ancilla诱导的 operational 错误，从而实现更高的QEC收益。这对于实现功能性 bosonic 量子计算具有重要的实验和理论意义。
...
### [155] [A Bravyi-König theorem for Floquet codes generated by locally conjugate instantaneous stabiliser groups](https://arxiv.org/abs/2601.21863)
*Jelena Mackeprang,Jonas Helsen*

Main category: quant-ph

TL;DR: 该论文表明，Floquet 代码遵循 Bravyi-König 定理，即它们实现的总体正则运算位于第 $D$ 级Clifford谱系。


<details>
  <summary>Details</summary>
Motivation: 我们想探讨Floquet 代码的容错性，而Bravyi-König定理为理解Indices代码的能力程度提供理论基础。

Method: 通过定义局部共轭稳定群并引入扩展单元，并分析其对 codespace 的影响，我们可以证明其符合BK定理。

Result: 结果显示，所有满足条件的扩展逻辑运算确实位于第 $D$ 级Clifford谱系。

Conclusion: Floquet 代码继承了Indices代码的无一看护能力，这是其在量子计算中的潜在应用之一。
...
### [156] [Entanglement-Assisted Bosonic MAC: Achievable Rates and Covert Communication](https://arxiv.org/abs/2601.21869)
*Yu-Chen Shen,Matthieu R. Bloch*

Main category: quant-ph

TL;DR: 研究了有纠缠辅助的玻色子多信道访问中的隐式通信问题，得出了闭式可实现速率区域，并在低光子数情况下速率区域缩小为矩形，随着多用户干扰的消逝逐步与点对点容量趋同；还表明确立纠缠时可实现秘密传输容量与块长n呈$	heta(ightarrow)$关系，远超点对点情况下的平方根法律；发现联合隐式传输的限制条件导致通信双方传输速率呈线性 trade-off。


<details>
  <summary>Details</summary>
Motivation: 随着通信网络的复杂化和需求的增长，如何在多用户协作下实现高效、安全的通信成为研究重点。

Method: 利用高阶相移键控调制，并考虑低光子数情况下的 communications限制，分析能够实现的速率范围，得出’ll通过公式推导和理论分析’的方法。

Result: 在低光子数下，速率区域收缩为矩形，这表明随着干扰减少，系统容量趋近于点对点容量；同时，通过纠缠辅助，双方可实现的传输容量与块长n呈$	heta(ightarrow)$关系，即增长速率高于单点对点情况；此外，联合信号的刻画揭示了通信双方传输速率间的线性 trade-off关系。

Conclusion: 研究结果发现，在有纠缠辅助的玻色子多用户访问系统中，通过高阶相移键控调制，不仅显著提升了隐式传输的容量，且通过减少多用户干扰能进一步优化系统性能；同时，纠缠辅助为通信双方提供了更灵活的速率调节方式，实现了更高的安全和效率。
...
### [157] [A scalable quantum-enhanced greedy algorithm for maximum independent set problems](https://arxiv.org/abs/2601.21923)
*Elisabeth Wybo,Jami Rönkkö,Olli Hirviniemi,Jernej Rudi Finžgar,Martin Leib*

Main category: quant-ph

TL;DR: A hybrid QAOA-classical algorithm for solving Maximum Independent Set (MIS) on regular graphs, which improves upon classical heuristics by combining quantum angle pre-computation with greedy decisions, implemented on a 20-qubit device and simulated for larger graphs.


<details>
  <summary>Details</summary>
Motivation: Exploring hybrid quantum-classical algorithms for combinatorial optimization problems on constrained hardware platforms.

Method: Combines QAOA with classical greedy algorithm, precomputes QAOA angles for regular trees, uses local expectation values for sequential greedy decisions, implemented on a 20-qubit IQM device; simulations for larger graphs not possible with current hardware.

Result: At p=4 depth, quantum-enhanced greedy outperforms classical baselines and more advanced approximation algorithms. Algorithm modular and resource-efficient, good candidate for NISQ hybrid optimization.

Conclusion: Hybrid approach yields improved MIS solutions on regular graphs using shallow QAOA, promising for practical quantum-classical optimization in NISQ era.
...
### [158] [Entropy production versus memory effects in two-level open quantum systems](https://arxiv.org/abs/2601.21930)
*Guillaume Théret,Dominique Sugny,Camille L. Latune*

Main category: quant-ph

TL;DR: The paper compares various entropy production rate definitions from literature, analyzing their relation with memory effects. It examines a qubit interacting with a single bosonic mode as a finite bath, showing that entropy production definitions coincide at weak coupling. In strong coupling, significant discrepancies exist between some definitions, though some similarities remain. Surprisingly, two definitions—based on local and non-local system quantities—coincide exactly, even under strong coupling. The paper also finds a high degree of correspondence between memory effects characterized by P-divisibility and the sign of entropy production rates in weak coupling but degrades at strong coupling. Finally, it extends the concept of entropy production to dynamical maps, showing perfect equivalence between the sign of this extended entropy production and P-divisibility in phase-covariant master equations.


<details>
  <summary>Details</summary>
Motivation: The purpose of this work is to analyze the different definitions of entropy production rate in the literature and their relation to memory effects, particularly in the context of a qubit interacting with a single bosonic mode serving as a finite bath. The goal is to understand how these definitions compare under weak and strong coupling regimes and how memory effects, characterized by P-divisibility, relate to the sign of entropy production rates.

Method: The study utilizes a theoretical framework to compare several entropy production rate definitions. It employs a relevant experimental setup involving a qubit interacting with a single bosonic mode. The analysis is conducted under two coupling regimes: weak and strong. The paper also extends the concept of entropy production to the dynamical map framework to further analyze the relationship between entropy production and P-divisibility in phase-covariant master equations.

Result: The key findings of the paper are that, under weak coupling, all entropy production definitions coincide. However, under strong coupling, significant discrepancies arise between some definitions, though some similarities remain. Surprisingly, two definitions—based on local and non-local system quantities—coincide exactly, even under strong coupling. Additionally, the paper finds a high degree of correspondence between memory effects characterized by P-divisibility and the sign of entropy production rates in weak coupling. However, this correspondence degrades at strong coupling. The paper also extends the concept of entropy production to dynamical maps and establishes a perfect equivalence between the sign of this extended entropy production and P-divisibility in phase-covariant master equations.

Conclusion: This paper makes a significant contribution to understanding the diversity of entropy production rate definitions in the literature and their relationship to memory effects. By analyzing these definitions in both weak and strong coupling regimes, the paper reveals insights into the behavior of entropy production under different conditions. Additionally, the extension of entropy production to dynamical maps provides a deeper understanding of the relationship between the sign of entropy production and P-divisibility.
...
### [159] [Hierarchy of discriminative power and complexity in learning quantum ensembles](https://arxiv.org/abs/2601.22005)
...
### [160] [Machine learning with minimal use of quantum computers: Provable advantages in Learning Under Quantum Privileged Information (LUQPI)](https://arxiv.org/abs/2601.22006)
*Vasily Bokov,Lisa Kohl,Sebastian Schmitt,Vedran Dunjko*

Main category: quant-ph

TL;DR: 使用量子计算机增强特征提取的半监督学习模型能显著提高学习性能。


<details>
  <summary>Details</summary>
Motivation: 量子计算在机器学习中的潜力被广泛认可，但如何证明量子计算对经典计算的优势仍然是一个未解之谜。本文探索了仅用量子计算机提取特征的前提下，量子 enhance 学习的潜在能力。

Method: 本文提出了 Learning Under Quantum Privileged Information (LUQPI) 模型，该模型将量子增强的特征作为在训练期间使用，但不用于部署。模型基于 Learning Under Privileged Information (LUPI) 的经典框架，通过量子计算提取特征并将其添加到训练数据集中。

Result: 在适当的概念类和数据分布下，即使是最小的量子增强特征提取也能导致指数级的量子-经典分离。此外，LUQPI 模型在许多体物理模型中通过量子增强的特征实现了性能优于经典基线的稳健结果。

Conclusion: 量子计算机在半监督学习中的作用是值得进一步探索的，尤其是在仅用于增强特征提取的最小模型中，潜在的巨大优势得以揭示。
...
### [161] [Thermodynamics of linear open quantum walks](https://arxiv.org/abs/2601.22064)
...
### [162] [Designing quantum technologies with a quantum computer](https://arxiv.org/abs/2601.22091)
*Juan Naranjo,Thi Ha Kyaw,Gaurav Saxena,Kevin Ferreira,Jack S. Baker*

Main category: quant-ph

TL;DR: The paper presents a quantum computer-aided framework for simulating interacting spin systems in solids, particularly for designing solid-state spin-qubit technologies. The framework incorporates advanced quantum algorithms and mappings to efficiently simulate long-time dynamics under experimental constraints. The framework achieves significant reductions in gate counts and circuit depth for time-evolution simulations compared to unoptimized methods. The accuracy of the simulations is primarily influenced by the reference-state selection in the quantum Krylov fast-forwarding algorithm.


<details>
  <summary>Details</summary>
Motivation: The motivation for this work stems from the growing importance of spin systems in quantum technologies, including quantum sensors, single-photon sources, and quantum registers/processors. The increasing complexity of these systems requires efficient simulation tools, especially for optimizing their performance on early fault-tolerant quantum hardware.

Method: The method employed in the paper involves developing a general Hamiltonian model for electron spin resonance systems, incorporating zero-field splitting, Zeeman effect, hyperfine interactions, dipole-dipole interactions, and electron-phonon decoherence. The authors then integrate Gray-encoded qudit-to-qubit mappings, qubit-wise commuting aggregation, and a multi-reference selected quantum Krylov fast-forwarding (sQKFF) hybrid algorithm to simulate long-time dynamics while respecting NISQ constraints.

Result: The results demonstrate that the proposed framework can compute autocorrelation functions over tens of nanoseconds, along with microwave absorption spectra and measures of quantum coherence. The simulations show that the framework reduces gate counts and circuit depths by 18-30% compared to unoptimized implementations. When using the nitrogen vacancy center in diamond as a testbed, the framework's accuracy is found to be primarily dependent on the reference-state selection in the sQKFF algorithm, while maintaining computational efficiency.

Conclusion: The framework developed in this paper provides a robust and efficient method for simulating and optimizing solid-state spin-qubit systems. It offers a clear pathway forward for advancing quantum technologies by enabling accurate and resource-efficient simulations on currently available quantum hardware.
...
### [163] [Experimental observation of ballistic correlations in integrable turbulence](https://arxiv.org/abs/2601.21085)
...
TL;DR: 光分领域通过实验观察到了双时间、色散关系下的非平衡统计特性，验证了广义流体动力学在可积李 bikes中的适用性。


<details>
  <summary>Details</summary>
Motivation: 该研究试图通过实验观察和理论分析非平衡光场的统计特性，验证广义流体动力学在可积系统中的适用性。

Method: 采用光环路和非调 Sig检测器，实时捕捉光场的空间时间动态，计算相干度，结合逆散射分析，验证理论预测。

Result: 相干度在时空缩放下崩溃，与广义流体动力学一致，验证了理论预测。

Conclusion: 实验结果支持了在可积系统中广义流体动力学的普适性，为理解复杂光场提供了新视角。
...
### [164] [Multiple binding modes of AKT on PIP$_3$-containing membranes](https://arxiv.org/abs/2601.21216)
*Yuki Nakagaki,Eiji Yamamoto*

Main category: physics.bio-ph

TL;DR: The paper explores how the PI3K/AKT signaling pathway regulates AKT activation on cellular membranes by studying the interactions between the AKT kinase domains and PIP$_3$ in lipid bilayers.


<details>
  <summary>Details</summary>
Motivation: Understanding the molecular mechanisms behind AKT activation is crucial for gaining insights into cellular signaling pathways and their role in various cellular processes.

Method: The research employs molecular dynamics simulations to analyze full-length AKT on PIP$_3$-containing lipid bilayers, identifying four distinct membrane-binding modes through computational modeling.

Result: The study reveals that PIP$_3$ interacts with both the PH and kinase domains of AKT, influencing its membrane-bound conformations. The interactions in the PH domain are site-specific, and the kinase domain's activation-loop phosphorylation site is exposed to solvent under certain conditions. The membrane-bound conformations and AKT activation are also affected by PIP$_3$ concentration.

Conclusion: This biomolecular approach enhances our understanding of lipid-protein interactions and could inform future research on molecular mechanisms involving signaling molecules, opening possibilities for therapeutic applications.
